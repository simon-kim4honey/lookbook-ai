...
`,mt=`CASE WHEN email LIKE '%@%' AND email NOT LIKE '%**%' THEN email ELSE crawled_email END`,ht=`COALESCE(NULLIF(bzmnNm, ''), NULLIF(domain_clean, ''), '거래처')`;B.get(`/mail-batch/status`,async e=>{let t=e.env.LOOKBOOK_DB,n=(await t.prepare(`SELECT COUNT(*) AS n FROM biz_leads WHERE ${pt}`).first())?.n||0,r=await t.prepare(`
  ...
  `).bind(t).all();if(!n.length)return e.json({success:!1,message:`해당 배치를 찾을 수 없습니다.`},404);let r=lt(n.map(e=>({name:e.name,email:e.email})));return new Response(r,{status:200,headers:{"Content-Type":`application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`,"Content-Disposition":`attachment; filename="bizleads_mail_batch_${t}.xlsx"`,"X-Batch-Id":String(t),"X-Batch-Count":String(n.length)}})});var gt=`mu1ysko0`;function _t(e){return e.env.TECHPACK_MENU_VISIBLE===`true`}var vt=e=>e?`
...
<\/script>
       ...
       FROM credit_logs WHERE user_id = ? ORDER BY created_at DESC, id DESC LIMIT ? OFFSET ?`).bind(n,i,a).all();return e.json({success:!0,events:s.results,total:o?.cnt||0,page:r,limit:i})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.patch(`/api/admin/users/:id`,W,async e=>{try{let t=e.env.LOOKBOOK_DB,n=await e.req.json(),r=e.req.param(`id`),i=[],a=[];if(n.status!==void 0&&(i.push(`status = ?`),a.push(n.status)),n.role!==void 0&&(i.push(`role = ?`),a.push(n.role)),n.set_password!==void 0){if(typeof n.set_password!=`string`||n.set_password.length<6)return e.json({success:!1,message:`비밀번호는 6자 이상이어야 합니다.`},400);i.push(`password_hash = ?`),a.push(await gn(n.set_password))}if(n.add_credits!==void 0){let o=(await t.prepare(`SELECT credits FROM users WHERE id = ?`).bind(r).first())?.credits??0,s=parseInt(n.add_credits),l=Math.max(0,o+s);return i.push(`credits = ?`),a.push(l),i.push(`updated_at = datetime('now')`),await t.prepare(`UPDATE users SET ${i.join(`, `)} WHERE id = ?`).bind(...a,r).run(),await t.prepare(`INSERT INTO credit_logs (user_id, type, amount, balance, reason, ref_id)
         VALUES (?, 'grant', ?, ?, 'admin_grant', ?)`).bind(r,s,l,`admin_${Date.now()}`).run(),n.status===`suspended`&&await t.prepare(`DELETE FROM user_sessions WHERE user_id = ?`).bind(r).run(),e.json({success:!0,newCredits:l})}if(n.credits!==void 0){let o=(await t.prepare(`SELECT credits FROM users WHERE id = ?`).bind(r).first())?.credits??0,s=parseInt(n.credits),l=s-o;return i.push(`credits = ?`),a.push(s),i.push(`updated_at = datetime('now')`),await t.prepare(`UPDATE users SET ${i.join(`, `)} WHERE id = ?`).bind(...a,r).run(),l!==0&&await t.prepare(`INSERT INTO credit_logs (user_id, type, amount, balance, reason, ref_id)
           VALUES (?, ?, ?, ?, 'admin_set', ?)`).bind(r,l>0?`grant`:`deduct`,l,s,`admin_${Date.now()}`).run(),n.status===`suspended`&&await t.prepare(`DELETE FROM user_sessions WHERE user_id = ?`).bind(r).run(),e.json({success:!0,newCredits:s})}return i.length===0?e.json({success:!1,message:`변경할 항목이 없습니다.`},400):(i.push(`updated_at = datetime('now')`),await t.prepare(`UPDATE users SET ${i.join(`, `)} WHERE id = ?`).bind(...a,r).run(),n.status===`suspended`&&await t.prepare(`DELETE FROM user_sessions WHERE user_id = ?`).bind(r).run(),e.json({success:!0}))}catch(t){return e.json({success:!1,message:t.message},500)}}),V.delete(`/api/admin/users/:id`,W,async e=>{try{let t=e.env.LOOKBOOK_DB,n=e.req.param(`id`);return await t.prepare(`UPDATE users SET status = 'deleted', updated_at = datetime('now') WHERE id = ?`).bind(n).run(),await t.prepare(`DELETE FROM user_sessions WHERE user_id = ?`).bind(n).run(),e.json({success:!0})}catch(t){return e.json({success:!1,message:t.message},500)}});var On=async(e,t)=>{let n=e.env.LOOKBOOK_DB,r=await vn(n,e.req.header(`X-Session-Token`)||null);if(!r)return e.json({success:!1,message:`로그인이 필요합니다.`},401);if(r.role!==`bfm_admin`)return e.json({success:!1,message:`BFM 관리자 권한이 없습니다.`},403);await t()};V.post(`/api/bfm/login`,async e=>{try{let t=e.env.LOOKBOOK_DB,n=await e.req.json(),r=(n.email||``).trim().toLowerCase(),i=n.password||``;if(!r||!i)return e.json({success:!1,message:`이메일과 비밀번호를 입력해주세요.`},400);let a=await t.prepare(`SELECT * FROM users WHERE email = ?`).bind(r).first();if(!a||a.status!==`active`||!a.password_hash||!await _n(i,a.password_hash))return e.json({success:!1,message:`이메일 또는 비밀번호가 올바르지 않습니다.`},401);if(a.role!==`bfm_admin`)return e.json({success:!1,message:`BFM 관리자 권한이 없습니다.`},403);let o=await yn(t,a.id);return e.json({success:!0,token:o,user:bn(a)})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.get(`/api/bfm/members`,On,async e=>{try{let t=e.env.LOOKBOOK_DB,n=Math.max(1,parseInt(e.req.query(`page`)||`1`)),r=Math.max(1,Math.min(50,parseInt(e.req.query(`limit`)||`20`))),i=(n-1)*r,a=await t.prepare(`SELECT COUNT(*) as cnt FROM users WHERE referrer = 'BFM회원'`).first(),o=await t.prepare(`SELECT id, name, email, credits, status, created_at FROM users WHERE referrer = 'BFM회원' ORDER BY created_at DESC LIMIT ? OFFSET ?`).bind(r,i).all();return e.json({success:!0,members:o.results,total:a?.cnt||0,page:n,limit:r})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.get(`/api/bfm/members/:id/payments`,On,async e=>{try{let t=e.env.LOOKBOOK_DB,n=e.req.param(`id`);if(!await t.prepare(`SELECT id FROM users WHERE id = ? AND referrer = 'BFM회원'`).bind(n).first())return e.json({success:!1,message:`조회 권한이 없는 회원입니다.`},403);let r=Math.max(1,parseInt(e.req.query(`page`)||`1`)),i=Math.max(1,parseInt(e.req.query(`limit`)||`10`)),a=(r-1)*i,o=await t.prepare(`SELECT COUNT(*) as cnt FROM payment_logs WHERE user_id = ?`).bind(n).first(),s=await t.prepare(`SELECT order_id, amount, credits, status, pg_provider, currency, pg_method, created_at, paid_at
       ...
       VALUES (?, ?, ?, ?, 'pending', 'toss', 'KRW')`).bind(r.user_id,u,s,a.credits).run(),e.json({success:!0,orderId:u,amount:s,credits:a.credits,orderName:a.label,customerName:r.name,customerEmail:r.email,userId:r.user_id,clientKey:e.env.TOSS_CLIENT_KEY||``})}catch(t){return e.json({success:!1,message:t.message},500)}});async function An(e,t){let n=await crypto.subtle.importKey(`raw`,new TextEncoder().encode(e),{name:`HMAC`,hash:`SHA-256`},!1,[`sign`]),r=await crypto.subtle.sign(`HMAC`,n,new TextEncoder().encode(t));return Array.from(new Uint8Array(r)).map(e=>e.toString(16).padStart(2,`0`)).join(``)}V.post(`/api/stripe/checkout`,async e=>{let t=e.env.STRIPE_SECRET_KEY;if(!t)return e.json({success:!1,message:`서버 설정 오류: STRIPE_SECRET_KEY 미설정`},500);let n=e.env.LOOKBOOK_DB,r=e.req.header(`X-Session-Token`)||``;if(!r)return e.json({error:`로그인이 필요합니다.`},401);let i=await n.prepare(`SELECT s.user_id, u.email, u.referrer FROM user_sessions s
     JOIN users u ON u.id = s.user_id
     WHERE s.token = ? AND s.expires_at > datetime('now')`).bind(r).first();if(!i)return e.json({error:`세션이 만료되었습니다.`},401);try{let{packageId:r,currency:a}=await e.req.json(),o=kn[r];if(!o)return e.json({error:`잘못된 패키지입니다.`},400);let s=String(a||`USD`).toUpperCase();if(s!==`USD`&&s!==`JPY`)return e.json({error:`지원하지 않는 통화입니다.`},400);let l=s===`USD`?o.usdCents:o.jpyAmount,u=Dn[i.referrer]||0,d=u>0?Math.round(l*(1-u)):l,f=`lookbook-${i.user_id.slice(0,6)}-${Date.now()}-${Math.random().toString(36).slice(2,6).toUpperCase()}`;await n.prepare(`INSERT INTO payment_logs (user_id, order_id, amount, credits, status, pg_provider, currency)
       ...
       LIMIT 50`).all();return e.json({success:!0,jobs:n.results||[]})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.get(`/api/admin/debug/atlas-job/:jobId`,W,async e=>{let t=e.req.param(`jobId`),n=e.env.LOOKBOOK_DB;try{let r=await n.prepare(`SELECT id, job_id, kind, status, video_url, created_at FROM generation_logs WHERE job_id = ?`).bind(t).first(),i=await fetch(`${H}/api/v1/model/prediction/${t}`,{headers:{Authorization:`Bearer ${e.env.ATLAS_API_KEY}`}}),a=await i.text(),o=null;try{o=JSON.parse(a)}catch{}return e.json({dbRow:r,atlasHttpStatus:i.status,atlasRaw:o??a})}catch(t){return e.json({success:!1,message:t.message},500)}});async function Un(e){try{let t=await e.prepare(`SELECT value FROM app_settings WHERE key = 'admin_prompt_config'`).first();if(t?.value){let e=JSON.parse(t.value);return{enabled:typeof e.enabled==`boolean`?e.enabled:G.enabled,prefix:typeof e.prefix==`string`?e.prefix:G.prefix,suffix:typeof e.suffix==`string`?e.suffix:G.suffix,styleGuide:typeof e.styleGuide==`string`?e.styleGuide:G.styleGuide,technicalSpec:typeof e.technicalSpec==`string`?e.technicalSpec:G.technicalSpec,updatedAt:e.updatedAt||G.updatedAt}}}catch(e){console.warn(`d1GetPromptConfig fallback to memory:`,e)}return G}async function Wn(e,t){await e.prepare(`INSERT INTO app_settings (key, value, updated_at) VALUES ('admin_prompt_config', ?, datetime('now'))
     ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = excluded.updated_at`).bind(JSON.stringify(t)).run()}V.get(`/api/admin/prompt`,W,async e=>{try{let t=e.env.LOOKBOOK_DB,n=await Un(t);return e.json({success:!0,config:n})}catch{return e.json({success:!0,config:G})}}),V.put(`/api/admin/prompt`,W,async e=>{try{let t=e.env.LOOKBOOK_DB,n=await e.req.json(),r=await Un(t),i={enabled:typeof n.enabled==`boolean`?n.enabled:r.enabled,prefix:typeof n.prefix==`string`?n.prefix:r.prefix,suffix:typeof n.suffix==`string`?n.suffix:r.suffix,styleGuide:typeof n.styleGuide==`string`?n.styleGuide:r.styleGuide,technicalSpec:typeof n.technicalSpec==`string`?n.technicalSpec:r.technicalSpec,updatedAt:new Date().toISOString()};return await Wn(t,i),G=i,console.log(`Admin prompt config saved to D1:`,i.updatedAt),e.json({success:!0,config:i})}catch(t){return e.json({success:!1,message:t.message},400)}}),V.post(`/api/admin/auth`,async e=>{let t=await e.req.json(),n=e.env.ADMIN_PASSWORD;return n?t.password===n?e.json({success:!0}):e.json({success:!1,message:`비밀번호가 올바르지 않습니다.`},401):e.json({success:!1,message:`서버 설정 오류: ADMIN_PASSWORD 환경변수가 설정되지 않았습니다.`},500)});var Gn=`의류 이미지 하나로 AI 온모델 피팅컷과 룩북 세트를 자동 생성하세요.`,Kn=(e,t,n=``,r=Gn,i)=>`<!DOCTYPE html>
...
`;return e.text(t,200,{"Content-Type":`text/markdown; charset=utf-8`})});var qn=[{q:`EZlook은 어떤 서비스인가요?`,a:`의류 이미지 한 장을 업로드하면 AI가 온모델 피팅컷과 룩북 세트를 자동으로 생성해주는 AI 패션 이미지 생성 플랫폼입니다. 촬영 스튜디오나 모델 섭외 없이 몇 번의 클릭만으로 전문적인 착용샷을 만들 수 있습니다.`},{q:`이미지 생성에 비용이 드나요?`,a:`이미지 생성 자체는 무료입니다. 마음에 드는 결과물을 실제 파일로 다운로드할 때만 장당 90크레딧이 차감됩니다.`},{q:`무료로 체험할 수 있나요?`,a:`네, 신용카드 등록 없이 회원가입만 하면 무료 크레딧이 바로 지급되어 AI 룩북 제작을 체험해볼 수 있습니다.`},{q:`영상도 만들 수 있나요?`,a:`네, 생성된 피팅컷 이미지를 기반으로 모델이 자연스럽게 포즈를 취하는 7초 분량의 세로형(9:16) 영상을 만들 수 있습니다.`},{q:`결과물은 얼마나 빨리 나오나요?`,a:`평균 30초, 최대 90초 이내에 고품질 온모델 피팅컷 이미지가 생성됩니다.`},{q:`크레딧은 어떻게 충전하나요?`,a:`월 정액 없이 필요한 만큼만 충전하는 방식입니다. 스타터(₩20,000 · 1,000크레딧)부터 베스트 밸류(₩60,000 · 4,000크레딧)까지 선택할 수 있습니다.`}],Jn=()=>{let e={"@type":`Organization`,name:`EZlook`,alternateName:`벌거벗은호랑이`,url:U,logo:`${U}/static/favicon.svg`,telephone:`070-4581-8166`,address:{"@type":`PostalAddress`,streetAddress:`무심서로 377-3`,addressLocality:`청주시 서원구`,addressRegion:`충청북도`,addressCountry:`KR`}};return[{"@context":`https://schema.org`,"@type":`WebSite`,name:`EZlook`,url:U,publisher:e},{"@context":`https://schema.org`,"@type":`Service`,name:`EZlook AI 패션 룩북 생성 서비스`,serviceType:`AI 패션 이미지/영상 생성`,description:`옷 사진 한 장을 업로드하면 AI 모델이 착용한 온모델 피팅컷과 룩북 세트, 홍보 영상을 자동으로 생성하는 서비스`,provider:e,areaServed:`KR`,offers:[{"@type":`Offer`,name:`스타터`,price:`20000`,priceCurrency:`KRW`,description:`1,000 크레딧 · 이미지 최대 11장`},{"@type":`Offer`,name:`인기 충전권`,price:`40000`,priceCurrency:`KRW`,description:`2,300 크레딧 · 이미지 최대 25장`},{"@type":`Offer`,name:`베스트 밸류`,price:`60000`,priceCurrency:`KRW`,description:`4,000 크레딧 · 이미지 최대 44장`}]},{"@context":`https://schema.org`,"@type":`FAQPage`,mainEntity:qn.map(e=>({"@type":`Question`,name:e.q,acceptedAnswer:{"@type":`Answer`,text:e.a}}))}].map(e=>`<script type="application/ld+json">${JSON.stringify(e)}<\/script>`).join(`
  ...
  `,t,`옷 사진 한 장으로 AI 온모델 피팅컷과 룩북 세트를 무료로 자동 생성하세요. 촬영 스튜디오나 모델 섭외 없이 평균 30초 만에 완성됩니다.`,e.env.GA4_MEASUREMENT_ID))}),V.get(`/dashboard`,e=>e.html(Kn(`내 프로필`,`
  <div class="toast-container" id="toastContainer"></div>
  ...
  <\/script>
  `,``,Gn,e.env.GA4_MEASUREMENT_ID))),V.get(`/credits`,e=>e.html(Kn(`크레딧 상세`,`
  <div class="toast-container" id="toastContainer"></div>
  ...
  `,a,n,e.env.GA4_MEASUREMENT_ID))};V.get(`/`,e=>Yn(e,`model`)),V.get(`/generator`,e=>e.redirect(`/`,301)),V.get(`/ghostcut`,e=>Yn(e,`ghostcut`)),V.get(`/techpack`,e=>{let t=`<link rel="canonical" href="${U}/techpack" />`;return e.html(Kn(`AI 도식화 만들기`,`
  <div class="toast-container" id="toastContainer"></div>
...
</html>`));var Xn=new Oe,Zn=Object.assign({"/src/index.tsx":V}),Qn=!1;for(let[,e]of Object.entries(Zn))e&&(Xn.all(`*`,t=>{let n;try{n=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,n)}),Xn.notFound(t=>{let n;try{n=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,n)}),Qn=!0);if(!Qn)throw Error(`Can't import modules from ['/src/index.ts','/src/index.tsx','/app/server.ts']`);export{Xn as default};