...
`,mt=`CASE WHEN email LIKE '%@%' AND email NOT LIKE '%**%' THEN email ELSE crawled_email END`,ht=`COALESCE(NULLIF(bzmnNm, ''), NULLIF(domain_clean, ''), '거래처')`;B.get(`/mail-batch/status`,async e=>{let t=e.env.LOOKBOOK_DB,n=(await t.prepare(`SELECT COUNT(*) AS n FROM biz_leads WHERE ${pt}`).first())?.n||0,r=await t.prepare(`
  ...
  `).bind(t).all();if(!n.length)return e.json({success:!1,message:`해당 배치를 찾을 수 없습니다.`},404);let r=lt(n.map(e=>({name:e.name,email:e.email})));return new Response(r,{status:200,headers:{"Content-Type":`application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`,"Content-Disposition":`attachment; filename="bizleads_mail_batch_${t}.xlsx"`,"X-Batch-Id":String(t),"X-Batch-Count":String(n.length)}})});var gt=`mttg1q3o`;function _t(e){return e.env.TECHPACK_MENU_VISIBLE===`true`}var vt=e=>e?`
...
<\/script>
     ...
     WHERE s.token = ? AND s.expires_at > datetime('now')`).bind(r).first();if(!i)return e.json({error:`세션이 만료되었습니다.`},401);try{let{packageId:r,currency:a}=await e.req.json(),o=On[r];if(!o)return e.json({error:`잘못된 패키지입니다.`},400);let s=String(a||`USD`).toUpperCase();if(s!==`USD`&&s!==`JPY`)return e.json({error:`지원하지 않는 통화입니다.`},400);let l=s===`USD`?o.usdCents:o.jpyAmount,u=Dn[i.referrer]||0,d=u>0?Math.round(l*(1-u)):l,f=`lookbook-${i.user_id.slice(0,6)}-${Date.now()}-${Math.random().toString(36).slice(2,6).toUpperCase()}`;await n.prepare(`INSERT INTO payment_logs (user_id, order_id, amount, credits, status, pg_provider, currency)
       ...
       VALUES (?, 'grant', ?, ?, 'payment', ?)`).bind(a.user_id,a.credits,f,r).run(),e.redirect(`/payment/success?orderId=${encodeURIComponent(r)}`,302)}catch(t){return console.error(`payment/toss/success error:`,t),e.redirect(`/payment/fail?message=%EA%B2%B0%EC%A0%9C%20%EC%B2%98%EB%A6%AC%20%EC%A4%91%20%EC%98%A4%EB%A5%98%EA%B0%80%20%EB%B0%9C%EC%83%9D%ED%96%88%EC%8A%B5%EB%8B%88%EB%8B%A4.`,302)}}),V.get(`/payment/toss/fail`,e=>{let t=e.req.query(`message`)||`결제가 취소되었습니다.`,n=e.req.query(`code`)||``;return e.redirect(`/payment/fail?message=${encodeURIComponent(t)}&code=${encodeURIComponent(n)}`,302)}),V.post(`/payment/toss/webhook`,async e=>{let t=e.env.LOOKBOOK_DB,n=()=>e.text(`OK`,200);try{let r=await e.req.text(),i={};try{i=JSON.parse(r)}catch{return n()}if(!e.env.TOSS_WEBHOOK_SECRET)return console.error(`토스 웹훅 수신했으나 TOSS_WEBHOOK_SECRET 미설정 — 서명 검증 불가, 크레딧 처리 건너뜀:`,JSON.stringify(i).slice(0,500)),n();let a=String(i.eventType||``),o=i.data||{},s=String(o.orderId||``),l=String(o.paymentKey||``),u=String(o.status||``);if(!(a===`PAYMENT_STATUS_CHANGED`&&(u===`CANCELED`||u===`PARTIAL_CANCELED`)))return n();let d=await t.prepare(`SELECT id, user_id, credits, status FROM payment_logs WHERE order_id = ? OR payment_key = ?`).bind(s,l).first();if(!d)return console.error(`토스 웹훅: 결제 내역을 찾을 수 없음:`,s,l),n();if(d.status===`canceled`)return n();let f=(await t.prepare(`SELECT credits FROM users WHERE id=?`).bind(d.user_id).first())?.credits??0,p=Number(d.credits)||0,m=Math.min(p,f),h=Math.max(0,f-m);return m>0&&(await t.prepare(`UPDATE users SET credits=?, updated_at=datetime('now') WHERE id=?`).bind(h,d.user_id).run(),await t.prepare(`INSERT INTO credit_logs (user_id, type, amount, balance, reason, ref_id)
         VALUES (?, 'revoke', ?, ?, 'payment_cancel', ?)`).bind(d.user_id,-m,h,s).run()),await t.prepare(`UPDATE payment_logs SET status='canceled' WHERE id=?`).bind(d.id).run(),console.log(`토스 웹훅: 결제취소 처리 완료 — orderId=${s}, 회수 크레딧=${m}`),n()}catch(e){return console.error(`payment/toss/webhook error:`,e),n()}}),V.get(`/api/admin/debug/recent-payments`,W,async e=>{let t=e.env.LOOKBOOK_DB;try{let n=await t.prepare(`SELECT p.order_id, p.user_id, u.email, u.name, p.amount, p.credits, p.status,
