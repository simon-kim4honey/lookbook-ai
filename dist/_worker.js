...
</html>`));var sr={fetch:H.fetch,scheduled:async(e,t,n)=>{n.waitUntil(wt(t).catch(e=>{console.error(`content-digest scheduled 실행 실패:`,e)}))}},cr=new Oe,lr=Object.assign({"/src/index.tsx":sr}),ur=!1;for(let[,e]of Object.entries(lr))e&&(cr.all(`*`,t=>{let n;try{n=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,n)}),cr.notFound(t=>{let n;try{n=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,n)}),ur=!0);if(!ur)throw Error(`Can't import modules from ['/src/index.ts','/src/index.tsx','/app/server.ts']`);export{cr as default};