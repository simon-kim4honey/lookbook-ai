...
`,mt=`CASE WHEN email LIKE '%@%' AND email NOT LIKE '%**%' THEN email ELSE crawled_email END`,ht=`COALESCE(NULLIF(bzmnNm, ''), NULLIF(domain_clean, ''), '거래처')`;B.get(`/mail-batch/status`,async e=>{let t=e.env.LOOKBOOK_DB,n=(await t.prepare(`SELECT COUNT(*) AS n FROM biz_leads WHERE ${pt}`).first())?.n||0,r=await t.prepare(`
  ...
  `).bind(t).all();if(!n.length)return e.json({success:!1,message:`해당 배치를 찾을 수 없습니다.`},404);let r=lt(n.map(e=>({name:e.name,email:e.email})));return new Response(r,{status:200,headers:{"Content-Type":`application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`,"Content-Disposition":`attachment; filename="bizleads_mail_batch_${t}.xlsx"`,"X-Batch-Id":String(t),"X-Batch-Count":String(n.length)}})});var gt=`mu3gyk1w`;function _t(e){return e.env.TECHPACK_MENU_VISIBLE===`true`}var vt=e=>e?`
...
Respond with ONLY one word: YES or NO.`};V.post(`/api/validate/clothing`,async e=>{try{let t=await e.req.json(),n=t?.imageBase64||``,r=t?.cat||``;if(!n)return e.json({success:!1,message:`imageBase64 필수`},400);let i=e.env?.OPENAI_API_KEY||``;if(!i)return e.json({success:!0,isClothing:!0});let a=await Zt(i,n,Jt[r]||Jt.DRESS);return a===null?e.json({success:!0,isClothing:!0}):e.json({success:!0,isClothing:a.trim().toUpperCase().startsWith(`YES`)})}catch(t){return console.error(`validate/clothing error:`,t),e.json({success:!0,isClothing:!0})}});var Yt=[`You are a clothing product classifier for a "ghost mannequin" e-commerce photography app.`,`Look at the uploaded image. First decide: is this clearly a photo of a SINGLE clothing/fashion product (worn, laid flat, on a hanger, or on a mannequin)? It does not need to be a professional product shot.`,`If it is NOT a clothing product at all (e.g. a person's portrait unrelated to showing a garment, an animal, food, scenery, a random object, or an image with no identifiable single garment), respond with exactly: NOT_CLOTHING`,`If it IS a clothing product, respond with EXACTLY ONE of these category codes (the single best match, nothing else):`,X.map(e=>`${e.code} (${e.group} - ${e.label})`).join(`, `),`Respond with ONLY the code (e.g. "TOP") or "NOT_CLOTHING" — no explanation, no punctuation, no extra words.`].join(`
`);V.post(`/api/ghostcut/classify`,async e=>{try{let t=(await e.req.json())?.imageBase64||``;if(!t)return e.json({success:!1,message:`imageBase64 필수`},400);let n=e.env?.OPENAI_API_KEY||``;if(!n)return e.json({success:!1,message:`서버 설정 오류: OPENAI_API_KEY 미설정`},500);let r=await Zt(n,t,Yt);if(r===null)return e.json({success:!1,message:`이미지 분석에 실패했습니다. 잠시 후 다시 시도해주세요.`});let i=r.trim().toUpperCase().replace(/[^A-Z_]/g,``);if(i===`NOT_CLOTHING`)return e.json({success:!0,isClothing:!1});let a=X.find(e=>e.code===i);if(!a)return console.warn(`ghostcut/classify: 알 수 없는 분류 응답:`,r),e.json({success:!1,message:`이미지 분류에 실패했습니다. 다른 사진으로 다시 시도해주세요.`});let o=e.env?.LOOKBOOK_KV,s=e.env?.LOOKBOOK_DB,l=!1;return l=o?!!await Bt(o,i):s?!!await Wt(s,i):!!Z[i],e.json({success:!0,isClothing:!0,category:i,group:a.group,label:a.label,displayLabel:a.label,sampleReady:l})}catch(t){return console.error(`ghostcut/classify error:`,t),e.json({success:!1,message:`이미지 분석 중 오류가 발생했습니다.`})}}),V.post(`/api/admin/auto-label`,W,async e=>{try{let{type:t,imageBase64:n}=await e.req.json();if(!n)return e.json({success:!1,message:`imageBase64 필수`},400);let r=e.env?.OPENAI_API_KEY||``;if(!r)return e.json({success:!1,message:`OPENAI_API_KEY 미설정`},500);let i=e=>{let t=e.match(/\{[\s\S]*\}/);return JSON.parse(t?.[0]||e)};if(t===`model`){let t=await Zt(r,n,`You are an AI fashion model classifier. Analyze this model photo and return ONLY a JSON object with these exact fields:
{
  "gender": "여성" or "남성",
...
Return ONLY the JSON, no explanation.`);if(t===null)return e.json({success:!1,message:`라벨링 요청 실패`},500);try{let n=i(t);return e.json({success:!0,labels:{category:n.category||`스튜디오`,mood:n.mood||`미니멀`,name_ko:n.name_ko||``}})}catch{return e.json({success:!1,message:`응답 파싱 실패`,raw:t})}}return e.json({success:!1,message:`type은 model 또는 background 이어야 합니다.`},400)}catch(t){return e.json({success:!1,message:t.message},500)}}),V.patch(`/api/admin/models/:id/labels`,W,async e=>{try{let t=e.req.param(`id`),{gender:n,age:r,mood:i}=await e.req.json(),a=e.env?.LOOKBOOK_KV,o=e.env?.LOOKBOOK_DB;if(a){let o=await K(a),s=o.findIndex(e=>e.id===t);return s===-1?e.json({success:!1,message:`모델을 찾을 수 없습니다.`},404):(o[s]={...o[s],gender:n||`미분류`,age:r||`미분류`,mood:i||`미분류`},await Ct(a,o),e.json({success:!0}))}return o?(await o.prepare(`UPDATE custom_models SET gender=?, age=?, mood=? WHERE id=?`).bind(n||`미분류`,r||`미분류`,i||`미분류`,t).run(),e.json({success:!0})):e.json({success:!1,message:`D1/KV 없음`},500)}catch(t){return e.json({success:!1,message:t.message},500)}}),V.get(`/api/admin/models`,W,async e=>{let t=e.env?.LOOKBOOK_KV,n=e.env?.LOOKBOOK_DB;if(t){let n=await K(t);return e.json({success:!0,models:n})}if(n){let t=await Ot(n);return e.json({success:!0,models:t})}let r=J.map(e=>({id:e.id,name:e.name,desc:e.desc,createdAt:e.createdAt}));return e.json({success:!0,models:r})}),V.delete(`/api/admin/models/:id`,W,async e=>{let t=e.req.param(`id`),n=e.env?.LOOKBOOK_KV,r=e.env?.LOOKBOOK_DB;if(n){let r=await K(n),i=r.filter(e=>e.id!==t);return await Ct(n,i),await n.delete(`model_img:${t}`),e.json({success:r.length>i.length})}if(r){let n=await At(r,t);return e.json({success:n})}let i=J.length;return J=J.filter(e=>e.id!==t),e.json({success:J.length<i})}),V.get(`/api/proxy/custom-model/:id`,async e=>{let t=e.req.param(`id`),n=e.env?.LOOKBOOK_KV,r=e.env?.LOOKBOOK_DB,i=null;if(i=n?await n.get(`model_img:${t}`):r?await jt(r,t):J.find(e=>e.id===t)?.imageBase64||null,!i)return e.notFound();let[a,o]=i.split(`,`),s=(a.match(/data:([^;]+)/)||[])[1]||`image/jpeg`,l=atob(o),u=new Uint8Array(l.length);for(let e=0;e<l.length;e++)u[e]=l.charCodeAt(e);return new Response(u.buffer,{headers:{"Content-Type":s,"Cache-Control":`public, max-age=3600`}})}),V.post(`/api/admin/backgrounds`,W,async e=>{try{let t=await e.req.json(),n=Array.isArray(t)?t:[t];if(n.length===0)return e.json({success:!1,message:`업로드할 항목이 없습니다.`},400);let r=e.env?.LOOKBOOK_KV,i=e.env?.LOOKBOOK_DB,a=[];if(r){let e=await q(r);for(let t of n){let{name:n,bgDesc:i,category:o,imageBase64:s}=t;if(!n||!s)continue;let l=await Et(r),u={id:l,name:n,bgDesc:i||n,category:o||`커스텀`,createdAt:new Date().toISOString()};e.push(u),await r.put(`bg_img:${l}`,s),a.push(u)}await wt(r,e)}else if(i)a=await Pt(i,n);else for(let e of n){let{name:t,bgDesc:n,category:r,imageBase64:i}=e;if(!t||!i)continue;let o=String(Rt++),s={id:o,name:t,bgDesc:n||t,category:r||`커스텀`,imageBase64:i,createdAt:new Date().toISOString()};Y.push(s),a.push({id:o,name:t,bgDesc:s.bgDesc,category:s.category,createdAt:s.createdAt})}return e.json({success:!0,backgrounds:a,count:a.length})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.get(`/api/admin/backgrounds`,W,async e=>{let t=e.env?.LOOKBOOK_KV,n=e.env?.LOOKBOOK_DB;if(t){let n=await q(t);return e.json({success:!0,backgrounds:n})}if(n){let t=await Mt(n);return e.json({success:!0,backgrounds:t})}let r=Y.map(e=>({id:e.id,name:e.name,bgDesc:e.bgDesc,category:e.category,createdAt:e.createdAt,hasGenImage:!!e.genImageBase64,isDefault:!!e.isDefault}));return e.json({success:!0,backgrounds:r})}),V.delete(`/api/admin/backgrounds/:id`,W,async e=>{let t=e.req.param(`id`),n=e.env?.LOOKBOOK_KV,r=e.env?.LOOKBOOK_DB;if(n){let r=await q(n),i=r.filter(e=>e.id!==t);return await wt(n,i),await n.delete(`bg_img:${t}`),await n.delete(`bg_gen_img:${t}`),e.json({success:r.length>i.length})}if(r){let n=await Ft(r,t);return e.json({success:n})}let i=Y.length;return Y=Y.filter(e=>e.id!==t),e.json({success:Y.length<i})}),V.put(`/api/admin/backgrounds/:id/gen-image`,W,async e=>{let t=e.req.param(`id`);try{let n=(await e.req.json())?.imageBase64||``;if(!n)return e.json({success:!1,message:`imageBase64 필수`},400);let r=e.env?.LOOKBOOK_KV,i=e.env?.LOOKBOOK_DB;if(r){let i=await q(r),a=i.find(e=>e.id===t);return a?(await r.put(`bg_gen_img:${t}`,n),a.hasGenImage=!0,await wt(r,i),e.json({success:!0})):e.json({success:!1,message:`배경을 찾을 수 없습니다.`},404)}if(i)return await Lt(i,t,n)?e.json({success:!0}):e.json({success:!1,message:`배경을 찾을 수 없습니다.`},404);let a=Y.find(e=>e.id===t);return a?(a.genImageBase64=n,e.json({success:!0})):e.json({success:!1,message:`배경을 찾을 수 없습니다.`},404)}catch(t){return e.json({success:!1,message:t.message},500)}}),V.put(`/api/admin/backgrounds/:id/default`,W,async e=>{let t=e.req.param(`id`);try{let n=e.env?.LOOKBOOK_KV,r=e.env?.LOOKBOOK_DB;if(n){let r=await Tt(n,t);return r.ok?e.json({success:!0,isDefault:r.isDefault}):e.json({success:!1,message:`배경을 찾을 수 없습니다.`},404)}if(r){let n=await Nt(r,t);return n.ok?e.json({success:!0,isDefault:n.isDefault}):e.json({success:!1,message:`배경을 찾을 수 없습니다.`},404)}let i=Y.find(e=>e.id===t);if(!i)return e.json({success:!1,message:`배경을 찾을 수 없습니다.`},404);let a=!i.isDefault;return Y.forEach(e=>{e.isDefault=!1}),i.isDefault=a,e.json({success:!0,isDefault:a})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.get(`/api/proxy/custom-bg/:id`,async e=>{let t=e.req.param(`id`),n=e.env?.LOOKBOOK_KV,r=e.env?.LOOKBOOK_DB,i=null;if(i=n?await n.get(`bg_img:${t}`):r?await It(r,t):Y.find(e=>e.id===t)?.imageBase64||null,!i)return e.notFound();let[a,o]=i.split(`,`),s=(a.match(/data:([^;]+)/)||[])[1]||`image/jpeg`,l=atob(o),u=new Uint8Array(l.length);for(let e=0;e<l.length;e++)u[e]=l.charCodeAt(e);return new Response(u.buffer,{headers:{"Content-Type":s,"Cache-Control":`public, max-age=3600`}})}),V.get(`/api/admin/ghostcut-samples`,W,async e=>{let t=e.env?.LOOKBOOK_KV,n=e.env?.LOOKBOOK_DB,r={};t?r=await Ut(t):n?r=await qt(n):X.forEach(e=>{r[e.code]=!!Z[e.code]});let i=X.map(e=>({...e,hasSample:!!r[e.code]}));return e.json({success:!0,categories:i})}),V.get(`/api/admin/ghostcut-samples/:category/image`,W,async e=>{let t=e.req.param(`category`);if(!zt.has(t))return e.json({success:!1,message:`알 수 없는 카테고리`},400);let n=e.env?.LOOKBOOK_KV,r=e.env?.LOOKBOOK_DB,i=null;return i=n?await Bt(n,t):r?await Wt(r,t):Z[t]||null,e.json({success:!0,imageBase64:i})}),V.post(`/api/admin/ghostcut-samples/:category`,W,async e=>{let t=e.req.param(`category`),n=X.find(e=>e.code===t);if(!n)return e.json({success:!1,message:`알 수 없는 카테고리`},400);try{let r=(await e.req.json())?.imageBase64||``;if(!r)return e.json({success:!1,message:`imageBase64 필수`},400);let i=e.env?.LOOKBOOK_KV,a=e.env?.LOOKBOOK_DB;return i?await Vt(i,t,r):a?await Gt(a,t,n.group,n.label,r):Z[t]=r,e.json({success:!0})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.delete(`/api/admin/ghostcut-samples/:category`,W,async e=>{let t=e.req.param(`category`);if(!zt.has(t))return e.json({success:!1,message:`알 수 없는 카테고리`},400);let n=e.env?.LOOKBOOK_KV,r=e.env?.LOOKBOOK_DB;if(n)return await Ht(n,t),e.json({success:!0});if(r){let n=await Kt(r,t);return e.json({success:n})}let i=!!Z[t];return delete Z[t],e.json({success:i})}),V.get(`/api/proxy/clothing/:jobId/:idx`,async e=>{let t=e.req.param(`jobId`),n=parseInt(e.req.param(`idx`)||`0`,10),r=e.env?.LOOKBOOK_KV;if(!r)return e.notFound();let i=await r.get(`clothing_img:${t}`);if(!i)return e.notFound();let a=[];try{a=JSON.parse(i)}catch{}let o=a[n];if(!o)return e.notFound();let[s,l]=o.split(`,`),u=(s.match(/data:([^;]+)/)||[])[1]||`image/jpeg`,d=atob(l),f=new Uint8Array(d.length);for(let e=0;e<d.length;e++)f[e]=d.charCodeAt(e);return new Response(f.buffer,{headers:{"Content-Type":u,"Cache-Control":`public, max-age=3600`}})});var Q=e=>({Authorization:`Bearer ${e}`,"Content-Type":`application/json`}),Xt=`https://api.openai.com`;async function Zt(e,t,n){let r=`${Xt}/v1/chat/completions`;try{let i=await fetch(r,{method:`POST`,headers:{Authorization:`Bearer ${e}`,"Content-Type":`application/json`},body:JSON.stringify({model:`gpt-4o-mini`,messages:[{role:`user`,content:[{type:`image_url`,image_url:{url:t.startsWith(`data:`)?t:`data:image/jpeg;base64,${t}`}},{type:`text`,text:n}]}]})}),a=await i.text();if(console.log(`openaiChatVision: POST ${r} → HTTP ${i.status} | body(첫 500자): ${a.slice(0,500)}`),!i.ok)return console.error(`openaiChatVision: 요청 실패 (HTTP ${i.status})`),null;let o;try{o=JSON.parse(a)}catch{return console.error(`openaiChatVision: JSON 파싱 실패, 원문:`,a.slice(0,300)),null}let s=o?.choices?.[0]?.message?.content;return typeof s==`string`?s:(console.warn(`openaiChatVision: 예상치 못한 응답 형식:`,JSON.stringify(o).slice(0,300)),null)}catch(e){return console.error(`openaiChatVision error:`,e?.message||e),null}}V.get(`/api/presets/models`,async e=>{let t=e.env?.LOOKBOOK_KV,n=e.env?.LOOKBOOK_DB,r;r=t?await K(t):n?await Ot(n):J.map(e=>({id:e.id,name:e.name,desc:e.desc,createdAt:e.createdAt}));let i=r.map(e=>({id:Number(e.id),name:e.name,gender:e.gender||`미분류`,age:e.age||`미분류`,mood:e.mood||`미분류`,body:`-`,skin:`-`,desc:e.desc,unsplashId:null,isCustom:!0,customId:e.id}));return e.json({models:i})}),V.get(`/api/presets/backgrounds`,async e=>{let t=e.env?.LOOKBOOK_KV,n=e.env?.LOOKBOOK_DB,r;r=t?await q(t):n?await Mt(n):Y.map(e=>({id:e.id,name:e.name,bgDesc:e.bgDesc,category:e.category,createdAt:e.createdAt,isDefault:!!e.isDefault}));let i=r.map(e=>({id:Number(e.id),name:e.name,category:e.category,mood:`-`,bgDesc:e.bgDesc,unsplashId:null,isCustom:!0,customId:e.id,isDefault:!!e.isDefault}));return e.json({backgrounds:i})});var Qt=`home_showcase_images`,$t=[1,2,3,4,5,6];async function en(e){let t=await e.get(Qt);if(!t)return[];try{return JSON.parse(t)}catch{return[]}}async function tn(e,t){await e.put(Qt,JSON.stringify(t))}V.get(`/api/home/showcase`,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({images:[]});let n=await en(t);return e.json({images:n.map(e=>({id:e.id,imageBase64:e.imageBase64}))})}),V.get(`/api/admin/home-showcase`,W,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({success:!1,message:`KV 미설정`},500);let n=await en(t);return e.json({success:!0,images:n})}),V.post(`/api/admin/home-showcase`,W,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({success:!1,message:`KV 미설정`},500);try{let n=await e.req.json(),r=Array.isArray(n?.images)?n.images:[];if(!r.length)return e.json({success:!1,message:`이미지가 필요합니다.`},400);let i=await en(t),a=r.map(e=>({id:crypto.randomUUID(),imageBase64:e,createdAt:new Date().toISOString()})),o=[...i,...a];return await tn(t,o),e.json({success:!0,count:a.length,images:o})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.delete(`/api/admin/home-showcase/:id`,W,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({success:!1,message:`KV 미설정`},500);let n=e.req.param(`id`);return await tn(t,(await en(t)).filter(e=>e.id!==n)),e.json({success:!0})});async function nn(e,t){return await e.get(`home_feature_bg_${t}`)}V.get(`/api/home/feature-bgs`,async e=>{let t=e.env?.LOOKBOOK_KV,n={};if(t)for(let e of $t)n[e]=await nn(t,e);else $t.forEach(e=>{n[e]=null});return e.json({backgrounds:n})}),V.put(`/api/admin/home-feature-bg/:slot`,W,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({success:!1,message:`KV 미설정`},500);let n=Number(e.req.param(`slot`));if(!$t.includes(n))return e.json({success:!1,message:`잘못된 슬롯`},400);try{let r=(await e.req.json())?.imageBase64||``;return r?(await t.put(`home_feature_bg_${n}`,r),e.json({success:!0})):e.json({success:!1,message:`imageBase64 필수`},400)}catch(t){return e.json({success:!1,message:t.message},500)}}),V.delete(`/api/admin/home-feature-bg/:slot`,W,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({success:!1,message:`KV 미설정`},500);let n=Number(e.req.param(`slot`));return $t.includes(n)?(await t.delete(`home_feature_bg_${n}`),e.json({success:!0})):e.json({success:!1,message:`잘못된 슬롯`},400)});var rn=[1],an=22*1024*1024;V.get(`/api/home/howto-videos`,async e=>{let t=e.env?.LOOKBOOK_KV,n={};if(t)for(let e of rn)n[e]=(await t.getWithMetadata(`home_howto_video_${e}`)).value==null?null:`/api/home/howto-video/${e}`;else rn.forEach(e=>{n[e]=null});return e.json({videos:n})}),V.get(`/api/home/howto-video/:slot`,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.notFound();let n=Number(e.req.param(`slot`));if(!rn.includes(n))return e.notFound();let{value:r,metadata:i}=await t.getWithMetadata(`home_howto_video_${n}`,`arrayBuffer`);if(!r)return e.notFound();let a=r,o=i?.contentType||`video/mp4`,s=a.byteLength,l=e.req.header(`range`);if(l){let e=l.match(/bytes=(\d*)-(\d*)/);if(e){let t=e[1]?parseInt(e[1],10):0,n=e[2]?parseInt(e[2],10):s-1,r=Math.max(0,Math.min(t,s-1)),i=Math.max(r,Math.min(n,s-1)),l=a.slice(r,i+1);return new Response(l,{status:206,headers:{"Content-Type":o,"Content-Range":`bytes ${r}-${i}/${s}`,"Accept-Ranges":`bytes`,"Content-Length":String(l.byteLength),"Cache-Control":`public, max-age=3600`}})}}return new Response(a,{headers:{"Content-Type":o,"Accept-Ranges":`bytes`,"Cache-Control":`public, max-age=3600`,"Content-Length":String(s)}})}),V.put(`/api/admin/home-howto-video/:slot`,W,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({success:!1,message:`KV 미설정`},500);let n=Number(e.req.param(`slot`));if(!rn.includes(n))return e.json({success:!1,message:`잘못된 슬롯`},400);try{let r=await e.req.arrayBuffer();if(!r||r.byteLength===0)return e.json({success:!1,message:`영상 파일이 필요합니다.`},400);if(r.byteLength>an)return e.json({success:!1,message:`영상 용량은 ${Math.floor(an/1024/1024)}MB 이하만 가능합니다.`},400);let i=e.req.header(`content-type`)||`video/mp4`;return await t.put(`home_howto_video_${n}`,r,{metadata:{contentType:i}}),e.json({success:!0})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.delete(`/api/admin/home-howto-video/:slot`,W,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({success:!1,message:`KV 미설정`},500);let n=Number(e.req.param(`slot`));return rn.includes(n)?(await t.delete(`home_howto_video_${n}`),e.json({success:!0})):e.json({success:!1,message:`잘못된 슬롯`},400)});var on=[1,2,3,4,5],sn=22*1024*1024;V.get(`/api/gen-loading-videos`,async e=>{let t=e.env?.LOOKBOOK_KV,n={};if(t)for(let e of on)n[e]=(await t.getWithMetadata(`gen_loading_video_${e}`)).value==null?null:`/api/gen-loading-video/${e}`;else on.forEach(e=>{n[e]=null});return e.json({videos:n})}),V.get(`/api/gen-loading-video/:slot`,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.notFound();let n=Number(e.req.param(`slot`));if(!on.includes(n))return e.notFound();let{value:r,metadata:i}=await t.getWithMetadata(`gen_loading_video_${n}`,`arrayBuffer`);if(!r)return e.notFound();let a=r,o=i?.contentType||`video/mp4`,s=a.byteLength,l=e.req.header(`range`);if(l){let e=l.match(/bytes=(\d*)-(\d*)/);if(e){let t=e[1]?parseInt(e[1],10):0,n=e[2]?parseInt(e[2],10):s-1,r=Math.max(0,Math.min(t,s-1)),i=Math.max(r,Math.min(n,s-1)),l=a.slice(r,i+1);return new Response(l,{status:206,headers:{"Content-Type":o,"Content-Range":`bytes ${r}-${i}/${s}`,"Accept-Ranges":`bytes`,"Content-Length":String(l.byteLength),"Cache-Control":`public, max-age=3600`}})}}return new Response(a,{headers:{"Content-Type":o,"Accept-Ranges":`bytes`,"Cache-Control":`public, max-age=3600`,"Content-Length":String(s)}})}),V.put(`/api/admin/gen-loading-video/:slot`,W,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({success:!1,message:`KV 미설정`},500);let n=Number(e.req.param(`slot`));if(!on.includes(n))return e.json({success:!1,message:`잘못된 슬롯`},400);try{let r=await e.req.arrayBuffer();if(!r||r.byteLength===0)return e.json({success:!1,message:`영상 파일이 필요합니다.`},400);if(r.byteLength>sn)return e.json({success:!1,message:`영상 용량은 ${Math.floor(sn/1024/1024)}MB 이하만 가능합니다.`},400);let i=e.req.header(`content-type`)||`video/mp4`;return await t.put(`gen_loading_video_${n}`,r,{metadata:{contentType:i}}),e.json({success:!0})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.delete(`/api/admin/gen-loading-video/:slot`,W,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({success:!1,message:`KV 미설정`},500);let n=Number(e.req.param(`slot`));return on.includes(n)?(await t.delete(`gen_loading_video_${n}`),e.json({success:!0})):e.json({success:!1,message:`잘못된 슬롯`},400)});var cn=[1,2,3,4,5],ln=8*1024*1024;V.get(`/api/gc-loading-images`,async e=>{let t=e.env?.LOOKBOOK_KV,n={};if(t)for(let e of cn)n[e]=(await t.getWithMetadata(`gc_loading_image_${e}`)).value==null?null:`/api/gc-loading-image/${e}`;else cn.forEach(e=>{n[e]=null});return e.json({images:n})}),V.get(`/api/gc-loading-image/:slot`,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.notFound();let n=Number(e.req.param(`slot`));if(!cn.includes(n))return e.notFound();let{value:r,metadata:i}=await t.getWithMetadata(`gc_loading_image_${n}`,`arrayBuffer`);if(!r)return e.notFound();let a=r,o=i?.contentType||`image/jpeg`;return new Response(a,{headers:{"Content-Type":o,"Cache-Control":`public, max-age=3600`,"Content-Length":String(a.byteLength)}})}),V.put(`/api/admin/gc-loading-image/:slot`,W,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({success:!1,message:`KV 미설정`},500);let n=Number(e.req.param(`slot`));if(!cn.includes(n))return e.json({success:!1,message:`잘못된 슬롯`},400);try{let r=await e.req.arrayBuffer();if(!r||r.byteLength===0)return e.json({success:!1,message:`이미지 파일이 필요합니다.`},400);if(r.byteLength>ln)return e.json({success:!1,message:`이미지 용량은 ${Math.floor(ln/1024/1024)}MB 이하만 가능합니다.`},400);let i=e.req.header(`content-type`)||`image/jpeg`;return await t.put(`gc_loading_image_${n}`,r,{metadata:{contentType:i}}),e.json({success:!0})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.delete(`/api/admin/gc-loading-image/:slot`,W,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({success:!1,message:`KV 미설정`},500);let n=Number(e.req.param(`slot`));return cn.includes(n)?(await t.delete(`gc_loading_image_${n}`),e.json({success:!0})):e.json({success:!1,message:`잘못된 슬롯`},400)});var un=`fashion_news_cache_v1`,dn=1200*1e3;function fn(e){let t=e.match(/<!\[CDATA\[([\s\S]*?)\]\]>/);return(t?t[1]:e).replace(/&amp;/g,`&`).replace(/&lt;/g,`<`).replace(/&gt;/g,`>`).replace(/&#39;/g,`'`).replace(/&quot;/g,`"`).trim()}function pn(e){let t=[],n=e.match(/<item>[\s\S]*?<\/item>/g)||[];for(let e of n){let n=e.match(/<title>([\s\S]*?)<\/title>/),r=e.match(/<link>([\s\S]*?)<\/link>/),i=e.match(/<source[^>]*>([\s\S]*?)<\/source>/),a=e.match(/<pubDate>([\s\S]*?)<\/pubDate>/);if(!n||!r)continue;let o=fn(n[1]),s=i?fn(i[1]):``;if(s){let e=` - `+s;o.endsWith(e)&&(o=o.slice(0,-e.length).trim())}t.push({title:o,link:fn(r[1]),source:s,pubDate:a?fn(a[1]):``})}return t.slice(0,12)}V.get(`/api/fashion-news`,async e=>{let t=e.env?.LOOKBOOK_KV;async function n(){if(!t)return null;let e=await t.get(un);if(!e)return null;try{return JSON.parse(e).news||null}catch{return null}}try{if(t){let n=await t.get(un);if(n){let t=JSON.parse(n);if(Date.now()-t.fetchedAt<dn)return e.json({news:t.news})}}let r=await fetch(`https://news.google.com/rss/search?q=%ED%8C%A8%EC%85%98&hl=ko&gl=KR&ceid=KR:ko`,{headers:{"User-Agent":`Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36`,Accept:`application/rss+xml, application/xml, text/xml, */*`,"Accept-Language":`ko-KR,ko;q=0.9`}});if(!r.ok)throw Error(`RSS fetch failed: ${r.status}`);let i=await r.text(),a=pn(i);if(a.length===0){let t=await n();return t&&t.length>0?e.json({news:t}):(console.warn(`fashion-news: RSS 응답에서 기사 파싱 실패 (item 0개)`),e.json({news:[],_debug:{httpStatus:r.status,xmlLength:i.length,hasItemTag:i.includes(`<item>`),hasRssTag:i.includes(`<rss`),preview:i.slice(0,500)}}))}return t&&await t.put(un,JSON.stringify({fetchedAt:Date.now(),news:a})),e.json({news:a})}catch(t){console.warn(`fashion-news fetch error:`,t.message);let r=await n();return r&&r.length>0?e.json({news:r}):e.json({news:[],_debug:{error:t.message}})}}),V.get(`/api/proxy/model-image/:id`,e=>e.notFound()),V.get(`/api/proxy/bg-image/:id`,e=>e.notFound()),V.get(`/api/proxy/gen-image`,async e=>{let t=e.req.query(`url`),n=e.req.query(`download`)===`1`;if(!t)return e.json({error:`Missing url param`},400);try{if(!new URL(t).protocol.startsWith(`https`))return e.json({error:`Only HTTPS URLs allowed`},400);let r=e.req.header(`Range`),i={"User-Agent":`Mozilla/5.0 (compatible; LookbookAI/1.0)`};r&&(i.Range=r);let a=await fetch(t,{headers:i});if(!a.ok&&a.status!==206)return e.json({error:`Upstream error: ${a.status}`},a.status);let o=a.headers.get(`Content-Type`)||`image/jpeg`,s=o.includes(`video`)?`mp4`:`jpg`,l=`lookbook_ai_${Date.now()}.${s}`,u={"Content-Type":o,"Cache-Control":`public, max-age=3600`,"Access-Control-Allow-Origin":`*`,"Accept-Ranges":`bytes`},d=a.headers.get(`Content-Range`);d&&(u[`Content-Range`]=d);let f=a.headers.get(`Content-Length`);return f&&(u[`Content-Length`]=f),n&&(u[`Content-Disposition`]=`attachment; filename="${l}"`),new Response(a.body,{status:a.status,headers:u})}catch(t){return console.error(`Gen image proxy error:`,t),e.json({error:t.message},500)}});var mn=[{id:`p1`,name:`2024 S/S 룩북`,status:`done`,images:8,created:`2024-03-15`,thumb_color:`#FF6B9D`},{id:`p2`,name:`캐주얼 티셔츠 컷`,status:`done`,images:4,created:`2024-03-12`,thumb_color:`#3182F6`},{id:`p3`,name:`데님 라인 촬영`,status:`processing`,images:0,created:`2024-03-10`,thumb_color:`#3B82F6`},{id:`p4`,name:`원피스 봄 컬렉션`,status:`draft`,images:0,created:`2024-03-08`,thumb_color:`#00D4AA`}];V.get(`/api/projects`,e=>e.json({projects:mn}));function hn(e){let t=(e||``).toUpperCase();return t===`KR`||!t?{locale:`ko`,currency:`KRW`,pg:`toss`,messenger:`kakao`}:t===`JP`?{locale:`ja`,currency:`JPY`,pg:`stripe`,messenger:`line`}:{locale:`en`,currency:`USD`,pg:`stripe`,messenger:`web-share`}}V.get(`/api/locale`,async e=>{let t=(e.req.header(`CF-IPCountry`)??e.req.header(`cf-ipcountry`)??e.req.raw.cf?.country??``).toUpperCase(),n=e.req.header(`X-Session-Token`)||``;if(n){let r=await e.env.LOOKBOOK_DB.prepare(`SELECT u.locale, u.country, u.currency FROM user_sessions s
       JOIN users u ON u.id = s.user_id
       WHERE s.token = ? AND s.expires_at > datetime('now')`).bind(n).first();if(r?.locale)return e.json({country:r.country||t,locale:r.locale,currency:r.currency||`USD`,pg:r.locale===`ko`?`toss`:`stripe`,messenger:r.locale===`ko`?`kakao`:r.locale===`ja`?`line`:`web-share`})}let r=hn(t);return e.json({country:t,...r})}),V.put(`/api/user/locale`,async e=>{let t=e.env.LOOKBOOK_DB,n=e.req.header(`X-Session-Token`)||``;if(!n)return e.json({success:!1,message:`로그인이 필요합니다.`},401);let r=await t.prepare(`SELECT user_id FROM user_sessions WHERE token = ? AND expires_at > datetime('now')`).bind(n).first();if(!r)return e.json({success:!1,message:`세션이 만료되었습니다.`},401);let i=await e.req.json(),a=[`ko`,`en`,`ja`].includes(i?.locale)?i.locale:null;if(!a)return e.json({success:!1,message:`지원하지 않는 언어입니다.`},400);let o=typeof i?.country==`string`?i.country.slice(0,8):null,s=typeof i?.currency==`string`?i.currency.slice(0,8):null;return await t.prepare(`UPDATE users SET locale = ?, country = ?, currency = ?, updated_at = datetime('now') WHERE id = ?`).bind(a,o,s,r.user_id).run(),e.json({success:!0})}),V.get(`/api/config/kakao-js-key`,e=>e.json({key:e.env.KAKAO_JS_KEY||``}));function gn(){let e=new Uint8Array(32);return crypto.getRandomValues(e),Array.from(e).map(e=>e.toString(16).padStart(2,`0`)).join(``)}function _n(){return`u_`+gn().substring(0,16)}async function vn(e){let t=gn().substring(0,16),n=new TextEncoder().encode(t+e),r=await crypto.subtle.digest(`SHA-256`,n);return`${t}:${Array.from(new Uint8Array(r)).map(e=>e.toString(16).padStart(2,`0`)).join(``)}`}async function yn(e,t){let[n,r]=t.split(`:`),i=new TextEncoder().encode(n+e),a=await crypto.subtle.digest(`SHA-256`,i);return Array.from(new Uint8Array(a)).map(e=>e.toString(16).padStart(2,`0`)).join(``)===r}async function bn(e,t){if(!t)return null;let n=new Date().toISOString();return await e.prepare(`
    SELECT u.id, u.name, u.email, u.role, u.status, u.credits, u.avatar_url, u.provider, u.referrer
    FROM user_sessions s JOIN users u ON s.user_id = u.id
    WHERE s.token = ? AND s.expires_at > ? AND u.status = 'active'
  `).bind(t,n).first()||null}async function xn(e,t){let n=gn(),r=new Date(Date.now()+720*60*60*1e3).toISOString();return await e.prepare(`DELETE FROM user_sessions WHERE user_id = ? AND token NOT IN (
    SELECT token FROM user_sessions WHERE user_id = ? ORDER BY created_at DESC LIMIT 4
  )`).bind(t,t).run(),await e.prepare(`INSERT INTO user_sessions (token, user_id, expires_at) VALUES (?, ?, ?)`).bind(n,t,r).run(),n}function Sn(e){return{id:e.id,name:e.name,email:e.email,role:e.role,credits:e.credits,avatar_url:e.avatar_url,provider:e.provider,referrer:e.referrer??null}}function Cn(e){let t=e instanceof Uint8Array?e:new Uint8Array(e),n=``;for(let e of t)n+=String.fromCharCode(e);return btoa(n).replace(/\+/g,`-`).replace(/\//g,`_`).replace(/=+$/,``)}async function wn(e,t){let n={alg:`HS256`,typ:`JWT`},r=Math.floor(Date.now()/1e3),i={...t,iat:r,exp:r+60},a=new TextEncoder,o=`${Cn(a.encode(JSON.stringify(n)))}.${Cn(a.encode(JSON.stringify(i)))}`,s=await crypto.subtle.importKey(`raw`,a.encode(e),{name:`HMAC`,hash:`SHA-256`},!1,[`sign`]);return`${o}.${Cn(await crypto.subtle.sign(`HMAC`,s,a.encode(o)))}`}function Tn(e){let t=e.replace(/-/g,`+`).replace(/_/g,`/`)+`=`.repeat((4-e.length%4)%4),n=atob(t),r=new Uint8Array(n.length);for(let e=0;e<n.length;e++)r[e]=n.charCodeAt(e);return r}async function En(e,t){let n=t.split(`.`);if(n.length!==3)return null;let[r,i,a]=n,o=new TextEncoder;try{let t=await crypto.subtle.importKey(`raw`,o.encode(e),{name:`HMAC`,hash:`SHA-256`},!1,[`verify`]);if(!await crypto.subtle.verify(`HMAC`,t,Tn(a),o.encode(`${r}.${i}`)))return null;let n=JSON.parse(new TextDecoder().decode(Tn(i)));return!n?.sub||typeof n.exp!=`number`||n.exp<Math.floor(Date.now()/1e3)?null:{sub:String(n.sub)}}catch{return null}}var Dn=[`BFM회원`,`코오롱 FnC`,`한섬`],On=500,kn={BFM회원:.2};V.post(`/api/auth/signup`,async e=>{try{let t=e.env.LOOKBOOK_DB,n=await e.req.json(),{name:r,email:i,password:a,agreeMarketing:o}=n,s=Dn.includes(n?.referrer)?n.referrer:null;if(!r||!i||!a)return e.json({success:!1,message:`모든 항목을 입력해주세요.`},400);if(a.length<8)return e.json({success:!1,message:`비밀번호는 8자 이상이어야 합니다.`},400);if(await t.prepare(`SELECT id FROM users WHERE email = ?`).bind(i).first())return e.json({success:!1,message:`이미 가입된 이메일입니다.`},409);let l=_n(),u=await vn(a),d=+!!o,f=s===`BFM회원`?On:200;await t.prepare(`
...
<\/script>
       ...
       FROM credit_logs WHERE user_id = ? ORDER BY created_at DESC, id DESC LIMIT ? OFFSET ?`).bind(n,i,a).all();return e.json({success:!0,events:s.results,total:o?.cnt||0,page:r,limit:i})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.patch(`/api/admin/users/:id`,W,async e=>{try{let t=e.env.LOOKBOOK_DB,n=await e.req.json(),r=e.req.param(`id`),i=[],a=[];if(n.status!==void 0&&(i.push(`status = ?`),a.push(n.status)),n.role!==void 0&&(i.push(`role = ?`),a.push(n.role)),n.set_password!==void 0){if(typeof n.set_password!=`string`||n.set_password.length<6)return e.json({success:!1,message:`비밀번호는 6자 이상이어야 합니다.`},400);i.push(`password_hash = ?`),a.push(await vn(n.set_password))}if(n.add_credits!==void 0){let o=(await t.prepare(`SELECT credits FROM users WHERE id = ?`).bind(r).first())?.credits??0,s=parseInt(n.add_credits),l=Math.max(0,o+s);return i.push(`credits = ?`),a.push(l),i.push(`updated_at = datetime('now')`),await t.prepare(`UPDATE users SET ${i.join(`, `)} WHERE id = ?`).bind(...a,r).run(),await t.prepare(`INSERT INTO credit_logs (user_id, type, amount, balance, reason, ref_id)
         VALUES (?, 'grant', ?, ?, 'admin_grant', ?)`).bind(r,s,l,`admin_${Date.now()}`).run(),n.status===`suspended`&&await t.prepare(`DELETE FROM user_sessions WHERE user_id = ?`).bind(r).run(),e.json({success:!0,newCredits:l})}if(n.credits!==void 0){let o=(await t.prepare(`SELECT credits FROM users WHERE id = ?`).bind(r).first())?.credits??0,s=parseInt(n.credits),l=s-o;return i.push(`credits = ?`),a.push(s),i.push(`updated_at = datetime('now')`),await t.prepare(`UPDATE users SET ${i.join(`, `)} WHERE id = ?`).bind(...a,r).run(),l!==0&&await t.prepare(`INSERT INTO credit_logs (user_id, type, amount, balance, reason, ref_id)
           VALUES (?, ?, ?, ?, 'admin_set', ?)`).bind(r,l>0?`grant`:`deduct`,l,s,`admin_${Date.now()}`).run(),n.status===`suspended`&&await t.prepare(`DELETE FROM user_sessions WHERE user_id = ?`).bind(r).run(),e.json({success:!0,newCredits:s})}return i.length===0?e.json({success:!1,message:`변경할 항목이 없습니다.`},400):(i.push(`updated_at = datetime('now')`),await t.prepare(`UPDATE users SET ${i.join(`, `)} WHERE id = ?`).bind(...a,r).run(),n.status===`suspended`&&await t.prepare(`DELETE FROM user_sessions WHERE user_id = ?`).bind(r).run(),e.json({success:!0}))}catch(t){return e.json({success:!1,message:t.message},500)}}),V.delete(`/api/admin/users/:id`,W,async e=>{try{let t=e.env.LOOKBOOK_DB,n=e.req.param(`id`);return await t.prepare(`UPDATE users SET status = 'deleted', updated_at = datetime('now') WHERE id = ?`).bind(n).run(),await t.prepare(`DELETE FROM user_sessions WHERE user_id = ?`).bind(n).run(),e.json({success:!0})}catch(t){return e.json({success:!1,message:t.message},500)}});var An=.2,jn=async(e,t)=>{let n=e.env.LOOKBOOK_DB,r=await bn(n,e.req.header(`X-Session-Token`)||null);if(!r)return e.json({success:!1,message:`로그인이 필요합니다.`},401);if(r.role!==`bfm_admin`)return e.json({success:!1,message:`BFM 관리자 권한이 없습니다.`},403);await t()};V.post(`/api/bfm/login`,async e=>{try{let t=e.env.LOOKBOOK_DB,n=await e.req.json(),r=(n.email||``).trim().toLowerCase(),i=n.password||``;if(!r||!i)return e.json({success:!1,message:`이메일과 비밀번호를 입력해주세요.`},400);let a=await t.prepare(`SELECT * FROM users WHERE email = ?`).bind(r).first();if(!a||a.status!==`active`||!a.password_hash||!await yn(i,a.password_hash))return e.json({success:!1,message:`이메일 또는 비밀번호가 올바르지 않습니다.`},401);if(a.role!==`bfm_admin`)return e.json({success:!1,message:`BFM 관리자 권한이 없습니다.`},403);let o=await xn(t,a.id);return e.json({success:!0,token:o,user:Sn(a)})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.get(`/api/bfm/members`,jn,async e=>{try{let t=e.env.LOOKBOOK_DB,n=Math.max(1,parseInt(e.req.query(`page`)||`1`)),r=Math.max(1,Math.min(50,parseInt(e.req.query(`limit`)||`20`))),i=(n-1)*r,a=await t.prepare(`SELECT COUNT(*) as cnt FROM users WHERE referrer = 'BFM회원'`).first(),o=((await t.prepare(`SELECT u.id, u.name, u.email, u.credits, u.status, u.created_at,
              COALESCE((SELECT SUM(amount) FROM payment_logs WHERE user_id = u.id AND status = 'paid'), 0) AS total_paid
       FROM users u WHERE u.referrer = 'BFM회원' ORDER BY u.created_at DESC LIMIT ? OFFSET ?`).bind(r,i).all()).results||[]).map(e=>({...e,bfm_revenue:Math.round((e.total_paid||0)*An)}));return e.json({success:!0,members:o,total:a?.cnt||0,page:n,limit:r})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.get(`/api/bfm/members/:id/payments`,jn,async e=>{try{let t=e.env.LOOKBOOK_DB,n=e.req.param(`id`);if(!await t.prepare(`SELECT id FROM users WHERE id = ? AND referrer = 'BFM회원'`).bind(n).first())return e.json({success:!1,message:`조회 권한이 없는 회원입니다.`},403);let r=Math.max(1,parseInt(e.req.query(`page`)||`1`)),i=Math.max(1,parseInt(e.req.query(`limit`)||`10`)),a=(r-1)*i,o=await t.prepare(`SELECT COUNT(*) as cnt FROM payment_logs WHERE user_id = ?`).bind(n).first(),s=await t.prepare(`SELECT order_id, amount, credits, status, pg_provider, currency, pg_method, created_at, paid_at
     ...
     WHERE s.token = ? AND s.expires_at > datetime('now')`).bind(r).first();if(!i)return e.json({error:`세션이 만료되었습니다.`},401);try{let{packageId:r,currency:a}=await e.req.json(),o=Mn[r];if(!o)return e.json({error:`잘못된 패키지입니다.`},400);let s=String(a||`USD`).toUpperCase();if(s!==`USD`&&s!==`JPY`)return e.json({error:`지원하지 않는 통화입니다.`},400);let l=s===`USD`?o.usdCents:o.jpyAmount,u=kn[i.referrer]||0,d=u>0?Math.round(l*(1-u)):l,f=`lookbook-${i.user_id.slice(0,6)}-${Date.now()}-${Math.random().toString(36).slice(2,6).toUpperCase()}`;await n.prepare(`INSERT INTO payment_logs (user_id, order_id, amount, credits, status, pg_provider, currency)
       ...
       LIMIT 50`).all();return e.json({success:!0,jobs:n.results||[]})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.get(`/api/admin/debug/atlas-job/:jobId`,W,async e=>{let t=e.req.param(`jobId`),n=e.env.LOOKBOOK_DB;try{let r=await n.prepare(`SELECT id, job_id, kind, status, video_url, created_at FROM generation_logs WHERE job_id = ?`).bind(t).first(),i=await fetch(`${H}/api/v1/model/prediction/${t}`,{headers:{Authorization:`Bearer ${e.env.ATLAS_API_KEY}`}}),a=await i.text(),o=null;try{o=JSON.parse(a)}catch{}return e.json({dbRow:r,atlasHttpStatus:i.status,atlasRaw:o??a})}catch(t){return e.json({success:!1,message:t.message},500)}});async function Kn(e){try{let t=await e.prepare(`SELECT value FROM app_settings WHERE key = 'admin_prompt_config'`).first();if(t?.value){let e=JSON.parse(t.value);return{enabled:typeof e.enabled==`boolean`?e.enabled:G.enabled,prefix:typeof e.prefix==`string`?e.prefix:G.prefix,suffix:typeof e.suffix==`string`?e.suffix:G.suffix,styleGuide:typeof e.styleGuide==`string`?e.styleGuide:G.styleGuide,technicalSpec:typeof e.technicalSpec==`string`?e.technicalSpec:G.technicalSpec,updatedAt:e.updatedAt||G.updatedAt}}}catch(e){console.warn(`d1GetPromptConfig fallback to memory:`,e)}return G}async function qn(e,t){await e.prepare(`INSERT INTO app_settings (key, value, updated_at) VALUES ('admin_prompt_config', ?, datetime('now'))
     ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = excluded.updated_at`).bind(JSON.stringify(t)).run()}V.get(`/api/admin/prompt`,W,async e=>{try{let t=e.env.LOOKBOOK_DB,n=await Kn(t);return e.json({success:!0,config:n})}catch{return e.json({success:!0,config:G})}}),V.put(`/api/admin/prompt`,W,async e=>{try{let t=e.env.LOOKBOOK_DB,n=await e.req.json(),r=await Kn(t),i={enabled:typeof n.enabled==`boolean`?n.enabled:r.enabled,prefix:typeof n.prefix==`string`?n.prefix:r.prefix,suffix:typeof n.suffix==`string`?n.suffix:r.suffix,styleGuide:typeof n.styleGuide==`string`?n.styleGuide:r.styleGuide,technicalSpec:typeof n.technicalSpec==`string`?n.technicalSpec:r.technicalSpec,updatedAt:new Date().toISOString()};return await qn(t,i),G=i,console.log(`Admin prompt config saved to D1:`,i.updatedAt),e.json({success:!0,config:i})}catch(t){return e.json({success:!1,message:t.message},400)}}),V.post(`/api/admin/auth`,async e=>{let t=await e.req.json(),n=e.env.ADMIN_PASSWORD;return n?t.password===n?e.json({success:!0}):e.json({success:!1,message:`비밀번호가 올바르지 않습니다.`},401):e.json({success:!1,message:`서버 설정 오류: ADMIN_PASSWORD 환경변수가 설정되지 않았습니다.`},500)});var Jn=`의류 이미지 하나로 AI 온모델 피팅컷과 룩북 세트를 자동 생성하세요.`,Yn=(e,t,n=``,r=Jn,i)=>`<!DOCTYPE html>
...
`;return e.text(t,200,{"Content-Type":`text/markdown; charset=utf-8`})});var Xn=[{q:`EZlook은 어떤 서비스인가요?`,a:`의류 이미지 한 장을 업로드하면 AI가 온모델 피팅컷과 룩북 세트를 자동으로 생성해주는 AI 패션 이미지 생성 플랫폼입니다. 촬영 스튜디오나 모델 섭외 없이 몇 번의 클릭만으로 전문적인 착용샷을 만들 수 있습니다.`},{q:`이미지 생성에 비용이 드나요?`,a:`이미지 생성 자체는 무료입니다. 마음에 드는 결과물을 실제 파일로 다운로드할 때만 장당 90크레딧이 차감됩니다.`},{q:`무료로 체험할 수 있나요?`,a:`네, 신용카드 등록 없이 회원가입만 하면 무료 크레딧이 바로 지급되어 AI 룩북 제작을 체험해볼 수 있습니다.`},{q:`영상도 만들 수 있나요?`,a:`네, 생성된 피팅컷 이미지를 기반으로 모델이 자연스럽게 포즈를 취하는 7초 분량의 세로형(9:16) 영상을 만들 수 있습니다.`},{q:`결과물은 얼마나 빨리 나오나요?`,a:`평균 30초, 최대 90초 이내에 고품질 온모델 피팅컷 이미지가 생성됩니다.`},{q:`크레딧은 어떻게 충전하나요?`,a:`월 정액 없이 필요한 만큼만 충전하는 방식입니다. 스타터(₩20,000 · 1,000크레딧)부터 베스트 밸류(₩60,000 · 4,000크레딧)까지 선택할 수 있습니다.`}],Zn=()=>{let e={"@type":`Organization`,name:`EZlook`,alternateName:`벌거벗은호랑이`,url:U,logo:`${U}/static/favicon.svg`,telephone:`070-4581-8166`,address:{"@type":`PostalAddress`,streetAddress:`무심서로 377-3`,addressLocality:`청주시 서원구`,addressRegion:`충청북도`,addressCountry:`KR`}};return[{"@context":`https://schema.org`,"@type":`WebSite`,name:`EZlook`,url:U,publisher:e},{"@context":`https://schema.org`,"@type":`Service`,name:`EZlook AI 패션 룩북 생성 서비스`,serviceType:`AI 패션 이미지/영상 생성`,description:`옷 사진 한 장을 업로드하면 AI 모델이 착용한 온모델 피팅컷과 룩북 세트, 홍보 영상을 자동으로 생성하는 서비스`,provider:e,areaServed:`KR`,offers:[{"@type":`Offer`,name:`스타터`,price:`20000`,priceCurrency:`KRW`,description:`1,000 크레딧 · 이미지 최대 11장`},{"@type":`Offer`,name:`인기 충전권`,price:`40000`,priceCurrency:`KRW`,description:`2,300 크레딧 · 이미지 최대 25장`},{"@type":`Offer`,name:`베스트 밸류`,price:`60000`,priceCurrency:`KRW`,description:`4,000 크레딧 · 이미지 최대 44장`}]},{"@context":`https://schema.org`,"@type":`FAQPage`,mainEntity:Xn.map(e=>({"@type":`Question`,name:e.q,acceptedAnswer:{"@type":`Answer`,text:e.a}}))}].map(e=>`<script type="application/ld+json">${JSON.stringify(e)}<\/script>`).join(`
  ...
  `,t,`옷 사진 한 장으로 AI 온모델 피팅컷과 룩북 세트를 무료로 자동 생성하세요. 촬영 스튜디오나 모델 섭외 없이 평균 30초 만에 완성됩니다.`,e.env.GA4_MEASUREMENT_ID))}),V.get(`/dashboard`,e=>e.html(Yn(`내 프로필`,`
  <div class="toast-container" id="toastContainer"></div>
  ...
  <\/script>
  `,``,Jn,e.env.GA4_MEASUREMENT_ID))),V.get(`/credits`,e=>e.html(Yn(`크레딧 상세`,`
  <div class="toast-container" id="toastContainer"></div>
  ...
  `,a,n,e.env.GA4_MEASUREMENT_ID))};V.get(`/`,e=>Qn(e,`model`)),V.get(`/generator`,e=>e.redirect(`/`,301)),V.get(`/ghostcut`,e=>Qn(e,`ghostcut`)),V.get(`/techpack`,e=>{let t=`<link rel="canonical" href="${U}/techpack" />`;return e.html(Yn(`AI 도식화 만들기`,`
  <div class="toast-container" id="toastContainer"></div>
...
</html>`));var $n=new Oe,er=Object.assign({"/src/index.tsx":V}),tr=!1;for(let[,e]of Object.entries(er))e&&($n.all(`*`,t=>{let n;try{n=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,n)}),$n.notFound(t=>{let n;try{n=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,n)}),tr=!0);if(!tr)throw Error(`Can't import modules from ['/src/index.ts','/src/index.tsx','/app/server.ts']`);export{$n as default};