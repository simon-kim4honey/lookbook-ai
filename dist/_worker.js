...
<\/script>
       ...
       VALUES (?, ?, ?, ?, 'pending')`).bind(r.user_id,s,a.amount,a.credits).run(),e.json({success:!0,orderId:s,amount:a.amount,credits:a.credits,orderName:a.label,customerName:r.name,customerEmail:r.email})}catch(t){return e.json({success:!1,message:t.message},500)}}),W.post(`/api/payments/confirm`,async e=>{try{let t=e.env.LOOKBOOK_DB,{paymentKey:n,orderId:r,amount:i}=await e.req.json();if(!n||!r||!i)return e.json({error:`paymentKey, orderId, amount 필수`},400);let a=await t.prepare(`SELECT id, user_id, amount, credits, status FROM payment_logs
...
<body>
...
</html>`)),W.get(`/_home_old`,e=>e.redirect(`/generator`,302)),W.get(`/`,e=>e.html(gt(`홈`,`
  <!-- Toast Container -->
  ...
  `))),W.get(`/dashboard`,e=>e.html(gt(`내 프로필`,`
  <div class="toast-container" id="toastContainer"></div>
  ...
  <\/script>
  `))),W.get(`/generator`,e=>e.html(gt(`AI 룩북 생성`,`
  <div class="toast-container" id="toastContainer"></div>
...
</html>`));var _t=new xe,vt=Object.assign({"/src/index.tsx":W}),yt=!1;for(let[,e]of Object.entries(vt))e&&(_t.all(`*`,t=>{let n;try{n=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,n)}),_t.notFound(t=>{let n;try{n=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,n)}),yt=!0);if(!yt)throw Error(`Can't import modules from ['/src/index.ts','/src/index.tsx','/app/server.ts']`);export{_t as default};