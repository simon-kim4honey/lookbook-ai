...
`)){let t=e.trim();if(!t||t.startsWith(`#`))continue;let[n,...r]=t.split(`:`),i=n.trim().toLowerCase(),l=r.join(`:`).trim();i===`user-agent`?o=l===`*`:o&&i===`disallow`&&l&&a.startsWith(l)&&(s=!0)}return!s}catch{return!0}}U.post(`/collect/scrape`,async e=>{let{platformId:t}=await e.req.json(),n=await e.env.LOOKBOOK_DB.prepare(`SELECT * FROM lead_platforms WHERE id = ?`).bind(t).first();if(!n)return e.json({success:!1,message:`플랫폼을 찾을 수 없습니다.`},404);if(!n.is_scrapable)return e.json({success:!1,message:`이 플랫폼은 is_scrapable=0 입니다. 이용약관/robots.txt를 확인하고 셀렉터를 검증한 뒤 관리자가 직접 활성화해야 합니다.`},400);if(!n.directory_url||!n.list_selector)return e.json({success:!1,message:`directory_url과 list_selector를 먼저 등록하세요.`},400);if(!await Fe(n.directory_url))return e.json({success:!1,message:`robots.txt에 의해 해당 경로 크롤링이 금지되어 있습니다. 수집을 중단합니다.`},403);let r=(await e.env.LOOKBOOK_DB.prepare(`INSERT INTO collection_jobs (platform_id, method, status) VALUES (?, 'scrape', 'running')`).bind(n.id).run()).meta.last_row_id;try{let t=await fetch(n.directory_url,{headers:{"User-Agent":`LookbookAI-LeadBot/1.0 (+brand outreach lead collection; contact: admin of lookbook-ai)`}});if(!t.ok)throw Error(`대상 페이지 응답 오류: HTTP ${t.status}`);let i=await t.text(),a=[];await new HTMLRewriter().on(n.list_selector,{element(e){a.push({name:``,href:e.getAttribute(`href`)||``}),e._captureIndex=a.length-1},text(e){a.length>0&&(a[a.length-1].name+=e.text)}}).transform(new Response(i)).text();let o=[];if(n.category_selector){let e=``;await new HTMLRewriter().on(`${n.list_selector} ${n.category_selector}`,{text(t){e+=t.text,t.lastInTextNode&&(o.push(e.trim()),e=``)}}).transform(new Response(i)).text()}let s=new URL(n.directory_url),l=0;for(let t=0;t<Math.min(a.length,300);t++){let r=a[t].name.trim();if(!r)continue;let i=a[t].href;try{i=new URL(i,s).toString()}catch{}let u=o[t]||null;(await e.env.LOOKBOOK_DB.prepare(`INSERT OR IGNORE INTO brands (platform_id, country, name, category, brand_url, source, status)
    ...
    WHERE 1=1
  `,l=[];t&&(s+=` AND b.country = ?`,l.push(t)),n&&(s+=` AND b.platform_id = ?`,l.push(n)),r&&(s+=` AND b.status = ?`,l.push(r)),i&&(s+=` AND b.name LIKE ?`,l.push(`%${i}%`)),s+=` ORDER BY COALESCE(a.priority_score, -1) DESC, b.collected_at DESC LIMIT ? OFFSET ?`,l.push(a,o);let{results:u}=await e.env.LOOKBOOK_DB.prepare(s).bind(...l).all();return e.json({success:!0,brands:u})}),U.get(`/brands/:id`,async e=>{let t=e.req.param(`id`),n=await e.env.LOOKBOOK_DB.prepare(`SELECT * FROM brands WHERE id = ?`).bind(t).first();if(!n)return e.json({success:!1,message:`브랜드를 찾을 수 없습니다.`},404);let{results:r}=await e.env.LOOKBOOK_DB.prepare(`SELECT * FROM brand_analysis WHERE brand_id = ? ORDER BY analyzed_at DESC`).bind(t).all(),{results:i}=await e.env.LOOKBOOK_DB.prepare(`SELECT * FROM outreach_drafts WHERE brand_id = ? ORDER BY created_at DESC`).bind(t).all();return e.json({success:!0,brand:n,analyses:r,drafts:i})}),U.patch(`/brands/:id`,async e=>{let t=e.req.param(`id`),n=await e.req.json(),r=[],i=[];for(let e of[`status`,`category`,`brand_url`,`contact_email`])n[e]!==void 0&&(r.push(`${e} = ?`),i.push(n[e]));return r.length===0?e.json({success:!1,message:`변경할 필드가 없습니다.`},400):(i.push(t),await e.env.LOOKBOOK_DB.prepare(`UPDATE brands SET ${r.join(`, `)} WHERE id = ?`).bind(...i).run(),e.json({success:!0}))});var Ie={street:[`스트릿`,`스트리트`,`street`,`ストリート`],casual:[`캐주얼`,`casual`,`カジュアル`],minimal:[`미니멀`,`minimal`,`ミニマル`,`contemporary`,`컨템포러리`],luxury:[`럭셔리`,`luxury`,`디자이너`,`designer`,`ラグジュアリー`],golf:[`골프`,`golf`,`ゴルフ`],outdoor:[`아웃도어`,`outdoor`,`アウトドア`],formal:[`포멀`,`수트`,`suit`,`formal`,`オフィス`,`フォーマル`],vintage:[`빈티지`,`vintage`,`ヴィンテージ`,`古着`],kids:[`키즈`,`아동`,`kids`,`baby`,`キッズ`],lingerie:[`언더웨어`,`속옷`,`lingerie`,`ランジェリー`],shoes:[`슈즈`,`신발`,`shoes`,`靴`],bag:[`가방`,`bag`,`バッグ`]},Le={luxury:[`럭셔리`,`luxury`,`디자이너`,`designer`,`ラグジュアリー`],premium:[`프리미엄`,`premium`,`컨템포러리`,`contemporary`],budget:[`보세`,`균일가`,`budget`,`저가`,`プチプラ`]};function Re(e,t){let n=`${e} ${t||``}`.toLowerCase(),r=[];for(let[e,t]of Object.entries(Ie))t.some(e=>n.includes(e.toLowerCase()))&&r.push(e);let i=`mid`;for(let[e,t]of Object.entries(Le))if(t.some(e=>n.includes(e.toLowerCase()))){i=e;break}let a=55;(r.includes(`street`)||r.includes(`casual`)||r.includes(`minimal`))&&(a+=15),(r.includes(`shoes`)||r.includes(`bag`))&&(a-=10),a=Math.max(0,Math.min(100,a));let o=Math.round(a*.7+({budget:40,mid:60,premium:80,luxury:70}[i]||60)*.3),s=`${e}은(는) ${t||`패션`} 카테고리${r.length?` (${r.join(`, `)} 스타일)`:``}로 추정되며, 가격대는 ${i} 수준으로 분류됩니다. 시즌 룩북 갱신 빈도가 높을 가능성이 있어 AI 룩북 자동 생성 제안 대상으로 적합합니다.`;return{style_tags:r,target_customer:r.includes(`kids`)?`키즈/부모`:`20~30대 일반 소비자(추정)`,price_tier:i,lookbook_need_score:a,priority_score:Math.max(0,Math.min(100,o)),ai_summary:s,analysis_method:`heuristic`}}async function ze(e,t,n,r){let i=`You are a B2B market analyst for an AI fashion lookbook generation SaaS. Given a fashion brand name and category, return ONLY strict JSON (no markdown fences) with this shape:
...
`),200,{"Content-Type":`text/csv; charset=utf-8`,"Content-Disposition":`attachment; filename="brand_leads_export.csv"`})});var Ue=`mt42skmf`,We=e=>e?`
...
Respond with ONLY one word: YES or NO.`};W.post(`/api/validate/clothing`,async e=>{try{let t=await e.req.json(),n=t?.imageBase64||``,r=t?.cat||``;if(!n)return e.json({success:!1,message:`imageBase64 필수`},400);let i=e.env?.OPENAI_API_KEY||``;if(!i)return e.json({success:!0,isClothing:!0});let a=await dt(i,n,ct[r]||ct.DRESS);return a===null?e.json({success:!0,isClothing:!0}):e.json({success:!0,isClothing:a.trim().toUpperCase().startsWith(`YES`)})}catch(t){return console.error(`validate/clothing error:`,t),e.json({success:!0,isClothing:!0})}}),W.post(`/api/admin/auto-label`,q,async e=>{try{let{type:t,imageBase64:n}=await e.req.json();if(!n)return e.json({success:!1,message:`imageBase64 필수`},400);let r=e.env?.OPENAI_API_KEY||``;if(!r)return e.json({success:!1,message:`OPENAI_API_KEY 미설정`},500);let i=e=>{let t=e.match(/\{[\s\S]*\}/);return JSON.parse(t?.[0]||e)};if(t===`model`){let t=await dt(r,n,`You are an AI fashion model classifier. Analyze this model photo and return ONLY a JSON object with these exact fields:
{
  "gender": "여성" or "남성",
...
Return ONLY the JSON, no explanation.`);if(t===null)return e.json({success:!1,message:`라벨링 요청 실패`},500);try{let n=i(t);return e.json({success:!0,labels:{category:n.category||`스튜디오`,mood:n.mood||`미니멀`,name_ko:n.name_ko||``}})}catch{return e.json({success:!1,message:`응답 파싱 실패`,raw:t})}}return e.json({success:!1,message:`type은 model 또는 background 이어야 합니다.`},400)}catch(t){return e.json({success:!1,message:t.message},500)}}),W.patch(`/api/admin/models/:id/labels`,q,async e=>{try{let t=e.req.param(`id`),{gender:n,age:r,mood:i}=await e.req.json(),a=e.env?.LOOKBOOK_KV,o=e.env?.LOOKBOOK_DB;if(a){let o=await Y(a),s=o.findIndex(e=>e.id===t);return s===-1?e.json({success:!1,message:`모델을 찾을 수 없습니다.`},404):(o[s]={...o[s],gender:n||`미분류`,age:r||`미분류`,mood:i||`미분류`},await Ke(a,o),e.json({success:!0}))}return o?(await o.prepare(`UPDATE custom_models SET gender=?, age=?, mood=? WHERE id=?`).bind(n||`미분류`,r||`미분류`,i||`미분류`,t).run(),e.json({success:!0})):e.json({success:!1,message:`D1/KV 없음`},500)}catch(t){return e.json({success:!1,message:t.message},500)}}),W.get(`/api/admin/models`,q,async e=>{let t=e.env?.LOOKBOOK_KV,n=e.env?.LOOKBOOK_DB;if(t){let n=await Y(t);return e.json({success:!0,models:n})}if(n){let t=await Ze(n);return e.json({success:!0,models:t})}let r=Z.map(e=>({id:e.id,name:e.name,desc:e.desc,createdAt:e.createdAt}));return e.json({success:!0,models:r})}),W.delete(`/api/admin/models/:id`,q,async e=>{let t=e.req.param(`id`),n=e.env?.LOOKBOOK_KV,r=e.env?.LOOKBOOK_DB;if(n){let r=await Y(n),i=r.filter(e=>e.id!==t);return await Ke(n,i),await n.delete(`model_img:${t}`),e.json({success:r.length>i.length})}if(r){let n=await $e(r,t);return e.json({success:n})}let i=Z.length;return Z=Z.filter(e=>e.id!==t),e.json({success:Z.length<i})}),W.get(`/api/proxy/custom-model/:id`,async e=>{let t=e.req.param(`id`),n=e.env?.LOOKBOOK_KV,r=e.env?.LOOKBOOK_DB,i=null;if(i=n?await n.get(`model_img:${t}`):r?await et(r,t):Z.find(e=>e.id===t)?.imageBase64||null,!i)return e.notFound();let[a,o]=i.split(`,`),s=(a.match(/data:([^;]+)/)||[])[1]||`image/jpeg`,l=atob(o),u=new Uint8Array(l.length);for(let e=0;e<l.length;e++)u[e]=l.charCodeAt(e);return new Response(u.buffer,{headers:{"Content-Type":s,"Cache-Control":`public, max-age=3600`}})}),W.post(`/api/admin/backgrounds`,q,async e=>{try{let t=await e.req.json(),n=Array.isArray(t)?t:[t];if(n.length===0)return e.json({success:!1,message:`업로드할 항목이 없습니다.`},400);let r=e.env?.LOOKBOOK_KV,i=e.env?.LOOKBOOK_DB,a=[];if(r){let e=await X(r);for(let t of n){let{name:n,bgDesc:i,category:o,imageBase64:s}=t;if(!n||!s)continue;let l=await Ye(r),u={id:l,name:n,bgDesc:i||n,category:o||`커스텀`,createdAt:new Date().toISOString()};e.push(u),await r.put(`bg_img:${l}`,s),a.push(u)}await qe(r,e)}else if(i)a=await rt(i,n);else for(let e of n){let{name:t,bgDesc:n,category:r,imageBase64:i}=e;if(!t||!i)continue;let o=String(st++),s={id:o,name:t,bgDesc:n||t,category:r||`커스텀`,imageBase64:i,createdAt:new Date().toISOString()};Q.push(s),a.push({id:o,name:t,bgDesc:s.bgDesc,category:s.category,createdAt:s.createdAt})}return e.json({success:!0,backgrounds:a,count:a.length})}catch(t){return e.json({success:!1,message:t.message},500)}}),W.get(`/api/admin/backgrounds`,q,async e=>{let t=e.env?.LOOKBOOK_KV,n=e.env?.LOOKBOOK_DB;if(t){let n=await X(t);return e.json({success:!0,backgrounds:n})}if(n){let t=await tt(n);return e.json({success:!0,backgrounds:t})}let r=Q.map(e=>({id:e.id,name:e.name,bgDesc:e.bgDesc,category:e.category,createdAt:e.createdAt,hasGenImage:!!e.genImageBase64,isDefault:!!e.isDefault}));return e.json({success:!0,backgrounds:r})}),W.delete(`/api/admin/backgrounds/:id`,q,async e=>{let t=e.req.param(`id`),n=e.env?.LOOKBOOK_KV,r=e.env?.LOOKBOOK_DB;if(n){let r=await X(n),i=r.filter(e=>e.id!==t);return await qe(n,i),await n.delete(`bg_img:${t}`),await n.delete(`bg_gen_img:${t}`),e.json({success:r.length>i.length})}if(r){let n=await it(r,t);return e.json({success:n})}let i=Q.length;return Q=Q.filter(e=>e.id!==t),e.json({success:Q.length<i})}),W.put(`/api/admin/backgrounds/:id/gen-image`,q,async e=>{let t=e.req.param(`id`);try{let n=(await e.req.json())?.imageBase64||``;if(!n)return e.json({success:!1,message:`imageBase64 필수`},400);let r=e.env?.LOOKBOOK_KV,i=e.env?.LOOKBOOK_DB;if(r){let i=await X(r),a=i.find(e=>e.id===t);return a?(await r.put(`bg_gen_img:${t}`,n),a.hasGenImage=!0,await qe(r,i),e.json({success:!0})):e.json({success:!1,message:`배경을 찾을 수 없습니다.`},404)}if(i)return await ot(i,t,n)?e.json({success:!0}):e.json({success:!1,message:`배경을 찾을 수 없습니다.`},404);let a=Q.find(e=>e.id===t);return a?(a.genImageBase64=n,e.json({success:!0})):e.json({success:!1,message:`배경을 찾을 수 없습니다.`},404)}catch(t){return e.json({success:!1,message:t.message},500)}}),W.put(`/api/admin/backgrounds/:id/default`,q,async e=>{let t=e.req.param(`id`);try{let n=e.env?.LOOKBOOK_KV,r=e.env?.LOOKBOOK_DB;if(n){let r=await Je(n,t);return r.ok?e.json({success:!0,isDefault:r.isDefault}):e.json({success:!1,message:`배경을 찾을 수 없습니다.`},404)}if(r){let n=await nt(r,t);return n.ok?e.json({success:!0,isDefault:n.isDefault}):e.json({success:!1,message:`배경을 찾을 수 없습니다.`},404)}let i=Q.find(e=>e.id===t);if(!i)return e.json({success:!1,message:`배경을 찾을 수 없습니다.`},404);let a=!i.isDefault;return Q.forEach(e=>{e.isDefault=!1}),i.isDefault=a,e.json({success:!0,isDefault:a})}catch(t){return e.json({success:!1,message:t.message},500)}}),W.get(`/api/proxy/custom-bg/:id`,async e=>{let t=e.req.param(`id`),n=e.env?.LOOKBOOK_KV,r=e.env?.LOOKBOOK_DB,i=null;if(i=n?await n.get(`bg_img:${t}`):r?await at(r,t):Q.find(e=>e.id===t)?.imageBase64||null,!i)return e.notFound();let[a,o]=i.split(`,`),s=(a.match(/data:([^;]+)/)||[])[1]||`image/jpeg`,l=atob(o),u=new Uint8Array(l.length);for(let e=0;e<l.length;e++)u[e]=l.charCodeAt(e);return new Response(u.buffer,{headers:{"Content-Type":s,"Cache-Control":`public, max-age=3600`}})}),W.get(`/api/proxy/clothing/:jobId/:idx`,async e=>{let t=e.req.param(`jobId`),n=parseInt(e.req.param(`idx`)||`0`,10),r=e.env?.LOOKBOOK_KV;if(!r)return e.notFound();let i=await r.get(`clothing_img:${t}`);if(!i)return e.notFound();let a=[];try{a=JSON.parse(i)}catch{}let o=a[n];if(!o)return e.notFound();let[s,l]=o.split(`,`),u=(s.match(/data:([^;]+)/)||[])[1]||`image/jpeg`,d=atob(l),f=new Uint8Array(d.length);for(let e=0;e<d.length;e++)f[e]=d.charCodeAt(e);return new Response(f.buffer,{headers:{"Content-Type":u,"Cache-Control":`public, max-age=3600`}})});var lt=e=>({Authorization:`Bearer ${e}`,"Content-Type":`application/json`}),ut=`https://api.openai.com`;async function dt(e,t,n){let r=`${ut}/v1/chat/completions`;try{let i=await fetch(r,{method:`POST`,headers:{Authorization:`Bearer ${e}`,"Content-Type":`application/json`},body:JSON.stringify({model:`gpt-4o-mini`,messages:[{role:`user`,content:[{type:`image_url`,image_url:{url:t.startsWith(`data:`)?t:`data:image/jpeg;base64,${t}`}},{type:`text`,text:n}]}]})}),a=await i.text();if(console.log(`openaiChatVision: POST ${r} → HTTP ${i.status} | body(첫 500자): ${a.slice(0,500)}`),!i.ok)return console.error(`openaiChatVision: 요청 실패 (HTTP ${i.status})`),null;let o;try{o=JSON.parse(a)}catch{return console.error(`openaiChatVision: JSON 파싱 실패, 원문:`,a.slice(0,300)),null}let s=o?.choices?.[0]?.message?.content;return typeof s==`string`?s:(console.warn(`openaiChatVision: 예상치 못한 응답 형식:`,JSON.stringify(o).slice(0,300)),null)}catch(e){return console.error(`openaiChatVision error:`,e?.message||e),null}}W.get(`/api/presets/models`,async e=>{let t=e.env?.LOOKBOOK_KV,n=e.env?.LOOKBOOK_DB,r;r=t?await Y(t):n?await Ze(n):Z.map(e=>({id:e.id,name:e.name,desc:e.desc,createdAt:e.createdAt}));let i=r.map(e=>({id:Number(e.id),name:e.name,gender:e.gender||`미분류`,age:e.age||`미분류`,mood:e.mood||`미분류`,body:`-`,skin:`-`,desc:e.desc,unsplashId:null,isCustom:!0,customId:e.id}));return e.json({models:i})}),W.get(`/api/presets/backgrounds`,async e=>{let t=e.env?.LOOKBOOK_KV,n=e.env?.LOOKBOOK_DB,r;r=t?await X(t):n?await tt(n):Q.map(e=>({id:e.id,name:e.name,bgDesc:e.bgDesc,category:e.category,createdAt:e.createdAt,isDefault:!!e.isDefault}));let i=r.map(e=>({id:Number(e.id),name:e.name,category:e.category,mood:`-`,bgDesc:e.bgDesc,unsplashId:null,isCustom:!0,customId:e.id,isDefault:!!e.isDefault}));return e.json({backgrounds:i})});var ft=`home_showcase_images`,pt=[1,2,3,4,5,6];async function mt(e){let t=await e.get(ft);if(!t)return[];try{return JSON.parse(t)}catch{return[]}}async function ht(e,t){await e.put(ft,JSON.stringify(t))}W.get(`/api/home/showcase`,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({images:[]});let n=await mt(t);return e.json({images:n.map(e=>({id:e.id,imageBase64:e.imageBase64}))})}),W.get(`/api/admin/home-showcase`,q,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({success:!1,message:`KV 미설정`},500);let n=await mt(t);return e.json({success:!0,images:n})}),W.post(`/api/admin/home-showcase`,q,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({success:!1,message:`KV 미설정`},500);try{let n=await e.req.json(),r=Array.isArray(n?.images)?n.images:[];if(!r.length)return e.json({success:!1,message:`이미지가 필요합니다.`},400);let i=await mt(t),a=r.map(e=>({id:crypto.randomUUID(),imageBase64:e,createdAt:new Date().toISOString()})),o=[...i,...a];return await ht(t,o),e.json({success:!0,count:a.length,images:o})}catch(t){return e.json({success:!1,message:t.message},500)}}),W.delete(`/api/admin/home-showcase/:id`,q,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({success:!1,message:`KV 미설정`},500);let n=e.req.param(`id`);return await ht(t,(await mt(t)).filter(e=>e.id!==n)),e.json({success:!0})});async function gt(e,t){return await e.get(`home_feature_bg_${t}`)}W.get(`/api/home/feature-bgs`,async e=>{let t=e.env?.LOOKBOOK_KV,n={};if(t)for(let e of pt)n[e]=await gt(t,e);else pt.forEach(e=>{n[e]=null});return e.json({backgrounds:n})}),W.put(`/api/admin/home-feature-bg/:slot`,q,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({success:!1,message:`KV 미설정`},500);let n=Number(e.req.param(`slot`));if(!pt.includes(n))return e.json({success:!1,message:`잘못된 슬롯`},400);try{let r=(await e.req.json())?.imageBase64||``;return r?(await t.put(`home_feature_bg_${n}`,r),e.json({success:!0})):e.json({success:!1,message:`imageBase64 필수`},400)}catch(t){return e.json({success:!1,message:t.message},500)}}),W.delete(`/api/admin/home-feature-bg/:slot`,q,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({success:!1,message:`KV 미설정`},500);let n=Number(e.req.param(`slot`));return pt.includes(n)?(await t.delete(`home_feature_bg_${n}`),e.json({success:!0})):e.json({success:!1,message:`잘못된 슬롯`},400)});var _t=[1],vt=22*1024*1024;W.get(`/api/home/howto-videos`,async e=>{let t=e.env?.LOOKBOOK_KV,n={};if(t)for(let e of _t)n[e]=(await t.getWithMetadata(`home_howto_video_${e}`)).value==null?null:`/api/home/howto-video/${e}`;else _t.forEach(e=>{n[e]=null});return e.json({videos:n})}),W.get(`/api/home/howto-video/:slot`,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.notFound();let n=Number(e.req.param(`slot`));if(!_t.includes(n))return e.notFound();let{value:r,metadata:i}=await t.getWithMetadata(`home_howto_video_${n}`,`arrayBuffer`);if(!r)return e.notFound();let a=r,o=i?.contentType||`video/mp4`,s=a.byteLength,l=e.req.header(`range`);if(l){let e=l.match(/bytes=(\d*)-(\d*)/);if(e){let t=e[1]?parseInt(e[1],10):0,n=e[2]?parseInt(e[2],10):s-1,r=Math.max(0,Math.min(t,s-1)),i=Math.max(r,Math.min(n,s-1)),l=a.slice(r,i+1);return new Response(l,{status:206,headers:{"Content-Type":o,"Content-Range":`bytes ${r}-${i}/${s}`,"Accept-Ranges":`bytes`,"Content-Length":String(l.byteLength),"Cache-Control":`public, max-age=3600`}})}}return new Response(a,{headers:{"Content-Type":o,"Accept-Ranges":`bytes`,"Cache-Control":`public, max-age=3600`,"Content-Length":String(s)}})}),W.put(`/api/admin/home-howto-video/:slot`,q,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({success:!1,message:`KV 미설정`},500);let n=Number(e.req.param(`slot`));if(!_t.includes(n))return e.json({success:!1,message:`잘못된 슬롯`},400);try{let r=await e.req.arrayBuffer();if(!r||r.byteLength===0)return e.json({success:!1,message:`영상 파일이 필요합니다.`},400);if(r.byteLength>vt)return e.json({success:!1,message:`영상 용량은 ${Math.floor(vt/1024/1024)}MB 이하만 가능합니다.`},400);let i=e.req.header(`content-type`)||`video/mp4`;return await t.put(`home_howto_video_${n}`,r,{metadata:{contentType:i}}),e.json({success:!0})}catch(t){return e.json({success:!1,message:t.message},500)}}),W.delete(`/api/admin/home-howto-video/:slot`,q,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({success:!1,message:`KV 미설정`},500);let n=Number(e.req.param(`slot`));return _t.includes(n)?(await t.delete(`home_howto_video_${n}`),e.json({success:!0})):e.json({success:!1,message:`잘못된 슬롯`},400)});var yt=[1,2,3,4,5],bt=22*1024*1024;W.get(`/api/gen-loading-videos`,async e=>{let t=e.env?.LOOKBOOK_KV,n={};if(t)for(let e of yt)n[e]=(await t.getWithMetadata(`gen_loading_video_${e}`)).value==null?null:`/api/gen-loading-video/${e}`;else yt.forEach(e=>{n[e]=null});return e.json({videos:n})}),W.get(`/api/gen-loading-video/:slot`,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.notFound();let n=Number(e.req.param(`slot`));if(!yt.includes(n))return e.notFound();let{value:r,metadata:i}=await t.getWithMetadata(`gen_loading_video_${n}`,`arrayBuffer`);if(!r)return e.notFound();let a=r,o=i?.contentType||`video/mp4`,s=a.byteLength,l=e.req.header(`range`);if(l){let e=l.match(/bytes=(\d*)-(\d*)/);if(e){let t=e[1]?parseInt(e[1],10):0,n=e[2]?parseInt(e[2],10):s-1,r=Math.max(0,Math.min(t,s-1)),i=Math.max(r,Math.min(n,s-1)),l=a.slice(r,i+1);return new Response(l,{status:206,headers:{"Content-Type":o,"Content-Range":`bytes ${r}-${i}/${s}`,"Accept-Ranges":`bytes`,"Content-Length":String(l.byteLength),"Cache-Control":`public, max-age=3600`}})}}return new Response(a,{headers:{"Content-Type":o,"Accept-Ranges":`bytes`,"Cache-Control":`public, max-age=3600`,"Content-Length":String(s)}})}),W.put(`/api/admin/gen-loading-video/:slot`,q,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({success:!1,message:`KV 미설정`},500);let n=Number(e.req.param(`slot`));if(!yt.includes(n))return e.json({success:!1,message:`잘못된 슬롯`},400);try{let r=await e.req.arrayBuffer();if(!r||r.byteLength===0)return e.json({success:!1,message:`영상 파일이 필요합니다.`},400);if(r.byteLength>bt)return e.json({success:!1,message:`영상 용량은 ${Math.floor(bt/1024/1024)}MB 이하만 가능합니다.`},400);let i=e.req.header(`content-type`)||`video/mp4`;return await t.put(`gen_loading_video_${n}`,r,{metadata:{contentType:i}}),e.json({success:!0})}catch(t){return e.json({success:!1,message:t.message},500)}}),W.delete(`/api/admin/gen-loading-video/:slot`,q,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({success:!1,message:`KV 미설정`},500);let n=Number(e.req.param(`slot`));return yt.includes(n)?(await t.delete(`gen_loading_video_${n}`),e.json({success:!0})):e.json({success:!1,message:`잘못된 슬롯`},400)});var xt=`fashion_news_cache_v1`,St=1200*1e3;function Ct(e){let t=e.match(/<!\[CDATA\[([\s\S]*?)\]\]>/);return(t?t[1]:e).replace(/&amp;/g,`&`).replace(/&lt;/g,`<`).replace(/&gt;/g,`>`).replace(/&#39;/g,`'`).replace(/&quot;/g,`"`).trim()}function wt(e){let t=[],n=e.match(/<item>[\s\S]*?<\/item>/g)||[];for(let e of n){let n=e.match(/<title>([\s\S]*?)<\/title>/),r=e.match(/<link>([\s\S]*?)<\/link>/),i=e.match(/<source[^>]*>([\s\S]*?)<\/source>/),a=e.match(/<pubDate>([\s\S]*?)<\/pubDate>/);if(!n||!r)continue;let o=Ct(n[1]),s=i?Ct(i[1]):``;if(s){let e=` - `+s;o.endsWith(e)&&(o=o.slice(0,-e.length).trim())}t.push({title:o,link:Ct(r[1]),source:s,pubDate:a?Ct(a[1]):``})}return t.slice(0,12)}W.get(`/api/fashion-news`,async e=>{let t=e.env?.LOOKBOOK_KV;async function n(){if(!t)return null;let e=await t.get(xt);if(!e)return null;try{return JSON.parse(e).news||null}catch{return null}}try{if(t){let n=await t.get(xt);if(n){let t=JSON.parse(n);if(Date.now()-t.fetchedAt<St)return e.json({news:t.news})}}let r=await fetch(`https://news.google.com/rss/search?q=%ED%8C%A8%EC%85%98&hl=ko&gl=KR&ceid=KR:ko`,{headers:{"User-Agent":`Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36`,Accept:`application/rss+xml, application/xml, text/xml, */*`,"Accept-Language":`ko-KR,ko;q=0.9`}});if(!r.ok)throw Error(`RSS fetch failed: ${r.status}`);let i=await r.text(),a=wt(i);if(a.length===0){let t=await n();return t&&t.length>0?e.json({news:t}):(console.warn(`fashion-news: RSS 응답에서 기사 파싱 실패 (item 0개)`),e.json({news:[],_debug:{httpStatus:r.status,xmlLength:i.length,hasItemTag:i.includes(`<item>`),hasRssTag:i.includes(`<rss`),preview:i.slice(0,500)}}))}return t&&await t.put(xt,JSON.stringify({fetchedAt:Date.now(),news:a})),e.json({news:a})}catch(t){console.warn(`fashion-news fetch error:`,t.message);let r=await n();return r&&r.length>0?e.json({news:r}):e.json({news:[],_debug:{error:t.message}})}}),W.get(`/api/proxy/model-image/:id`,e=>e.notFound()),W.get(`/api/proxy/bg-image/:id`,e=>e.notFound()),W.get(`/api/proxy/gen-image`,async e=>{let t=e.req.query(`url`),n=e.req.query(`download`)===`1`;if(!t)return e.json({error:`Missing url param`},400);try{if(!new URL(t).protocol.startsWith(`https`))return e.json({error:`Only HTTPS URLs allowed`},400);let r=e.req.header(`Range`),i={"User-Agent":`Mozilla/5.0 (compatible; LookbookAI/1.0)`};r&&(i.Range=r);let a=await fetch(t,{headers:i});if(!a.ok&&a.status!==206)return e.json({error:`Upstream error: ${a.status}`},a.status);let o=a.headers.get(`Content-Type`)||`image/jpeg`,s=o.includes(`video`)?`mp4`:`jpg`,l=`lookbook_ai_${Date.now()}.${s}`,u={"Content-Type":o,"Cache-Control":`public, max-age=3600`,"Access-Control-Allow-Origin":`*`,"Accept-Ranges":`bytes`},d=a.headers.get(`Content-Range`);d&&(u[`Content-Range`]=d);let f=a.headers.get(`Content-Length`);return f&&(u[`Content-Length`]=f),n&&(u[`Content-Disposition`]=`attachment; filename="${l}"`),new Response(a.body,{status:a.status,headers:u})}catch(t){return console.error(`Gen image proxy error:`,t),e.json({error:t.message},500)}});var Tt=[{id:`p1`,name:`2024 S/S 룩북`,status:`done`,images:8,created:`2024-03-15`,thumb_color:`#FF6B9D`},{id:`p2`,name:`캐주얼 티셔츠 컷`,status:`done`,images:4,created:`2024-03-12`,thumb_color:`#6C47FF`},{id:`p3`,name:`데님 라인 촬영`,status:`processing`,images:0,created:`2024-03-10`,thumb_color:`#3B82F6`},{id:`p4`,name:`원피스 봄 컬렉션`,status:`draft`,images:0,created:`2024-03-08`,thumb_color:`#00D4AA`}];W.get(`/api/projects`,e=>e.json({projects:Tt}));function Et(e){let t=(e||``).toUpperCase();return t===`KR`||!t?{locale:`ko`,currency:`KRW`,pg:`nicepay`,messenger:`kakao`}:t===`JP`?{locale:`ja`,currency:`JPY`,pg:`stripe`,messenger:`line`}:{locale:`en`,currency:`USD`,pg:`stripe`,messenger:`web-share`}}W.get(`/api/locale`,async e=>{let t=(e.req.header(`CF-IPCountry`)??e.req.header(`cf-ipcountry`)??e.req.raw.cf?.country??``).toUpperCase(),n=e.req.header(`X-Session-Token`)||``;if(n){let r=await e.env.LOOKBOOK_DB.prepare(`SELECT u.locale, u.country, u.currency FROM user_sessions s
       JOIN users u ON u.id = s.user_id
       WHERE s.token = ? AND s.expires_at > datetime('now')`).bind(n).first();if(r?.locale)return e.json({country:r.country||t,locale:r.locale,currency:r.currency||`USD`,pg:r.locale===`ko`?`nicepay`:`stripe`,messenger:r.locale===`ko`?`kakao`:r.locale===`ja`?`line`:`web-share`})}let r=Et(t);return e.json({country:t,...r})}),W.put(`/api/user/locale`,async e=>{let t=e.env.LOOKBOOK_DB,n=e.req.header(`X-Session-Token`)||``;if(!n)return e.json({success:!1,message:`로그인이 필요합니다.`},401);let r=await t.prepare(`SELECT user_id FROM user_sessions WHERE token = ? AND expires_at > datetime('now')`).bind(n).first();if(!r)return e.json({success:!1,message:`세션이 만료되었습니다.`},401);let i=await e.req.json(),a=[`ko`,`en`,`ja`].includes(i?.locale)?i.locale:null;if(!a)return e.json({success:!1,message:`지원하지 않는 언어입니다.`},400);let o=typeof i?.country==`string`?i.country.slice(0,8):null,s=typeof i?.currency==`string`?i.currency.slice(0,8):null;return await t.prepare(`UPDATE users SET locale = ?, country = ?, currency = ?, updated_at = datetime('now') WHERE id = ?`).bind(a,o,s,r.user_id).run(),e.json({success:!0})}),W.get(`/api/config/kakao-js-key`,e=>e.json({key:e.env.KAKAO_JS_KEY||``}));function Dt(){let e=new Uint8Array(32);return crypto.getRandomValues(e),Array.from(e).map(e=>e.toString(16).padStart(2,`0`)).join(``)}function Ot(){return`u_`+Dt().substring(0,16)}async function kt(e){let t=Dt().substring(0,16),n=new TextEncoder().encode(t+e),r=await crypto.subtle.digest(`SHA-256`,n);return`${t}:${Array.from(new Uint8Array(r)).map(e=>e.toString(16).padStart(2,`0`)).join(``)}`}async function At(e,t){let[n,r]=t.split(`:`),i=new TextEncoder().encode(n+e),a=await crypto.subtle.digest(`SHA-256`,i);return Array.from(new Uint8Array(a)).map(e=>e.toString(16).padStart(2,`0`)).join(``)===r}async function jt(e,t){if(!t)return null;let n=new Date().toISOString();return await e.prepare(`
    SELECT u.id, u.name, u.email, u.role, u.status, u.credits, u.avatar_url, u.provider, u.referrer
    FROM user_sessions s JOIN users u ON s.user_id = u.id
    WHERE s.token = ? AND s.expires_at > ? AND u.status = 'active'
  `).bind(t,n).first()||null}async function Mt(e,t){let n=Dt(),r=new Date(Date.now()+720*60*60*1e3).toISOString();return await e.prepare(`DELETE FROM user_sessions WHERE user_id = ? AND token NOT IN (
...
<head>
  ...
  <link rel="icon" type="image/svg+xml" href="/static/favicon.svg?v=${Ue}" />
  <link rel="preconnect" href="https://fonts.googleapis.com" />
  <link href="https://fonts.googleapis.com/css2?family=Pretendard:wght@300;400;500;600;700;800&display=swap" rel="stylesheet" />
  <link href="https://cdn.jsdelivr.net/npm/@fortawesome/fontawesome-free@6.4.0/css/all.min.css" rel="stylesheet" />
  <link href="/static/style.css?v=${Ue}" rel="stylesheet" />
  ${We(i)}
  <!-- app.js는 head에 defer — body 인라인 script보다 항상 먼저 파싱·실행됨 -->
  <script src="/static/app.js?v=${Ue}" defer><\/script>
  ${n}
</head>
<body>
${t}
...
`;return e.text(t,200,{"Content-Type":`text/markdown; charset=utf-8`})});var $t=[{q:`EZlook은 어떤 서비스인가요?`,a:`의류 이미지 한 장을 업로드하면 AI가 온모델 피팅컷과 룩북 세트를 자동으로 생성해주는 AI 패션 이미지 생성 플랫폼입니다. 촬영 스튜디오나 모델 섭외 없이 몇 번의 클릭만으로 전문적인 착용샷을 만들 수 있습니다.`},{q:`이미지 생성에 비용이 드나요?`,a:`이미지 생성 자체는 무료입니다. 마음에 드는 결과물을 실제 파일로 다운로드할 때만 장당 90크레딧이 차감됩니다.`},{q:`무료로 체험할 수 있나요?`,a:`네, 신용카드 등록 없이 회원가입만 하면 무료 크레딧이 바로 지급되어 AI 룩북 제작을 체험해볼 수 있습니다.`},{q:`영상도 만들 수 있나요?`,a:`네, 생성된 피팅컷 이미지를 기반으로 모델이 자연스럽게 포즈를 취하는 7초 분량의 세로형(9:16) 영상을 만들 수 있습니다.`},{q:`결과물은 얼마나 빨리 나오나요?`,a:`평균 30초, 최대 90초 이내에 고품질 온모델 피팅컷 이미지가 생성됩니다.`},{q:`크레딧은 어떻게 충전하나요?`,a:`월 정액 없이 필요한 만큼만 충전하는 방식입니다. 스타터(₩20,000 · 1,000크레딧)부터 베스트 밸류(₩60,000 · 4,000크레딧)까지 선택할 수 있습니다.`}],en=()=>{let e={"@type":`Organization`,name:`EZlook`,alternateName:`벌거벗은호랑이`,url:K,logo:`${K}/static/favicon.svg`,telephone:`070-4581-8166`,address:{"@type":`PostalAddress`,streetAddress:`무심서로 377-3`,addressLocality:`청주시 서원구`,addressRegion:`충청북도`,addressCountry:`KR`}};return[{"@context":`https://schema.org`,"@type":`WebSite`,name:`EZlook`,url:K,publisher:e},{"@context":`https://schema.org`,"@type":`Service`,name:`EZlook AI 패션 룩북 생성 서비스`,serviceType:`AI 패션 이미지/영상 생성`,description:`옷 사진 한 장을 업로드하면 AI 모델이 착용한 온모델 피팅컷과 룩북 세트, 홍보 영상을 자동으로 생성하는 서비스`,provider:e,areaServed:`KR`,offers:[{"@type":`Offer`,name:`스타터`,price:`20000`,priceCurrency:`KRW`,description:`1,000 크레딧 · 이미지 최대 11장`},{"@type":`Offer`,name:`인기 충전권`,price:`40000`,priceCurrency:`KRW`,description:`2,300 크레딧 · 이미지 최대 25장`},{"@type":`Offer`,name:`베스트 밸류`,price:`60000`,priceCurrency:`KRW`,description:`4,000 크레딧 · 이미지 최대 44장`}]},{"@context":`https://schema.org`,"@type":`FAQPage`,mainEntity:$t.map(e=>({"@type":`Question`,name:e.q,acceptedAnswer:{"@type":`Answer`,text:e.a}}))}].map(e=>`<script type="application/ld+json">${JSON.stringify(e)}<\/script>`).join(`
  ...
  `,t,`옷 사진 한 장으로 AI 온모델 피팅컷과 룩북 세트를 무료로 자동 생성하세요. 촬영 스튜디오나 모델 섭외 없이 평균 30초 만에 완성됩니다.`,e.env.GA4_MEASUREMENT_ID))}),W.get(`/dashboard`,e=>e.html(Qt(`내 프로필`,`
  <div class="toast-container" id="toastContainer"></div>
  ...
  <\/script>
  `,``,Zt,e.env.GA4_MEASUREMENT_ID))),W.get(`/credits`,e=>e.html(Qt(`크레딧 상세`,`
  <div class="toast-container" id="toastContainer"></div>
  ...
  <\/script>
  `,``,`내 크레딧 충전/사용 내역과 잔여 크레딧을 확인하세요.`,e.env.GA4_MEASUREMENT_ID))),W.get(`/generator`,e=>e.html(Qt(`무료 AI 룩북 생성기`,`
  <div class="toast-container" id="toastContainer"></div>
...
</html>`));var tn=new Se,nn=Object.assign({"/src/index.tsx":W}),rn=!1;for(let[,e]of Object.entries(nn))e&&(tn.all(`*`,t=>{let n;try{n=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,n)}),tn.notFound(t=>{let n;try{n=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,n)}),rn=!0);if(!rn)throw Error(`Can't import modules from ['/src/index.ts','/src/index.tsx','/app/server.ts']`);export{tn as default};