...
`,mt=`CASE WHEN email LIKE '%@%' AND email NOT LIKE '%**%' THEN email ELSE crawled_email END`,ht=`COALESCE(NULLIF(bzmnNm, ''), NULLIF(domain_clean, ''), '거래처')`;B.get(`/mail-batch/status`,async e=>{let t=e.env.LOOKBOOK_DB,n=(await t.prepare(`SELECT COUNT(*) AS n FROM biz_leads WHERE ${pt}`).first())?.n||0,r=await t.prepare(`
  ...
  `).bind(t).all();if(!n.length)return e.json({success:!1,message:`해당 배치를 찾을 수 없습니다.`},404);let r=lt(n.map(e=>({name:e.name,email:e.email})));return new Response(r,{status:200,headers:{"Content-Type":`application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`,"Content-Disposition":`attachment; filename="bizleads_mail_batch_${t}.xlsx"`,"X-Batch-Id":String(t),"X-Batch-Count":String(n.length)}})});var gt=`mt5q0fo6`,_t=e=>e?`
...
<\/script>
     ...
     WHERE s.token = ? AND s.expires_at > datetime('now')`).bind(r).first();if(!i)return e.json({error:`세션이 만료되었습니다.`},401);try{let{packageId:r,currency:a}=await e.req.json(),o=Cn[r];if(!o)return e.json({error:`잘못된 패키지입니다.`},400);let s=String(a||`USD`).toUpperCase();if(s!==`USD`&&s!==`JPY`)return e.json({error:`지원하지 않는 통화입니다.`},400);let l=s===`USD`?o.usdCents:o.jpyAmount,u=Sn[i.referrer]||0,d=u>0?Math.round(l*(1-u)):l,f=`lookbook-${i.user_id.slice(0,6)}-${Date.now()}-${Math.random().toString(36).slice(2,6).toUpperCase()}`;await n.prepare(`INSERT INTO payment_logs (user_id, order_id, amount, credits, status, pg_provider, currency)
       ...
       VALUES (?, ?, 1, ?, ?, '9:16', ?, 'video', datetime('now', '+14 days'), ?)`).bind(r.user_id,d,a||`패션 모델`,o||`스튜디오`,p,JSON.stringify([i])).run(),e.json({success:!0,jobId:d,creditsRemaining:f})}catch(t){return console.error(`video/start error:`,t),e.json({success:!1,message:t.message},500)}}),V.post(`/api/ghostcut/video/start`,async e=>{try{let t=e.env.LOOKBOOK_DB,n=e.req.header(`X-Session-Token`)||``;if(!n)return e.json({error:`로그인이 필요합니다.`,code:`UNAUTHORIZED`},401);let r=await t.prepare(`SELECT s.user_id, u.credits, u.name FROM user_sessions s
       JOIN users u ON u.id = s.user_id
       WHERE s.token = ? AND s.expires_at > datetime('now')`).bind(n).first();if(!r)return e.json({error:`세션이 만료되었습니다.`,code:`UNAUTHORIZED`},401);let{imageUrl:i,categoryLabel:a}=await e.req.json();if(!i)return e.json({error:`imageUrl 필수`},400);let o=Mn;if(r.credits<o)return e.json({error:`크레딧이 부족합니다. (보유: ${r.credits}크레딧, 필요: ${o}크레딧)`,code:`INSUFFICIENT_CREDITS`,available:r.credits,required:o},402);let s=await fetch(`${H}/api/v1/model/generateVideo`,{method:`POST`,headers:Kt(e.env.ATLAS_API_KEY),body:JSON.stringify({model:`bytedance/seedance-2.5/image-to-video`,prompt:`The garment shown in the image keeps the ghost-mannequin (invisible-body) effect and stays in the exact same position and framing throughout — it does NOT walk, float away, spin, or change position. The fabric gently sways and ripples as if moved by a soft, natural breeze: subtle, realistic movement in the sleeves, hem, and any loose or flowing fabric areas, with natural cloth physics — gentle, not exaggerated or stormy. The garment's color, pattern, print, texture, and design remain exactly as shown in the first frame, completely unchanged throughout the entire video. The background remains pure solid white (#FFFFFF), completely flat and even, with absolutely no shadow, no gradient, no vignette, and no change at any point — identical to the first frame from start to finish. Do NOT introduce any person, human body, face, hands, model, or visible mannequin form at any point in the video. The camera stays essentially static — no zoom, no dolly, no pan, no orbital movement — only the fabric itself moves. Smooth, realistic motion at regular playback speed — absolutely no slow motion, no slow-mo effect, no frame-rate ramping. Add soft, tasteful ambient background music suited for a clean e-commerce product showcase — no vocals, no jarring sound effects.`,image:i,duration:7,resolution:`1080p-esr`,ratio:`adaptive`,output_format:`mp4`,generate_audio:!0,watermark:!1})}),l=await s.json(),u=l?.data?.id||l?.id||null;if(!s.ok||!u)return console.error(`ghostcut/video/start Atlas 요청 실패:`,l),e.json({success:!1,message:l?.msg||l?.message||`영상 생성 요청 실패`},502);let d=r.credits-o;await t.prepare(`UPDATE users SET credits = ?, updated_at = datetime('now') WHERE id = ?`).bind(d,r.user_id).run(),await t.prepare(`INSERT INTO credit_logs (user_id, type, amount, balance, reason, ref_id)
       VALUES (?, 'deduct', ?, ?, 'video_generation', ?)`).bind(r.user_id,-600,d,u).run();let f=((await t.prepare(`SELECT COALESCE(MAX(seq_no), 0) AS last_seq FROM generation_logs WHERE user_id = ?`).bind(r.user_id).first())?.last_seq||0)+1;return await t.prepare(`INSERT INTO generation_logs (user_id, job_id, image_count, model_name, bg_name, ratio, seq_no, kind, expires_at, image_urls)
       VALUES (?, ?, 1, ?, ?, '1:1', ?, 'video', datetime('now', '+14 days'), ?)`).bind(r.user_id,u,`고스트컷·${a||``}`,`화이트 배경`,f,JSON.stringify([i])).run(),e.json({success:!0,jobId:u,creditsRemaining:d})}catch(t){return console.error(`ghostcut/video/start error:`,t),e.json({success:!1,message:t.message},500)}}),V.get(`/api/video/:jobId/status`,async e=>{let t=e.req.param(`jobId`),n=e.env.LOOKBOOK_DB;try{let r=null;try{r=await n.prepare(`SELECT status, video_url, created_at FROM generation_logs WHERE job_id = ?`).bind(t).first()}catch{r=null}if(r?.status===`completed`&&r.video_url)return e.json({status:`completed`,progress:100,videoUrl:r.video_url});if(r?.status===`failed`)return e.json({status:`failed`,progress:100,error:`영상 생성에 실패했습니다. (크레딧은 차감되지 않았습니다)`});let i=r?.created_at?Date.now()-new Date(r.created_at.replace(` `,`T`)+`Z`).getTime()>900*1e3:!1,a=async(r,a)=>{if(i){let r=await Nn(n,t);return e.json({status:`failed`,progress:100,error:a,...r})}return e.json({status:`processing`,progress:r})},o;try{o=await fetch(`${H}/api/v1/model/prediction/${t}`,{headers:{Authorization:`Bearer ${e.env.ATLAS_API_KEY}`}}).then(e=>e.json())}catch(e){return console.error(`Atlas 상태 조회 일시 실패:`,e),await a(50,`영상 생성 응답 시간이 초과되었습니다. (크레딧은 차감되지 않았습니다)`)}let s=String(o.data?.status??o.status??``).toLowerCase();if(!new Set([`completed`,`succeeded`,`success`,`failed`,`failure`,`timeout`,`canceled`,`cancelled`,`error`]).has(s))return await a(50,`영상 생성 응답 시간이 초과되었습니다. (크레딧은 차감되지 않았습니다)`);if(s!==`completed`&&s!==`succeeded`&&s!==`success`){console.error(`video status 실패:`,s,o);let r=await Nn(n,t);return e.json({status:`failed`,progress:100,error:`영상 생성에 실패했습니다. (크레딧은 차감되지 않았습니다)`,...r})}let l=o.data?.outputs??o.data?.output??o.data?.video??o.data?.videos??o.output??null,u=Array.isArray(l)?l.find(e=>typeof e==`string`&&e.startsWith(`http`))||null:typeof l==`string`&&l.startsWith(`http`)?l:null;return u?(await n.prepare(`UPDATE generation_logs SET video_url = ?, status = 'completed' WHERE job_id = ?`).bind(u,t).run(),e.json({status:`completed`,progress:100,videoUrl:u})):(console.error(`video 완료 응답이지만 URL 파싱 실패:`,JSON.stringify(o).slice(0,500)),await a(90,`영상 URL을 찾을 수 없습니다. (크레딧은 차감되지 않았습니다)`))}catch(t){return console.error(`video status poll error (재시도 예정):`,t),e.json({status:`processing`,progress:50})}}),V.get(`/api/admin/debug/stuck-videos`,W,async e=>{let t=e.env.LOOKBOOK_DB;try{let n=await t.prepare(`SELECT id, user_id, job_id, status, video_url, created_at,
       ...
       LIMIT 50`).all();return e.json({success:!0,jobs:n.results||[]})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.get(`/api/admin/debug/atlas-job/:jobId`,W,async e=>{let t=e.req.param(`jobId`),n=e.env.LOOKBOOK_DB;try{let r=await n.prepare(`SELECT id, job_id, kind, status, video_url, created_at FROM generation_logs WHERE job_id = ?`).bind(t).first(),i=await fetch(`${H}/api/v1/model/prediction/${t}`,{headers:{Authorization:`Bearer ${e.env.ATLAS_API_KEY}`}}),a=await i.text(),o=null;try{o=JSON.parse(a)}catch{}return e.json({dbRow:r,atlasHttpStatus:i.status,atlasRaw:o??a})}catch(t){return e.json({success:!1,message:t.message},500)}});async function Fn(e){try{let t=await e.prepare(`SELECT value FROM app_settings WHERE key = 'admin_prompt_config'`).first();if(t?.value){let e=JSON.parse(t.value);return{enabled:typeof e.enabled==`boolean`?e.enabled:G.enabled,prefix:typeof e.prefix==`string`?e.prefix:G.prefix,suffix:typeof e.suffix==`string`?e.suffix:G.suffix,styleGuide:typeof e.styleGuide==`string`?e.styleGuide:G.styleGuide,technicalSpec:typeof e.technicalSpec==`string`?e.technicalSpec:G.technicalSpec,updatedAt:e.updatedAt||G.updatedAt}}}catch(e){console.warn(`d1GetPromptConfig fallback to memory:`,e)}return G}async function In(e,t){await e.prepare(`INSERT INTO app_settings (key, value, updated_at) VALUES ('admin_prompt_config', ?, datetime('now'))
     ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = excluded.updated_at`).bind(JSON.stringify(t)).run()}V.get(`/api/admin/prompt`,W,async e=>{try{let t=e.env.LOOKBOOK_DB,n=await Fn(t);return e.json({success:!0,config:n})}catch{return e.json({success:!0,config:G})}}),V.put(`/api/admin/prompt`,W,async e=>{try{let t=e.env.LOOKBOOK_DB,n=await e.req.json(),r=await Fn(t),i={enabled:typeof n.enabled==`boolean`?n.enabled:r.enabled,prefix:typeof n.prefix==`string`?n.prefix:r.prefix,suffix:typeof n.suffix==`string`?n.suffix:r.suffix,styleGuide:typeof n.styleGuide==`string`?n.styleGuide:r.styleGuide,technicalSpec:typeof n.technicalSpec==`string`?n.technicalSpec:r.technicalSpec,updatedAt:new Date().toISOString()};return await In(t,i),G=i,console.log(`Admin prompt config saved to D1:`,i.updatedAt),e.json({success:!0,config:i})}catch(t){return e.json({success:!1,message:t.message},400)}}),V.post(`/api/admin/auth`,async e=>{let t=await e.req.json(),n=e.env.ADMIN_PASSWORD;return n?t.password===n?e.json({success:!0}):e.json({success:!1,message:`비밀번호가 올바르지 않습니다.`},401):e.json({success:!1,message:`서버 설정 오류: ADMIN_PASSWORD 환경변수가 설정되지 않았습니다.`},500)});var Ln=`의류 이미지 하나로 AI 온모델 피팅컷과 룩북 세트를 자동 생성하세요.`,Rn=(e,t,n=``,r=Ln,i)=>`<!DOCTYPE html>
...
</html>`;V.get(`/share/:jobId/:idx`,async e=>{let t=e.env.LOOKBOOK_DB,n=e.req.param(`jobId`),r=parseInt(e.req.param(`idx`)||`0`,10),i=t=>{let i=`${$(e)}/share/${n}/${r}`,a=``;if(t.state===`ok`){let e=t.sourceTabs||[];a=`
...
`;return e.text(t,200,{"Content-Type":`text/markdown; charset=utf-8`})});var zn=[{q:`EZlook은 어떤 서비스인가요?`,a:`의류 이미지 한 장을 업로드하면 AI가 온모델 피팅컷과 룩북 세트를 자동으로 생성해주는 AI 패션 이미지 생성 플랫폼입니다. 촬영 스튜디오나 모델 섭외 없이 몇 번의 클릭만으로 전문적인 착용샷을 만들 수 있습니다.`},{q:`이미지 생성에 비용이 드나요?`,a:`이미지 생성 자체는 무료입니다. 마음에 드는 결과물을 실제 파일로 다운로드할 때만 장당 90크레딧이 차감됩니다.`},{q:`무료로 체험할 수 있나요?`,a:`네, 신용카드 등록 없이 회원가입만 하면 무료 크레딧이 바로 지급되어 AI 룩북 제작을 체험해볼 수 있습니다.`},{q:`영상도 만들 수 있나요?`,a:`네, 생성된 피팅컷 이미지를 기반으로 모델이 자연스럽게 포즈를 취하는 7초 분량의 세로형(9:16) 영상을 만들 수 있습니다.`},{q:`결과물은 얼마나 빨리 나오나요?`,a:`평균 30초, 최대 90초 이내에 고품질 온모델 피팅컷 이미지가 생성됩니다.`},{q:`크레딧은 어떻게 충전하나요?`,a:`월 정액 없이 필요한 만큼만 충전하는 방식입니다. 스타터(₩20,000 · 1,000크레딧)부터 베스트 밸류(₩60,000 · 4,000크레딧)까지 선택할 수 있습니다.`}],Bn=()=>{let e={"@type":`Organization`,name:`EZlook`,alternateName:`벌거벗은호랑이`,url:U,logo:`${U}/static/favicon.svg`,telephone:`070-4581-8166`,address:{"@type":`PostalAddress`,streetAddress:`무심서로 377-3`,addressLocality:`청주시 서원구`,addressRegion:`충청북도`,addressCountry:`KR`}};return[{"@context":`https://schema.org`,"@type":`WebSite`,name:`EZlook`,url:U,publisher:e},{"@context":`https://schema.org`,"@type":`Service`,name:`EZlook AI 패션 룩북 생성 서비스`,serviceType:`AI 패션 이미지/영상 생성`,description:`옷 사진 한 장을 업로드하면 AI 모델이 착용한 온모델 피팅컷과 룩북 세트, 홍보 영상을 자동으로 생성하는 서비스`,provider:e,areaServed:`KR`,offers:[{"@type":`Offer`,name:`스타터`,price:`20000`,priceCurrency:`KRW`,description:`1,000 크레딧 · 이미지 최대 11장`},{"@type":`Offer`,name:`인기 충전권`,price:`40000`,priceCurrency:`KRW`,description:`2,300 크레딧 · 이미지 최대 25장`},{"@type":`Offer`,name:`베스트 밸류`,price:`60000`,priceCurrency:`KRW`,description:`4,000 크레딧 · 이미지 최대 44장`}]},{"@context":`https://schema.org`,"@type":`FAQPage`,mainEntity:zn.map(e=>({"@type":`Question`,name:e.q,acceptedAnswer:{"@type":`Answer`,text:e.a}}))}].map(e=>`<script type="application/ld+json">${JSON.stringify(e)}<\/script>`).join(`
  ...
  `,t,`옷 사진 한 장으로 AI 온모델 피팅컷과 룩북 세트를 무료로 자동 생성하세요. 촬영 스튜디오나 모델 섭외 없이 평균 30초 만에 완성됩니다.`,e.env.GA4_MEASUREMENT_ID))}),V.get(`/dashboard`,e=>e.html(Rn(`내 프로필`,`
  <div class="toast-container" id="toastContainer"></div>
  ...
  <\/script>
  `,``,Ln,e.env.GA4_MEASUREMENT_ID))),V.get(`/credits`,e=>e.html(Rn(`크레딧 상세`,`
  <div class="toast-container" id="toastContainer"></div>
  ...
  `,i,n,e.env.GA4_MEASUREMENT_ID))};V.get(`/generator`,e=>Vn(e,`model`)),V.get(`/ghostcut`,e=>Vn(e,`ghostcut`)),V.get(`/admin02`,e=>e.html(`<!DOCTYPE html>
<html lang="ko">
<head>
  <meta charset="UTF-8"/>
  <meta name="viewport" content="width=device-width,initial-scale=1.0"/>
...
</html>`));var Hn=new Oe,Un=Object.assign({"/src/index.tsx":V}),Wn=!1;for(let[,e]of Object.entries(Un))e&&(Hn.all(`*`,t=>{let n;try{n=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,n)}),Hn.notFound(t=>{let n;try{n=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,n)}),Wn=!0);if(!Wn)throw Error(`Can't import modules from ['/src/index.ts','/src/index.tsx','/app/server.ts']`);export{Hn as default};