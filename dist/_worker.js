...
`),200,{"Content-Type":`text/csv; charset=utf-8`,"Content-Disposition":`attachment; filename="brand_leads_export.csv"`})});var Be=`msxy9mme`,Ve=e=>e?`
...
Return ONLY the JSON, no explanation.`}]}]}})})).json())?.data?.id;if(!t)return e.json({success:!1,message:`라벨링 작업 시작 실패`},500);for(let n=0;n<15;n++){await new Promise(e=>setTimeout(e,1e3));let n=await fetch(`${i}/api/v1/model/prediction/${t}`,{headers:{Authorization:`Bearer ${r}`}}).then(e=>e.json()),a=n?.data?.status;if(a===`completed`||a===`succeeded`){let t=n?.data?.outputs?.[0]||n?.data?.output||``;try{let n=t.match(/\{[\s\S]*\}/),r=JSON.parse(n?.[0]||t);return e.json({success:!0,labels:{category:r.category||`스튜디오`,mood:r.mood||`미니멀`,name_ko:r.name_ko||``}})}catch{return e.json({success:!1,message:`응답 파싱 실패`,raw:t})}}if(a===`failed`||a===`error`)break}return e.json({success:!1,message:`라벨링 타임아웃`})}return e.json({success:!1,message:`type은 model 또는 background 이어야 합니다.`},400)}catch(t){return e.json({success:!1,message:t.message},500)}}),q.patch(`/api/admin/models/:id/labels`,Y,async e=>{try{let t=e.req.param(`id`),{gender:n,age:r,mood:i}=await e.req.json(),a=e.env?.LOOKBOOK_DB;return a?(await a.prepare(`UPDATE custom_models SET gender=?, age=?, mood=? WHERE id=?`).bind(n||`미분류`,r||`미분류`,i||`미분류`,t).run(),e.json({success:!0})):e.json({success:!1,message:`D1 없음`},500)}catch(t){return e.json({success:!1,message:t.message},500)}}),q.get(`/api/admin/models`,Y,async e=>{let t=e.env?.LOOKBOOK_KV,n=e.env?.LOOKBOOK_DB;if(t){let n=await Z(t);return e.json({success:!0,models:n})}if(n){let t=await Je(n);return e.json({success:!0,models:t})}let r=Q.map(e=>({id:e.id,name:e.name,desc:e.desc,createdAt:e.createdAt}));return e.json({success:!0,models:r})}),q.delete(`/api/admin/models/:id`,Y,async e=>{let t=e.req.param(`id`),n=e.env?.LOOKBOOK_KV,r=e.env?.LOOKBOOK_DB;if(n){let r=await Z(n),i=r.filter(e=>e.id!==t);return await Ue(n,i),await n.delete(`model_img:${t}`),e.json({success:r.length>i.length})}if(r){let n=await Xe(r,t);return e.json({success:n})}let i=Q.length;return Q=Q.filter(e=>e.id!==t),e.json({success:Q.length<i})}),q.get(`/api/proxy/custom-model/:id`,async e=>{let t=e.req.param(`id`),n=e.env?.LOOKBOOK_KV,r=e.env?.LOOKBOOK_DB,i=null;if(i=n?await n.get(`model_img:${t}`):r?await Ze(r,t):Q.find(e=>e.id===t)?.imageBase64||null,!i)return e.notFound();let[a,o]=i.split(`,`),s=(a.match(/data:([^;]+)/)||[])[1]||`image/jpeg`,l=atob(o),u=new Uint8Array(l.length);for(let e=0;e<l.length;e++)u[e]=l.charCodeAt(e);return new Response(u.buffer,{headers:{"Content-Type":s,"Cache-Control":`public, max-age=3600`}})}),q.post(`/api/admin/backgrounds`,Y,async e=>{try{let t=await e.req.json(),n=Array.isArray(t)?t:[t];if(n.length===0)return e.json({success:!1,message:`업로드할 항목이 없습니다.`},400);let r=e.env?.LOOKBOOK_KV,i=e.env?.LOOKBOOK_DB,a=[];if(r){let e=await We(r);for(let t of n){let{name:n,bgDesc:i,category:o,imageBase64:s}=t;if(!n||!s)continue;let l=await Ke(r),u={id:l,name:n,bgDesc:i||n,category:o||`커스텀`,createdAt:new Date().toISOString()};e.push(u),await r.put(`bg_img:${l}`,s),a.push(u)}await Ge(r,e)}else if(i)a=await $e(i,n);else for(let e of n){let{name:t,bgDesc:n,category:r,imageBase64:i}=e;if(!t||!i)continue;let o=String(nt++),s={id:o,name:t,bgDesc:n||t,category:r||`커스텀`,imageBase64:i,createdAt:new Date().toISOString()};$.push(s),a.push({id:o,name:t,bgDesc:s.bgDesc,category:s.category,createdAt:s.createdAt})}return e.json({success:!0,backgrounds:a,count:a.length})}catch(t){return e.json({success:!1,message:t.message},500)}}),q.get(`/api/admin/backgrounds`,Y,async e=>{let t=e.env?.LOOKBOOK_KV,n=e.env?.LOOKBOOK_DB;if(t){let n=await We(t);return e.json({success:!0,backgrounds:n})}if(n){let t=await Qe(n);return e.json({success:!0,backgrounds:t})}let r=$.map(e=>({id:e.id,name:e.name,bgDesc:e.bgDesc,category:e.category,createdAt:e.createdAt}));return e.json({success:!0,backgrounds:r})}),q.delete(`/api/admin/backgrounds/:id`,Y,async e=>{let t=e.req.param(`id`),n=e.env?.LOOKBOOK_KV,r=e.env?.LOOKBOOK_DB;if(n){let r=await We(n),i=r.filter(e=>e.id!==t);return await Ge(n,i),await n.delete(`bg_img:${t}`),e.json({success:r.length>i.length})}if(r){let n=await et(r,t);return e.json({success:n})}let i=$.length;return $=$.filter(e=>e.id!==t),e.json({success:$.length<i})}),q.get(`/api/proxy/custom-bg/:id`,async e=>{let t=e.req.param(`id`),n=e.env?.LOOKBOOK_KV,r=e.env?.LOOKBOOK_DB,i=null;if(i=n?await n.get(`bg_img:${t}`):r?await tt(r,t):$.find(e=>e.id===t)?.imageBase64||null,!i)return e.notFound();let[a,o]=i.split(`,`),s=(a.match(/data:([^;]+)/)||[])[1]||`image/jpeg`,l=atob(o),u=new Uint8Array(l.length);for(let e=0;e<l.length;e++)u[e]=l.charCodeAt(e);return new Response(u.buffer,{headers:{"Content-Type":s,"Cache-Control":`public, max-age=3600`}})});var rt=e=>({Authorization:`Bearer ${e}`,"Content-Type":`application/json`});q.get(`/api/presets/models`,async e=>{let t=e.env?.LOOKBOOK_KV,n=e.env?.LOOKBOOK_DB,r;r=t?await Z(t):n?await Je(n):Q.map(e=>({id:e.id,name:e.name,desc:e.desc,createdAt:e.createdAt}));let i=r.map(e=>({id:Number(e.id),name:e.name,gender:e.gender||`미분류`,age:e.age||`미분류`,mood:e.mood||`미분류`,body:`-`,skin:`-`,desc:e.desc,unsplashId:null,isCustom:!0,customId:e.id}));return e.json({models:i})}),q.get(`/api/presets/backgrounds`,async e=>{let t=e.env?.LOOKBOOK_KV,n=e.env?.LOOKBOOK_DB,r;r=t?await We(t):n?await Qe(n):$.map(e=>({id:e.id,name:e.name,bgDesc:e.bgDesc,category:e.category,createdAt:e.createdAt}));let i=r.map(e=>({id:Number(e.id),name:e.name,category:e.category,mood:`-`,bgDesc:e.bgDesc,unsplashId:null,isCustom:!0,customId:e.id}));return e.json({backgrounds:i})}),q.get(`/api/proxy/model-image/:id`,e=>e.notFound()),q.get(`/api/proxy/bg-image/:id`,e=>e.notFound()),q.get(`/api/proxy/gen-image`,async e=>{let t=e.req.query(`url`),n=e.req.query(`download`)===`1`;if(!t)return e.json({error:`Missing url param`},400);try{if(!new URL(t).protocol.startsWith(`https`))return e.json({error:`Only HTTPS URLs allowed`},400);let r=await fetch(t,{headers:{"User-Agent":`Mozilla/5.0 (compatible; LookbookAI/1.0)`}});if(!r.ok)return e.json({error:`Upstream error: ${r.status}`},r.status);let i=await r.arrayBuffer(),a=r.headers.get(`Content-Type`)||`image/jpeg`,o=`lookbook_ai_${Date.now()}.jpg`,s={"Content-Type":a,"Cache-Control":`public, max-age=3600`,"Access-Control-Allow-Origin":`*`};return n&&(s[`Content-Disposition`]=`attachment; filename="${o}"`),new Response(i,{headers:s})}catch(t){return console.error(`Gen image proxy error:`,t),e.json({error:t.message},500)}});var it=[{id:`p1`,name:`2024 S/S 룩북`,status:`done`,images:8,created:`2024-03-15`,thumb_color:`#FF6B9D`},{id:`p2`,name:`캐주얼 티셔츠 컷`,status:`done`,images:4,created:`2024-03-12`,thumb_color:`#6C47FF`},{id:`p3`,name:`데님 라인 촬영`,status:`processing`,images:0,created:`2024-03-10`,thumb_color:`#3B82F6`},{id:`p4`,name:`원피스 봄 컬렉션`,status:`draft`,images:0,created:`2024-03-08`,thumb_color:`#00D4AA`}];q.get(`/api/projects`,e=>e.json({projects:it})),q.get(`/api/locale`,e=>{let t=(e.req.header(`CF-IPCountry`)??e.req.header(`cf-ipcountry`)??e.req.raw.cf?.country??``).toUpperCase(),n=`en`;return t===`KR`?n=`ko`:t===`JP`&&(n=`ja`),e.json({locale:n,country:t})});function at(){let e=new Uint8Array(32);return crypto.getRandomValues(e),Array.from(e).map(e=>e.toString(16).padStart(2,`0`)).join(``)}function ot(){return`u_`+at().substring(0,16)}async function st(e){let t=at().substring(0,16),n=new TextEncoder().encode(t+e),r=await crypto.subtle.digest(`SHA-256`,n);return`${t}:${Array.from(new Uint8Array(r)).map(e=>e.toString(16).padStart(2,`0`)).join(``)}`}async function ct(e,t){let[n,r]=t.split(`:`),i=new TextEncoder().encode(n+e),a=await crypto.subtle.digest(`SHA-256`,i);return Array.from(new Uint8Array(a)).map(e=>e.toString(16).padStart(2,`0`)).join(``)===r}async function lt(e,t){if(!t)return null;let n=new Date().toISOString();return await e.prepare(`
...
<\/script>
</body></html>`)}catch(e){return console.error(`kakao callback error:`,e),o(e.message||`로그인 오류`)}}),q.get(`/api/auth/google`,e=>{let t=ft(e),n=e.req.query(`mode`)||`popup`,r=`${t}/api/auth/google/callback`,i=e.env.GOOGLE_CLIENT_ID||``;if(!i)return n===`redirect`?e.redirect(`/?oauth_error=google_no_key`):e.html(`<script>window.opener?.postMessage({type:'oauth_error',provider:'google',error:'구글 클라이언트 ID가 설정되지 않았습니다.'},'*');window.close();<\/script>`);let a=new URLSearchParams({client_id:i,redirect_uri:r,response_type:`code`,scope:`openid email profile`,access_type:`offline`,prompt:`select_account`,state:n});return e.redirect(`https://accounts.google.com/o/oauth2/v2/auth?${a}`)}),q.get(`/api/auth/google/callback`,async e=>{let t=e.env.LOOKBOOK_DB,n=ft(e),r=e.req.query(`code`),i=e.req.query(`error`),a=e.req.query(`state`)||`popup`;function o(t){return a===`redirect`?e.redirect(`/?oauth_error=${encodeURIComponent(t)}`):e.html(`<script>window.opener?.postMessage({type:'oauth_error',provider:'google',error:'${t}'},'*');window.close();<\/script>`)}if(i||!r)return o(i||`cancelled`);try{let i=`${n}/api/auth/google/callback`,o=await(await fetch(`https://oauth2.googleapis.com/token`,{method:`POST`,headers:{"Content-Type":`application/x-www-form-urlencoded`},body:new URLSearchParams({grant_type:`authorization_code`,code:r,client_id:e.env.GOOGLE_CLIENT_ID||``,client_secret:e.env.GOOGLE_CLIENT_SECRET||``,redirect_uri:i})})).json();if(!o.access_token)throw Error(`구글 토큰 발급 실패`);let s=await(await fetch(`https://www.googleapis.com/oauth2/v2/userinfo`,{headers:{Authorization:`Bearer ${o.access_token}`}})).json(),l=s.id,u=s.email,d=s.name||`구글 사용자`,f=s.picture||null,p=!1,m=await t.prepare(`SELECT * FROM users WHERE provider = 'google' AND provider_id = ?`).bind(l).first();if(!m)if(m=await t.prepare(`SELECT * FROM users WHERE email = ?`).bind(u).first(),m)await t.prepare(`UPDATE users SET provider_id = ?, avatar_url = ? WHERE id = ?`).bind(l,f,m.id).run();else{let e=ot();await t.prepare(`INSERT INTO users (id, email, name, provider, provider_id, avatar_url, status, credits, role) VALUES (?, ?, ?, 'google', ?, ?, 'active', 1000, 'user')`).bind(e,u,d,l,f).run(),m=await t.prepare(`SELECT * FROM users WHERE id = ?`).bind(e).first(),p=!0}if(!m||m.status!==`active`)throw Error(`계정이 정지 상태입니다.`);await t.prepare(`UPDATE users SET last_login_at = datetime('now') WHERE id = ?`).bind(m.id).run();let h=await ut(t,m.id),g=JSON.stringify(dt(m));return a===`redirect`?e.html(`<!DOCTYPE html>
<html lang="ko">
<head><meta charset="UTF-8"><title>로그인 성공</title>
<meta name="viewport" content="width=device-width,initial-scale=1">
</head>
...
<body>
...
</html>`)),q.get(`/_home_old`,e=>e.redirect(`/generator`,302)),q.get(`/`,e=>{let t=e.req.header(`host`)||``;return t.includes(`studiob.aifashion.co.kr`)?e.redirect(`/generator`,302):t===`www.aifashion.co.kr`||t===`aifashion.co.kr`?e.redirect(`https://studiob.aifashion.co.kr`,302):e.html(Ct(`홈`,`
  <!-- Toast Container -->
  ...
  `,e.env.GA4_MEASUREMENT_ID))}),q.get(`/dashboard`,e=>e.html(Ct(`내 프로필`,`
  <div class="toast-container" id="toastContainer"></div>
  ...
  <\/script>
  `,e.env.GA4_MEASUREMENT_ID))),q.get(`/generator`,e=>e.html(Ct(`AI 룩북 생성`,`
  <div class="toast-container" id="toastContainer"></div>
  ...
  `,e.env.GA4_MEASUREMENT_ID))),q.get(`/admin02`,e=>e.html(`<!DOCTYPE html>
<html lang="ko">
<head>
  <meta charset="UTF-8"/>
  <meta name="viewport" content="width=device-width,initial-scale=1.0"/>
