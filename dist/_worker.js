...
`,mt=`CASE WHEN email LIKE '%@%' AND email NOT LIKE '%**%' THEN email ELSE crawled_email END`,ht=`COALESCE(NULLIF(bzmnNm, ''), NULLIF(domain_clean, ''), '거래처')`;B.get(`/mail-batch/status`,async e=>{let t=e.env.LOOKBOOK_DB,n=(await t.prepare(`SELECT COUNT(*) AS n FROM biz_leads WHERE ${pt}`).first())?.n||0,r=await t.prepare(`
  ...
  `).bind(t).all();if(!n.length)return e.json({success:!1,message:`해당 배치를 찾을 수 없습니다.`},404);let r=lt(n.map(e=>({name:e.name,email:e.email})));return new Response(r,{status:200,headers:{"Content-Type":`application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`,"Content-Disposition":`attachment; filename="bizleads_mail_batch_${t}.xlsx"`,"X-Batch-Id":String(t),"X-Batch-Count":String(n.length)}})});var gt=`mtptbjik`;function _t(e){return e.env.TECHPACK_MENU_VISIBLE===`true`}var vt=e=>e?`
...
<\/script>
       ...
       VALUES (?, ?, ?, ?, 'pending')`).bind(r.user_id,u,s,a.credits).run(),e.json({success:!0,orderId:u,amount:s,credits:a.credits,orderName:a.label,customerName:r.name,customerEmail:r.email,clientId:e.env.NICEPAY_CLIENT_ID||``})}catch(t){return e.json({success:!1,message:t.message},500)}});async function kn(e){let t=new TextEncoder().encode(e),n=await crypto.subtle.digest(`SHA-256`,t);return Array.from(new Uint8Array(n)).map(e=>e.toString(16).padStart(2,`0`)).join(``)}async function An(e,t){let n=await crypto.subtle.importKey(`raw`,new TextEncoder().encode(e),{name:`HMAC`,hash:`SHA-256`},!1,[`sign`]),r=await crypto.subtle.sign(`HMAC`,n,new TextEncoder().encode(t));return Array.from(new Uint8Array(r)).map(e=>e.toString(16).padStart(2,`0`)).join(``)}V.post(`/api/stripe/checkout`,async e=>{let t=e.env.STRIPE_SECRET_KEY;if(!t)return e.json({success:!1,message:`서버 설정 오류: STRIPE_SECRET_KEY 미설정`},500);let n=e.env.LOOKBOOK_DB,r=e.req.header(`X-Session-Token`)||``;if(!r)return e.json({error:`로그인이 필요합니다.`},401);let i=await n.prepare(`SELECT s.user_id, u.email, u.referrer FROM user_sessions s
     JOIN users u ON u.id = s.user_id
