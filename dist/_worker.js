...
`),200,{"Content-Type":`text/csv; charset=utf-8`,"Content-Disposition":`attachment; filename="brand_leads_export.csv"`})});var U=new Ce;U.use(`/*`,async(e,t)=>{let n=e.req.header(`X-Admin-Password`),r=e.env.ADMIN_PASSWORD;if(!r)return e.json({success:!1,message:`서버 설정 오류: ADMIN_PASSWORD 환경변수가 설정되지 않았습니다.`},500);if(n!==r)return e.json({success:!1,message:`인증 실패`},401);await t()});function We(e){if(!e)return``;let t=e.replace(/[^0-9]/g,``);if(t.length<7)return e;if(t.startsWith(`02`)){if(t.length===9)return t.replace(/(\d{2})(\d{3})(\d{4})/,`$1-$2-$3`);if(t.length===10)return t.replace(/(\d{2})(\d{4})(\d{4})/,`$1-$2-$3`)}else if(t.startsWith(`0`)){if(t.length===9)return t.replace(/(\d{3})(\d{2})(\d{4})/,`$1-$2-$3`);if(t.length===10)return t.replace(/(\d{3})(\d{3})(\d{4})/,`$1-$2-$3`);if(t.length===11)return t.replace(/(\d{3})(\d{4})(\d{4})/,`$1-$2-$3`)}if(!t.startsWith(`0`)&&t.length>=7){let e=`0`+t;if(e.startsWith(`02`)){if(e.length===9)return e.replace(/(\d{2})(\d{3})(\d{4})/,`$1-$2-$3`);if(e.length===10)return e.replace(/(\d{2})(\d{4})(\d{4})/,`$1-$2-$3`)}else if(e.length===10)return e.replace(/(\d{3})(\d{3})(\d{4})/,`$1-$2-$3`);else if(e.length===11)return e.replace(/(\d{3})(\d{4})(\d{4})/,`$1-$2-$3`)}return e}function Ge(e){if(!e)return``;let t=e.replace(/[^0-9]/g,``);return t.length===10?`${t.slice(0,3)}-${t.slice(3,5)}-${t.slice(5)}`:e}U.get(`/stats`,async e=>{let t=e.env.LOOKBOOK_DB,n=(await t.prepare(`SELECT COUNT(*) AS n FROM biz_leads`).first())?.n||0,{results:r}=await t.prepare(`SELECT region, COUNT(*) AS n FROM biz_leads WHERE region != 'N/A' AND region IS NOT NULL GROUP BY region ORDER BY n DESC`).all(),i=await t.prepare(`
  ...
  `).bind(...o,r,i).all();return e.json({success:!0,total:s,page:n,limit:r,rows:l})}),U.get(`/detail/:id`,async e=>{let t=e.req.param(`id`),n=await e.env.LOOKBOOK_DB.prepare(`SELECT * FROM biz_leads WHERE id = ?`).bind(t).first();return n?e.json({success:!0,...n}):e.json({success:!1,message:`찾을 수 없습니다.`},404)}),U.get(`/download.csv`,async e=>{let t=e.env.LOOKBOOK_DB,{where:n,params:r}=Ke(e),{results:i}=await t.prepare(`
...
`);return e.text(n,200,{"Content-Type":`text/csv; charset=utf-8`,"Content-Disposition":`attachment; filename="fashion_biz_instagram.csv"`})});var qe=`mt42uhfv`,Je=e=>e?`
