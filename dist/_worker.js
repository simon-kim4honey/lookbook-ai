...
`,mt=`CASE WHEN email LIKE '%@%' AND email NOT LIKE '%**%' THEN email ELSE crawled_email END`,ht=`COALESCE(NULLIF(bzmnNm, ''), NULLIF(domain_clean, ''), '거래처')`;B.get(`/mail-batch/status`,async e=>{let t=e.env.LOOKBOOK_DB,n=(await t.prepare(`SELECT COUNT(*) AS n FROM biz_leads WHERE ${pt}`).first())?.n||0,r=await t.prepare(`
  ...
  `).bind(t).all();if(!n.length)return e.json({success:!1,message:`해당 배치를 찾을 수 없습니다.`},404);let r=lt(n.map(e=>({name:e.name,email:e.email})));return new Response(r,{status:200,headers:{"Content-Type":`application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`,"Content-Disposition":`attachment; filename="bizleads_mail_batch_${t}.xlsx"`,"X-Batch-Id":String(t),"X-Batch-Count":String(n.length)}})});var gt=`mt5xfhsy`,_t=e=>e?`
...
<\/script>
     ...
     WHERE s.token = ? AND s.expires_at > datetime('now')`).bind(r).first();if(!i)return e.json({error:`세션이 만료되었습니다.`},401);try{let{packageId:r,currency:a}=await e.req.json(),o=Cn[r];if(!o)return e.json({error:`잘못된 패키지입니다.`},400);let s=String(a||`USD`).toUpperCase();if(s!==`USD`&&s!==`JPY`)return e.json({error:`지원하지 않는 통화입니다.`},400);let l=s===`USD`?o.usdCents:o.jpyAmount,u=xn[i.referrer]||0,d=u>0?Math.round(l*(1-u)):l,f=`lookbook-${i.user_id.slice(0,6)}-${Date.now()}-${Math.random().toString(36).slice(2,6).toUpperCase()}`;await n.prepare(`INSERT INTO payment_logs (user_id, order_id, amount, credits, status, pg_provider, currency)
       ...
       JOIN users u ON u.id = s.user_id
       WHERE s.token = ? AND s.expires_at > datetime('now')`).bind(n).first();if(!r)return e.json({error:`세션이 만료되었습니다.`,code:`UNAUTHORIZED`},401);let i=await e.req.json(),{imageUrl:a,categoryLabel:o}=i,s=Math.max(1,Math.min(4,parseInt(i.count,10)||1));if(!a)return e.json({error:`imageUrl 필수`},400);let l=Fn[s];if(r.credits<l)return e.json({error:`크레딧이 부족합니다. (보유: ${r.credits}크레딧, 필요: ${l}크레딧)`,code:`INSUFFICIENT_CREDITS`,available:r.credits,required:l},402);let u=e=>[`PRODUCT DETAIL CLOSE-UP PHOTOGRAPHY — take the exact garment shown in the source image and create an extreme close-up macro shot of ONE specific design detail area of it.`,`Source image = the ONLY reference for the garment's design, color, pattern, print, texture, fabric, stitching, and every visual detail. Do NOT redesign, alter, invent, or change ANY detail of the garment — this is a crop/zoom of the exact same real garment, not a new interpretation.`,`CRITICAL — DO NOT ADD ANY WEAR, AGING, OR DISTRESSING THAT IS NOT ALREADY IN THE SOURCE IMAGE: if an edge, hem, seam, or surface is clean and smooth in the source image, it MUST remain exactly that clean and smooth in this close-up — do NOT add fraying, raw-edge unraveling, rips, tears, whiskering, faded/worn patches, scuffs, or any "distressed"/"vintage"/"aged" look that is not already present in the source. If the source already shows some existing wear or distressing, reproduce it EXACTLY as-is — do not add MORE fraying or tears beyond what the source shows, and do not invent new damage. This is a magnified crop of the real product photo, not a stylized reinterpretation — every stitch, seam, edge finish, and surface texture must match the source image, only zoomed in.`,e,`Frame it as a tight macro/close-up product shot — fill most of the frame with the chosen detail area, sharp focus, professional e-commerce detail-shot style (the kind of close-up photo online stores use to show fabric texture, stitching quality, buttons, zippers, or prints up close).`,`Background MUST remain pure solid white (#FFFFFF), completely flat and shadowless — NO drop shadow, NO contact shadow, NO reflection, NO gradient, NO vignette anywhere in the frame, exactly like the source image's background.`,`ABSOLUTE RULES: DO NOT insert, overlay, embed, or render ANY text, letters, numbers, words, logos, watermarks, brand marks, or typographic elements ANYWHERE in the image (unless an existing brand logo/print is already part of the garment's real design in the source image — reproduce that exactly, do not add new ones). NO visible human body, face, hands, mannequin form, or hanger anywhere in the output. DO NOT change the garment's color, pattern, print, texture, fabric, or any design element from the source image — this must look like a real macro photograph of the exact same product.`,`Ultra-photorealistic, 8K quality, sharp macro focus, professional e-commerce product detail photography.`].join(` `),d=Array.from({length:s},(t,n)=>fetch(`${H}/api/v1/model/generateImage`,{method:`POST`,headers:$(e.env.ATLAS_API_KEY),body:JSON.stringify({model:`google/nano-banana-2/edit`,prompt:u(Rn[n%Rn.length]),aspect_ratio:`1:1`,resolution:`2k`,thinking_level:`high`,output_format:`jpeg`,images:[a]})}).then(e=>e.json())),f=await Promise.all(d),p=f.filter(e=>e.code===200&&e.data?.id).map(e=>e.data.id);if(p.length===0){let t=f[0];return console.error(`[GhostCut Detail] Atlas 요청 전체 실패:`,t),e.json({success:!1,message:t?.msg||t?.message||`디테일컷 생성 요청 실패`},502)}let m=p.length,h=Fn[m],g=p.join(`,`),_=r.credits-h;await t.prepare(`UPDATE users SET credits = ?, updated_at = datetime('now') WHERE id = ?`).bind(_,r.user_id).run(),await t.prepare(`INSERT INTO credit_logs (user_id, type, amount, balance, reason, ref_id)
