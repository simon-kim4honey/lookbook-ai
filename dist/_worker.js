...
`)){let t=e.trim();if(!t||t.startsWith(`#`))continue;let[n,...r]=t.split(`:`),i=n.trim().toLowerCase(),l=r.join(`:`).trim();i===`user-agent`?o=l===`*`:o&&i===`disallow`&&l&&a.startsWith(l)&&(s=!0)}return!s}catch{return!0}}I.post(`/collect/scrape`,async e=>{let{platformId:t}=await e.req.json(),n=await e.env.LOOKBOOK_DB.prepare(`SELECT * FROM lead_platforms WHERE id = ?`).bind(t).first();if(!n)return e.json({success:!1,message:`플랫폼을 찾을 수 없습니다.`},404);if(!n.is_scrapable)return e.json({success:!1,message:`이 플랫폼은 is_scrapable=0 입니다. 이용약관/robots.txt를 확인하고 셀렉터를 검증한 뒤 관리자가 직접 활성화해야 합니다.`},400);if(!n.directory_url||!n.list_selector)return e.json({success:!1,message:`directory_url과 list_selector를 먼저 등록하세요.`},400);if(!await He(n.directory_url))return e.json({success:!1,message:`robots.txt에 의해 해당 경로 크롤링이 금지되어 있습니다. 수집을 중단합니다.`},403);let r=(await e.env.LOOKBOOK_DB.prepare(`INSERT INTO collection_jobs (platform_id, method, status) VALUES (?, 'scrape', 'running')`).bind(n.id).run()).meta.last_row_id;try{let t=await fetch(n.directory_url,{headers:{"User-Agent":`LookbookAI-LeadBot/1.0 (+brand outreach lead collection; contact: admin of lookbook-ai)`}});if(!t.ok)throw Error(`대상 페이지 응답 오류: HTTP ${t.status}`);let i=await t.text(),a=[];await new HTMLRewriter().on(n.list_selector,{element(e){a.push({name:``,href:e.getAttribute(`href`)||``}),e._captureIndex=a.length-1},text(e){a.length>0&&(a[a.length-1].name+=e.text)}}).transform(new Response(i)).text();let o=[];if(n.category_selector){let e=``;await new HTMLRewriter().on(`${n.list_selector} ${n.category_selector}`,{text(t){e+=t.text,t.lastInTextNode&&(o.push(e.trim()),e=``)}}).transform(new Response(i)).text()}let s=new URL(n.directory_url),l=0;for(let t=0;t<Math.min(a.length,300);t++){let r=a[t].name.trim();if(!r)continue;let i=a[t].href;try{i=new URL(i,s).toString()}catch{}let u=o[t]||null;(await e.env.LOOKBOOK_DB.prepare(`INSERT OR IGNORE INTO brands (platform_id, country, name, category, brand_url, source, status)
    ...
    WHERE 1=1
  `,l=[];t&&(s+=` AND b.country = ?`,l.push(t)),n&&(s+=` AND b.platform_id = ?`,l.push(n)),r&&(s+=` AND b.status = ?`,l.push(r)),i&&(s+=` AND b.name LIKE ?`,l.push(`%${i}%`)),s+=` ORDER BY COALESCE(a.priority_score, -1) DESC, b.collected_at DESC LIMIT ? OFFSET ?`,l.push(a,o);let{results:u}=await e.env.LOOKBOOK_DB.prepare(s).bind(...l).all();return e.json({success:!0,brands:u})}),I.get(`/brands/:id`,async e=>{let t=e.req.param(`id`),n=await e.env.LOOKBOOK_DB.prepare(`SELECT * FROM brands WHERE id = ?`).bind(t).first();if(!n)return e.json({success:!1,message:`브랜드를 찾을 수 없습니다.`},404);let{results:r}=await e.env.LOOKBOOK_DB.prepare(`SELECT * FROM brand_analysis WHERE brand_id = ? ORDER BY analyzed_at DESC`).bind(t).all(),{results:i}=await e.env.LOOKBOOK_DB.prepare(`SELECT * FROM outreach_drafts WHERE brand_id = ? ORDER BY created_at DESC`).bind(t).all();return e.json({success:!0,brand:n,analyses:r,drafts:i})}),I.patch(`/brands/:id`,async e=>{let t=e.req.param(`id`),n=await e.req.json(),r=[],i=[];for(let e of[`status`,`category`,`brand_url`,`contact_email`])n[e]!==void 0&&(r.push(`${e} = ?`),i.push(n[e]));return r.length===0?e.json({success:!1,message:`변경할 필드가 없습니다.`},400):(i.push(t),await e.env.LOOKBOOK_DB.prepare(`UPDATE brands SET ${r.join(`, `)} WHERE id = ?`).bind(...i).run(),e.json({success:!0}))});var Ue={street:[`스트릿`,`스트리트`,`street`,`ストリート`],casual:[`캐주얼`,`casual`,`カジュアル`],minimal:[`미니멀`,`minimal`,`ミニマル`,`contemporary`,`컨템포러리`],luxury:[`럭셔리`,`luxury`,`디자이너`,`designer`,`ラグジュアリー`],golf:[`골프`,`golf`,`ゴルフ`],outdoor:[`아웃도어`,`outdoor`,`アウトドア`],formal:[`포멀`,`수트`,`suit`,`formal`,`オフィス`,`フォーマル`],vintage:[`빈티지`,`vintage`,`ヴィンテージ`,`古着`],kids:[`키즈`,`아동`,`kids`,`baby`,`キッズ`],lingerie:[`언더웨어`,`속옷`,`lingerie`,`ランジェリー`],shoes:[`슈즈`,`신발`,`shoes`,`靴`],bag:[`가방`,`bag`,`バッグ`]},We={luxury:[`럭셔리`,`luxury`,`디자이너`,`designer`,`ラグジュアリー`],premium:[`프리미엄`,`premium`,`컨템포러리`,`contemporary`],budget:[`보세`,`균일가`,`budget`,`저가`,`プチプラ`]};function Ge(e,t){let n=`${e} ${t||``}`.toLowerCase(),r=[];for(let[e,t]of Object.entries(Ue))t.some(e=>n.includes(e.toLowerCase()))&&r.push(e);let i=`mid`;for(let[e,t]of Object.entries(We))if(t.some(e=>n.includes(e.toLowerCase()))){i=e;break}let a=55;(r.includes(`street`)||r.includes(`casual`)||r.includes(`minimal`))&&(a+=15),(r.includes(`shoes`)||r.includes(`bag`))&&(a-=10),a=Math.max(0,Math.min(100,a));let o=Math.round(a*.7+({budget:40,mid:60,premium:80,luxury:70}[i]||60)*.3),s=`${e}은(는) ${t||`패션`} 카테고리${r.length?` (${r.join(`, `)} 스타일)`:``}로 추정되며, 가격대는 ${i} 수준으로 분류됩니다. 시즌 룩북 갱신 빈도가 높을 가능성이 있어 AI 룩북 자동 생성 제안 대상으로 적합합니다.`;return{style_tags:r,target_customer:r.includes(`kids`)?`키즈/부모`:`20~30대 일반 소비자(추정)`,price_tier:i,lookbook_need_score:a,priority_score:Math.max(0,Math.min(100,o)),ai_summary:s,analysis_method:`heuristic`}}async function Ke(e,t,n,r){let i=`You are a B2B market analyst for an AI fashion lookbook generation SaaS. Given a fashion brand name and category, return ONLY strict JSON (no markdown fences) with this shape:
...
Brand name: ${t}
Category: ${n||`unknown`}
Country: ${r}`,a=await fetch(`https://api.anthropic.com/v1/messages`,{method:`POST`,headers:{"Content-Type":`application/json`,"x-api-key":e.ANTHROPIC_API_KEY,"anthropic-version":`2023-06-01`,"User-Agent":`EZlook-ContentDigest/1.0 (+https://www.aifashion.co.kr)`},body:JSON.stringify({model:`claude-sonnet-5`,thinking:{type:`disabled`},max_tokens:2e3,messages:[{role:`user`,content:i}]})});if(!a.ok)throw Error(`Claude API 오류: HTTP ${a.status}`);let o=(((await a.json())?.content||[]).find(e=>e.type===`text`)?.text||`{}`).replace(/^```json\s*|```$/g,``).trim(),s=JSON.parse(o);return{style_tags:s.style_tags||[],target_customer:s.target_customer||``,price_tier:s.price_tier||`mid`,lookbook_need_score:Math.max(0,Math.min(100,Number(s.lookbook_need_score)||0)),priority_score:Math.max(0,Math.min(100,Number(s.priority_score)||0)),ai_summary:s.ai_summary||``,analysis_method:`claude`}}I.post(`/analyze`,async e=>{let{brandIds:t,platformId:n,onlyUnanalyzed:r}=await e.req.json(),i=[];if(t&&t.length)i=t;else if(n){let t=`SELECT id FROM brands WHERE platform_id = ?`;r&&(t+=` AND status = 'new'`);let{results:a}=await e.env.LOOKBOOK_DB.prepare(t).bind(n).all();i=a.map(e=>e.id)}else return e.json({success:!1,message:`brandIds 또는 platformId가 필요합니다.`},400);i=i.slice(0,100);let a=!!e.env.ANTHROPIC_API_KEY,o=0,s=[];for(let t of i){let n=await e.env.LOOKBOOK_DB.prepare(`SELECT * FROM brands WHERE id = ?`).bind(t).first();if(!n)continue;let r;try{r=a?await Ke(e.env,n.name,n.category,n.country):Ge(n.name,n.category)}catch(e){r=Ge(n.name,n.category),s.push(`brand ${t}: ${String(e?.message||e)}`)}await e.env.LOOKBOOK_DB.prepare(`INSERT INTO brand_analysis (brand_id, style_tags, target_customer, price_tier, lookbook_need_score, priority_score, ai_summary, analysis_method)
       VALUES (?,?,?,?,?,?,?,?)`).bind(t,JSON.stringify(r.style_tags),r.target_customer,r.price_tier,r.lookbook_need_score,r.priority_score,r.ai_summary,r.analysis_method).run(),await e.env.LOOKBOOK_DB.prepare(`UPDATE brands SET status = 'analyzed' WHERE id = ? AND status = 'new'`).bind(t).run(),o++}return e.json({success:!0,analyzed:o,method:a?`claude`:`heuristic`,errors:s.slice(0,10)})});var qe={ko:{valueProps:[`촬영 스튜디오·모델 섭외 없이 몇 분 만에 시즌 룩북 이미지를 생성`,`다양한 AI 모델·배경 프리셋으로 여러 컨셉을 동시에 테스트`,`크레딧제 과금으로 소량 생산도 부담 없이 시작 가능`]},en:{valueProps:[`Generate seasonal lookbook imagery in minutes — no studio or model booking required`,`Test multiple concepts at once with a range of AI model & background presets`,`Pay-as-you-go credits, so even small runs are affordable`]},ja:{valueProps:[`スタジオ撮影やモデル手配なしで、数分でシーズンルックブック画像を生成`,`多彩なAIモデル・背景プリセットで複数のコンセプトを同時にテスト`,`クレジット制課金で少量からでも気軽に開始可能`]}};function Je(e,t,n,r){let i=qe[e];return e===`ko`?{subject:`[제안] ${t}님을 위한 AI 룩북 자동 생성 서비스 소개`,body:`안녕하세요, ${t} 담당자님.\n\n저희는 AI로 패션 룩북 이미지를 자동 생성하는 서비스를 운영하고 있습니다. 공개된 브랜드 정보를 바탕으로 ${t}님과 잘 맞을 것 같아 연락드립니다.\n\n- ${i.valueProps.join(`
...
`),200,{"Content-Type":`text/csv; charset=utf-8`,"Content-Disposition":`attachment; filename="brand_leads_export.csv"`})});var Xe=(()=>{let e=new Uint32Array(256);for(let t=0;t<256;t++){let n=t;for(let e=0;e<8;e++)n=n&1?3988292384^n>>>1:n>>>1;e[t]=n>>>0}return e})();function Ze(e){let t=4294967295;for(let n=0;n<e.length;n++)t=Xe[(t^e[n])&255]^t>>>8;return(t^4294967295)>>>0}function Qe(e){return String(e??``).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&apos;`)}var $e=0,et=33;function L(e){let t=new Uint8Array(2);return t[0]=e&255,t[1]=e>>8&255,t}function R(e){let t=new Uint8Array(4);return t[0]=e&255,t[1]=e>>8&255,t[2]=e>>16&255,t[3]=e>>>24&255,t}function tt(...e){let t=e.reduce((e,t)=>e+t.length,0),n=new Uint8Array(t),r=0;for(let t of e)n.set(t,r),r+=t.length;return n}function nt(e){let t=new TextEncoder,n=[],r=[],i=0;for(let a of e){let e=t.encode(a.name),o=t.encode(a.content),s=Ze(o),l=tt(R(67324752),L(20),L(0),L(0),L($e),L(et),R(s),R(o.length),R(o.length),L(e.length),L(0),e);n.push(l,o);let u=tt(R(33639248),L(20),L(20),L(0),L(0),L($e),L(et),R(s),R(o.length),R(o.length),L(e.length),L(0),L(0),L(0),L(0),R(0),R(i),e);r.push(u),i+=l.length+o.length}let a=i,o=tt(...r),s=tt(R(101010256),L(0),L(0),L(e.length),L(e.length),R(o.length),R(a),L(0));return tt(...n,o,s)}var rt=`<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/><Default Extension="xml" ContentType="application/xml"/><Override PartName="/xl/workbook.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml"/><Override PartName="/xl/worksheets/sheet1.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml"/><Override PartName="/xl/styles.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.styles+xml"/></Types>`,it=`<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="xl/workbook.xml"/></Relationships>`,at=`<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet1.xml"/><Relationship Id="rId2" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles" Target="styles.xml"/></Relationships>`,ot=`<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="Sheet1" sheetId="1" r:id="rId1"/></sheets></workbook>`,st=`<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><fonts count="1"><font><sz val="11"/><name val="Calibri"/></font></fonts><fills count="1"><fill><patternFill patternType="none"/></fill></fills><borders count="1"><border><left/><right/><top/><bottom/><diagonal/></border></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs><cellXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/></cellXfs></styleSheet>`;function ct(e){let t=``;for(e+=1;e>0;){let n=(e-1)%26;t=String.fromCharCode(65+n)+t,e=Math.floor((e-1)/26)}return t}function lt(e,t,n){return`<c r="${ct(t)+e}" t="inlineStr"><is><t xml:space="preserve">${Qe(n)}</t></is></c>`}function ut(e){let t=[];t.push(`<row r="1">${lt(1,0,`이름`)}${lt(1,1,`이메일`)}</row>`),e.forEach((e,n)=>{let r=n+2;t.push(`<row r="${r}">${lt(r,0,e.name)}${lt(r,1,e.email)}</row>`)});let n=`<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><dimension ref="A1:B${e.length+1}"/><sheetData>${t.join(``)}</sheetData></worksheet>`;return nt([{name:`[Content_Types].xml`,content:rt},{name:`_rels/.rels`,content:it},{name:`xl/workbook.xml`,content:ot},{name:`xl/_rels/workbook.xml.rels`,content:at},{name:`xl/styles.xml`,content:st},{name:`xl/worksheets/sheet1.xml`,content:n}])}var z=new ke;z.use(`/*`,async(e,t)=>{let n=e.req.header(`X-Admin-Password`),r=e.env.ADMIN_PASSWORD;if(!r)return e.json({success:!1,message:`서버 설정 오류: ADMIN_PASSWORD 환경변수가 설정되지 않았습니다.`},500);if(n!==r)return e.json({success:!1,message:`인증 실패`},401);await t()});function dt(e){if(!e)return``;let t=e.replace(/[^0-9]/g,``);if(t.length<7)return e;if(t.startsWith(`02`)){if(t.length===9)return t.replace(/(\d{2})(\d{3})(\d{4})/,`$1-$2-$3`);if(t.length===10)return t.replace(/(\d{2})(\d{4})(\d{4})/,`$1-$2-$3`)}else if(t.startsWith(`0`)){if(t.length===9)return t.replace(/(\d{3})(\d{2})(\d{4})/,`$1-$2-$3`);if(t.length===10)return t.replace(/(\d{3})(\d{3})(\d{4})/,`$1-$2-$3`);if(t.length===11)return t.replace(/(\d{3})(\d{4})(\d{4})/,`$1-$2-$3`)}if(!t.startsWith(`0`)&&t.length>=7){let e=`0`+t;if(e.startsWith(`02`)){if(e.length===9)return e.replace(/(\d{2})(\d{3})(\d{4})/,`$1-$2-$3`);if(e.length===10)return e.replace(/(\d{2})(\d{4})(\d{4})/,`$1-$2-$3`)}else if(e.length===10)return e.replace(/(\d{3})(\d{3})(\d{4})/,`$1-$2-$3`);else if(e.length===11)return e.replace(/(\d{3})(\d{4})(\d{4})/,`$1-$2-$3`)}return e}function ft(e){if(!e)return``;let t=e.replace(/[^0-9]/g,``);return t.length===10?`${t.slice(0,3)}-${t.slice(3,5)}-${t.slice(5)}`:e}z.get(`/stats`,async e=>{let t=e.env.LOOKBOOK_DB,n=(await t.prepare(`SELECT COUNT(*) AS n FROM biz_leads`).first())?.n||0,{results:r}=await t.prepare(`SELECT region, COUNT(*) AS n FROM biz_leads WHERE region != 'N/A' AND region IS NOT NULL GROUP BY region ORDER BY n DESC`).all(),i=await t.prepare(`
    SELECT
  ...
  `).first(),o=await t.prepare(`
    ...
    FROM biz_leads WHERE is_valid = 1
  `).first();return e.json({success:!0,total:n,regions:r,validStats:i,contactStats:a,crawlStats:o})});function pt(e){let t=e.req.query(`q`)||``,n=e.req.query(`region`)||``,r=e.req.query(`status`)||``,i=e.req.query(`validOnly`)||``,a=e.req.query(`emailOnly`)||``,o=e.req.query(`telOnly`)||``,s=[],l=[];if(t){s.push(`(bzmnNm LIKE ? OR domain_clean LIKE ? OR domain LIKE ? OR addr LIKE ? OR ceo LIKE ? OR brno LIKE ? OR email LIKE ? OR crawled_email LIKE ?)`);let e=`%${t}%`;l.push(e,e,e,e,e,e,e,e)}return n&&(s.push(`region = ?`),l.push(n)),r&&(s.push(`status = ?`),l.push(r)),i===`1`&&s.push(`is_valid = 1`),a===`1`&&s.push(`email LIKE '%@%' AND email NOT LIKE '%**%'`),o===`1`&&s.push(`tel NOT LIKE '%개인정보%' AND tel != 'N/A' AND tel != '' AND tel IS NOT NULL`),{where:s.length?`WHERE `+s.join(` AND `):``,params:l}}z.get(`/list`,async e=>{let t=e.env.LOOKBOOK_DB,n=Math.max(1,parseInt(e.req.query(`page`)||`1`)),r=Math.min(200,parseInt(e.req.query(`limit`)||`100`)),i=(n-1)*r,{where:a,params:o}=pt(e),s=(await t.prepare(`SELECT COUNT(*) AS n FROM biz_leads ${a}`).bind(...o).first())?.n||0,{results:l}=await t.prepare(`
  ...
  `).bind(...o,r,i).all();return e.json({success:!0,total:s,page:n,limit:r,rows:l})}),z.get(`/detail/:id`,async e=>{let t=e.req.param(`id`),n=await e.env.LOOKBOOK_DB.prepare(`SELECT * FROM biz_leads WHERE id = ?`).bind(t).first();return n?e.json({success:!0,...n}):e.json({success:!1,message:`찾을 수 없습니다.`},404)}),z.get(`/download.csv`,async e=>{let t=e.env.LOOKBOOK_DB,{where:n,params:r}=pt(e),{results:i}=await t.prepare(`
...
`+i.map(e=>{let t=(e.email||``).includes(`**`)||!(e.email||``).includes(`@`),n=(e.tel||``).includes(`개인정보`);return[e.id,e.bzmnNm,e.codeName,e.status,e.region,e.ceo,ft(e.brno||``),e.declDate,e.domain_clean,a[String(e.is_valid)]??`미검증`,t?`[마스킹]`:e.email||``,n?`[마스킹]`:dt(e.tel||``),e.crawled_email||``,e.crawled_tel||``,e.crawled_kakao||``,e.crawled_insta||``,e.addr,e.server].map(e=>`"${String(e??``).replace(/"/g,`""`)}"`).join(`,`)}).join(`\r
`);return e.text(o,200,{"Content-Type":`text/csv; charset=utf-8`,"Content-Disposition":`attachment; filename="fashion_biz_leads.csv"`})}),z.get(`/export/kakao.csv`,async e=>{let{results:t}=await e.env.LOOKBOOK_DB.prepare(`
...
`);return e.text(n,200,{"Content-Type":`text/csv; charset=utf-8`,"Content-Disposition":`attachment; filename="fashion_biz_kakao.csv"`})}),z.get(`/export/insta.csv`,async e=>{let{results:t}=await e.env.LOOKBOOK_DB.prepare(`
...
`);return e.text(n,200,{"Content-Type":`text/csv; charset=utf-8`,"Content-Disposition":`attachment; filename="fashion_biz_instagram.csv"`})});var mt=`
  is_valid = 1
...
`,ht=`CASE WHEN email LIKE '%@%' AND email NOT LIKE '%**%' THEN email ELSE crawled_email END`,gt=`COALESCE(NULLIF(bzmnNm, ''), NULLIF(domain_clean, ''), '거래처')`;z.get(`/mail-batch/status`,async e=>{let t=e.env.LOOKBOOK_DB,n=(await t.prepare(`SELECT COUNT(*) AS n FROM biz_leads WHERE ${mt}`).first())?.n||0,r=await t.prepare(`
  ...
  `).first(),i=(await t.prepare(`SELECT COUNT(*) AS n FROM biz_leads WHERE mail_sent_at IS NOT NULL`).first())?.n||0;return e.json({success:!0,remaining:n,totalSent:i,lastBatch:r||null})}),z.post(`/mail-batch/next`,async e=>{let t=e.env.LOOKBOOK_DB,n=Math.min(200,Math.max(1,parseInt(e.req.query(`size`)||`200`))),{results:r}=await t.prepare(`
    SELECT id, ${gt} AS name, ${ht} AS email
    FROM biz_leads
    WHERE ${mt}
    ORDER BY CASE WHEN email LIKE '%@%' AND email NOT LIKE '%**%' THEN 0 ELSE 1 END, id
    LIMIT ?
  `).bind(n).all();if(!r.length)return e.json({success:!1,message:`더 이상 뽑을 수 있는 리드가 없습니다 (조건에 맞는 리드가 모두 소진됨).`},404);let i=(await t.prepare(`SELECT COALESCE(MAX(mail_batch), 0) + 1 AS n FROM biz_leads`).first())?.n||1,a=new Date().toISOString(),o=r.map(e=>e.id),s=[];for(let e=0;e<o.length;e+=90){let n=o.slice(e,e+90),r=n.map(()=>`?`).join(`,`);s.push(t.prepare(`UPDATE biz_leads SET mail_sent_at = ?, mail_batch = ? WHERE id IN (${r})`).bind(a,i,...n))}await t.batch(s);let l=ut(r.map(e=>({name:e.name,email:e.email})));return new Response(l,{status:200,headers:{"Content-Type":`application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`,"Content-Disposition":`attachment; filename="bizleads_mail_batch_${i}.xlsx"`,"X-Batch-Id":String(i),"X-Batch-Count":String(r.length)}})}),z.get(`/mail-batch/:batchId`,async e=>{let t=parseInt(e.req.param(`batchId`));if(!t)return e.json({success:!1,message:`잘못된 배치 번호`},400);let{results:n}=await e.env.LOOKBOOK_DB.prepare(`
    SELECT id, ${gt} AS name, ${ht} AS email
    FROM biz_leads WHERE mail_batch = ? ORDER BY id
  `).bind(t).all();if(!n.length)return e.json({success:!1,message:`해당 배치를 찾을 수 없습니다.`},404);let r=ut(n.map(e=>({name:e.name,email:e.email})));return new Response(r,{status:200,headers:{"Content-Type":`application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`,"Content-Disposition":`attachment; filename="bizleads_mail_batch_${t}.xlsx"`,"X-Batch-Id":String(t),"X-Batch-Count":String(n.length)}})});var _t=`simon-kim4honey/lookbook-ai`;async function vt(e,t,n){if(!t)return null;try{let r=await e.prepare(`SELECT github_issue_url FROM error_logs
       WHERE message = ? AND (route = ? OR (route IS NULL AND ? IS NULL))
         AND github_issue_url IS NOT NULL
         AND created_at > datetime('now', '-1 day')
       ORDER BY created_at DESC LIMIT 1`).bind(n.message,n.route,n.route).first();if(r?.github_issue_url)return r.github_issue_url;let i=`[자동 에러] ${n.route||`(경로 없음)`} — ${n.message}`.slice(0,250),a=[`**출처**: ${n.source}`,`**경로/URL**: ${n.route||`(없음)`}`,`**메시지**: ${n.message}`,n.stack?`\n**스택 트레이스**\n\`\`\`\n${n.stack.slice(0,3e3)}\n\`\`\``:``,`
---
lookbook-ai 서버가 자동 생성한 이슈입니다. 유지보수 세션이 이 이슈를 보고 진단 후 수정 PR을 연결하면 \`in-review\` 라벨이 붙습니다. 실제 배포 확인 후 이 이슈를 닫아주세요.`].join(`
`),o=await fetch(`https://api.github.com/repos/${_t}/issues`,{method:`POST`,headers:{Authorization:`Bearer ${t}`,Accept:`application/vnd.github+json`,"User-Agent":`lookbook-ai-error-bot`,"Content-Type":`application/json`},body:JSON.stringify({title:i,body:a,labels:[`auto-error`]})});return o.ok?(await o.json()).html_url||null:(console.error(`[findOrCreateGithubIssue] GitHub API 실패:`,o.status,await o.text()),null)}catch(e){return console.error(`[findOrCreateGithubIssue] 실패 (무시):`,e),null}}async function yt(e,t){try{let n=e.LOOKBOOK_DB,r=String(t.message||``).slice(0,2e3),i=t.stack?String(t.stack).slice(0,4e3):null,a=t.route?String(t.route).slice(0,300):null,o=t.extra?JSON.stringify(t.extra).slice(0,2e3):null,s=await vt(n,e.GITHUB_TOKEN,{message:r,route:a,stack:i,source:t.source});await n.prepare(`INSERT INTO error_logs (source, message, stack, route, extra, github_issue_url) VALUES (?, ?, ?, ?, ?, ?)`).bind(t.source,r,i,a,o,s).run()}catch(e){console.error(`[logError] 에러 기록 실패 (무시):`,e)}}var B=new ke;B.use(`/*`,async(e,t)=>{let n=e.req.header(`X-Admin-Password`),r=e.env.ADMIN_PASSWORD;if(!r)return e.json({success:!1,message:`서버 설정 오류: ADMIN_PASSWORD 환경변수가 설정되지 않았습니다.`},500);if(n!==r)return e.json({success:!1,message:`인증 실패`},401);await t()});var bt=[`패션 트렌드`,`패션 브랜드`,`K-패션`,`패션 이커머스`,`온라인 쇼핑몰 패션`,`패션 스타트업`,`AI 패션`,`패션 플랫폼`];function xt(e=new Date){let t=e.getMonth()+1,n=[`청바지`,`니트`,`가디건`];return t>=3&&t<=5?[...n,`원피스`,`블라우스`,`트렌치코트`,`스커트`,`자켓`]:t>=6&&t<=8?[...n,`원피스`,`반팔티`,`린넨셔츠`,`반바지`,`샌들`]:t>=9&&t<=11?[...n,`코트`,`맨투맨`,`자켓`,`부츠`,`원피스`]:[...n,`패딩`,`코트`,`목도리`,`맨투맨`,`부츠`]}function St(e){return e.replace(/&lt;/g,`<`).replace(/&gt;/g,`>`).replace(/&amp;/g,`&`).replace(/&quot;/g,`"`).replace(/&#39;/g,`'`).replace(/&apos;/g,`'`)}function Ct(e){return St(e.replace(/<\/?b>/g,``)).trim()}async function wt(e,t){try{let n=new Date().toISOString().slice(0,7);await e.LOOKBOOK_DB.prepare(`INSERT INTO naver_api_usage (year_month, api_type, call_count) VALUES (?, ?, 1)
       ON CONFLICT(year_month, api_type) DO UPDATE SET call_count = call_count + 1`).bind(n,t).run()}catch{}}async function Tt(e,t,n){if(!e.NAVER_CLIENT_ID||!e.NAVER_CLIENT_SECRET)return[];let r=`https://naverapihub.apigw.ntruss.com/search/v1/news?query=${encodeURIComponent(t)}&display=${n}&sort=date`;try{let t=await fetch(r,{headers:{"X-NCP-APIGW-API-KEY-ID":e.NAVER_CLIENT_ID,"X-NCP-APIGW-API-KEY":e.NAVER_CLIENT_SECRET},signal:AbortSignal.timeout(12e3)});if(await wt(e,`news`),!t.ok)return[];let i=await t.json();return(Array.isArray(i?.items)?i.items:[]).map(e=>{let t=e.originallink||e.link||``,n=``;try{n=t?new URL(t).hostname.replace(/^www\./,``):``}catch{}return{title:Ct(e.title||``),link:t,pubDate:e.pubDate||``,source:n}}).slice(0,n)}catch{return[]}}async function Et(e,t){let n=await Promise.all(bt.map(t=>Tt(e,t,12))),r=Date.now()-t*24*60*60*1e3,i=new Set,a=[];for(let e of n)for(let t of e){let e=t.link||t.title;if(!e||i.has(e))continue;let n=t.pubDate?Date.parse(t.pubDate):NaN;!isNaN(n)&&n<r||(i.add(e),a.push(t))}return a.slice(0,40)}function Dt(){let e=new Date,t=new Date(e.getTime()-504*60*60*1e3),n=e=>e.toISOString().slice(0,10);return{start:n(t),end:n(e)}}function Ot(e){if(!e||e.length<3)return null;let t=e[e.length-3].ratio,n=e[e.length-2].ratio;return t?Math.round((n-t)/t*1e3)/10:null}async function kt(e,t){if(!e.NAVER_CLIENT_ID||!e.NAVER_CLIENT_SECRET)return[];let{start:n,end:r}=Dt(),i=[];for(let e=0;e<t.length;e+=5)i.push(t.slice(e,e+5));let a=[];for(let t of i)try{let i=await fetch(`https://naverapihub.apigw.ntruss.com/search-trend/v1/search`,{method:`POST`,headers:{"X-NCP-APIGW-API-KEY-ID":e.NAVER_CLIENT_ID,"X-NCP-APIGW-API-KEY":e.NAVER_CLIENT_SECRET,"Content-Type":`application/json`},body:JSON.stringify({startDate:n,endDate:r,timeUnit:`week`,keywordGroups:t.map(e=>({groupName:e,keywords:[e]}))}),signal:AbortSignal.timeout(12e3)});if(await wt(e,`search_trend`),!i.ok)continue;let o=await i.json();for(let e of o?.results||[]){let t=Ot(e.data||[]);t!==null&&a.push({label:e.title,changePct:t})}}catch{}return a}var At={name:`패션의류`,code:`50000000`};async function jt(e,t,n){if(!e.NAVER_CLIENT_ID||!e.NAVER_CLIENT_SECRET)return[];let{start:r,end:i}=Dt(),a=[];for(let e=0;e<t.length;e+=5)a.push(t.slice(e,e+5));let o=[];for(let t of a)try{let a={startDate:r,endDate:i,timeUnit:`week`,category:At.code,keyword:t.map(e=>({name:e,param:[e]}))};n&&(a.gender=n);let s=await fetch(`https://naverapihub.apigw.ntruss.com/shopping/v1/category/keywords`,{method:`POST`,headers:{"X-NCP-APIGW-API-KEY-ID":e.NAVER_CLIENT_ID,"X-NCP-APIGW-API-KEY":e.NAVER_CLIENT_SECRET,"Content-Type":`application/json`},body:JSON.stringify(a),signal:AbortSignal.timeout(12e3)});if(await wt(e,`shopping_insight`),!s.ok)continue;let l=await s.json();for(let e of l?.results||[]){let t=Ot(e.data||[]);t!==null&&o.push({label:e.title,changePct:t})}}catch{}return o}async function Mt(e,t){let[n,r]=await Promise.all([jt(e,t,`f`),jt(e,t,`m`)]);if(!n.length||!r.length)return null;let i=e=>Math.round(e.reduce((e,t)=>e+t.changePct,0)/e.length*10)/10;return{female:i(n),male:i(r)}}var Nt=[`트렌드`,`브랜드`,`유통`,`시장`,`글로벌`];async function Pt(e,t){if(!e.ANTHROPIC_API_KEY)throw Error(`ANTHROPIC_API_KEY가 설정되지 않았습니다.`);if(!t.length)return{summary:`이번 주 수집된 기사가 없습니다.`,keywords:[],items:[]};let n=`당신은 국내 중소 패션 브랜드를 위한 패션 산업 뉴스 큐레이터입니다.
...
기사 목록:
${t.map((e,t)=>`${t+1}. [${e.source||`출처미상`}] ${e.title}`).join(`
`)}

원본 목록 순번(1-based)을 idx로 사용해서, 아래 JSON 형식으로만 응답하세요 (마크다운 코드펜스 없이, 내부/시스템 태그 없이):
{"overallSummary": string, "keywords": string[], "items": [{"idx": number, "category": string, "summary": string, "importance": number}]}`,r=null,i=``;for(let t=1;t<=3&&(r=await fetch(`https://api.anthropic.com/v1/messages`,{method:`POST`,headers:{"Content-Type":`application/json`,"x-api-key":e.ANTHROPIC_API_KEY,"anthropic-version":`2023-06-01`,"User-Agent":`EZlook-ContentDigest/1.0 (+https://www.aifashion.co.kr)`},body:JSON.stringify({model:`claude-sonnet-5`,thinking:{type:`disabled`},max_tokens:3e3,messages:[{role:`user`,content:n}]}),signal:AbortSignal.timeout(45e3)}),!(r.ok||(i=(await r.text()).slice(0,300),!(r.status===403||r.status===429||r.status>=500)||t===3)));t++)await new Promise(e=>setTimeout(e,t*800));if(!r||!r.ok)throw Error(`Claude API 오류: HTTP ${r?.status} — ${i}`);let a=await r.json();if(a?.stop_reason===`max_tokens`)throw Error(`Claude API 응답이 max_tokens 제한으로 중간에 잘렸습니다. max_tokens를 늘려야 합니다.`);let o=((a?.content||[]).find(e=>e.type===`text`)?.text||`{}`).replace(/^```json\s*|```$/g,``).trim(),s=JSON.parse(o),l=(s.items||[]).filter(e=>e.idx>=1&&e.idx<=t.length).map(e=>{let n=t[e.idx-1];return{category:Nt.includes(e.category)?e.category:`트렌드`,title:n.title,source:n.source||``,url:n.link,published_at:n.pubDate,summary:e.summary||``,importance:Math.max(1,Math.min(5,Number(e.importance)||3))}});return{summary:s.overallSummary||``,keywords:s.keywords||[],items:l}}function Ft(){let e=new Date;return`${e.getFullYear()}년 ${e.getMonth()+1}월 ${Math.ceil(e.getDate()/7)}주차`}function It(e,t,n){let r=e===`search_trend`?`검색 관심도`:`구매 클릭`,i=e===`search_trend`?`오르는 품목 위주로 콘텐츠·마케팅 노출을 늘려보세요.`:`클릭이 느는 품목은 재고·프로모션을 우선 챙길 타이밍입니다.`,a=(()=>{if(!n)return``;let e=n.female-n.male;return Math.abs(e)<5?``:` 성별로 보면 ${e>0?`여성`:`남성`} 고객의 클릭 증가가 더 두드러졌습니다 (여성 ${n.female>0?`+`:``}${n.female}% / 남성 ${n.male>0?`+`:``}${n.male}%).`})();if(!t.length)return`이번 주 ${e===`search_trend`?`검색어트렌드`:`쇼핑인사이트`} 데이터를 가져오지 못했습니다.`;let o=[...t].sort((e,t)=>t.changePct-e.changePct),s=o.filter(e=>e.changePct>0),l=o.filter(e=>e.changePct<0).slice(-2).reverse(),u=e=>`${e.label}(${e.changePct>0?`+`:``}${e.changePct}%)`,d=o[0],f=o[1],p=o.every(e=>Math.abs(e.changePct)<5),m=l.length===0&&s.length===o.length,h=s.length===0&&l.length>0,g;if(p)g=`이번 주 ${r}는 품목별로 큰 변화 없이 대체로 비슷한 수준을 유지했습니다 (전주 대비 ±5% 이내). ${i}`;else if(m)g=`이번 주는 ${s.slice(0,4).map(u).join(`, `)} 등 추적 품목 전반이 고르게 상승했습니다. ${i}`;else if(h)g=`이번 주는 ${l.slice(0,4).map(u).join(`, `)} 등 추적 품목 전반이 하락세입니다 — 계절 전환 등 일시적 요인일 수 있으니 다음 주 추이도 같이 확인해보세요.`;else if(d&&f&&d.changePct>0&&d.changePct>f.changePct*2&&d.changePct>=30){let e=s.slice(1,3).map(u).join(`, `);g=`이번 주는 단연 ${u(d)}입니다 — 다른 품목보다 확실히 눈에 띄는 상승폭입니다.${e?` (${e}도 함께 올랐습니다)`:``} ${i}`}else{let e=s.slice(0,3).map(u).join(`, `),t=l.map(u).join(`, `);g=e?`이번 주 ${r}는 ${e}이 가장 크게 올랐습니다.`:`이번 주 ${r}는 오른 품목이 없습니다.`,t&&(g+=` 반대로 ${t}는 하락세입니다.`),g+=` ${i}`}return g+a}async function Lt(e,t,n){if(!n.length)return``;try{let r=await e.prepare(`SELECT keywords FROM content_digests WHERE period = ? AND type = 'news' ORDER BY generated_at DESC LIMIT 1`).bind(t).first();if(!r)return``;let i=JSON.parse(r.keywords||`[]`);if(!i.length)return``;let a=[...n].sort((e,t)=>t.changePct-e.changePct)[0];if(!a||a.changePct<=0)return``;let o=i.find(e=>e.includes(a.label)||a.label.includes(e));return o?`참고로 이번 주 뉴스에서도 "${o}" 키워드가 다뤄졌습니다 — 관련 기사를 월요일 기사 다이제스트에서 확인해보세요.`:``}catch{return``}}async function Rt(e,t=`news`){let n=e.LOOKBOOK_DB,r=Ft();if(t===`search_trend`||t===`shopping_insight`){let i=xt(),a=t===`search_trend`?await kt(e,i):await jt(e,i),o=It(t,a,t===`shopping_insight`?await Mt(e,i):null),s=await Lt(n,r,a);s&&(o+=` ${s}`);let l=(await n.prepare(`INSERT INTO content_digests (period, status, summary, keywords, type) VALUES (?, 'draft', ?, '[]', ?)`).bind(r,o,t).run()).meta.last_row_id;for(let e of a)await n.prepare(`INSERT INTO digest_trends (digest_id, type, label, change_pct) VALUES (?,?,?,?)`).bind(l,t,e.label,e.changePct).run();return{digestId:l,articleCount:a.length}}let{summary:i,keywords:a,items:o}=await Pt(e,await Et(e,7)),s=(await n.prepare(`INSERT INTO content_digests (period, status, summary, keywords, type) VALUES (?, 'draft', ?, ?, 'news')`).bind(r,i,JSON.stringify(a)).run()).meta.last_row_id;for(let e of o)await n.prepare(`INSERT INTO digest_articles (digest_id, category, title, source, url, published_at, summary, importance)
       VALUES (?,?,?,?,?,?,?,?)`).bind(s,e.category,e.title,e.source,e.url,e.published_at,e.summary,e.importance).run();return{digestId:s,articleCount:o.length}}B.post(`/generate`,async e=>{let t=e.req.query(`type`)||(await e.req.json().catch(()=>({})))?.type,n=t===`search_trend`||t===`shopping_insight`?t:`news`;try{let t=await Rt(e.env,n);return t.articleCount===0&&await yt(e.env,{source:`server`,message:`콘텐츠 다이제스트(${n}) 생성 결과 0건`,route:`/api/admin/content-digest/generate`,extra:{type:n,digestId:t.digestId}}),e.json({success:!0,...t})}catch(t){return await yt(e.env,{source:`server`,message:`콘텐츠 다이제스트(${n}) 생성 실패: ${String(t?.message||t)}`,stack:t?.stack,route:`/api/admin/content-digest/generate`,extra:{type:n}}),e.json({success:!1,message:String(t?.message||t)},500)}}),B.get(`/debug-news`,async e=>{let t=e.req.query(`kw`)||bt[0];if(!e.env.NAVER_CLIENT_ID||!e.env.NAVER_CLIENT_SECRET)return e.json({success:!1,keyword:t,error:`NAVER_CLIENT_ID/NAVER_CLIENT_SECRET이 설정되지 않았습니다.`});let n=`https://naverapihub.apigw.ntruss.com/search/v1/news?query=${encodeURIComponent(t)}&display=10&sort=date`;try{let r=await fetch(n,{headers:{"X-NCP-APIGW-API-KEY-ID":e.env.NAVER_CLIENT_ID,"X-NCP-APIGW-API-KEY":e.env.NAVER_CLIENT_SECRET},signal:AbortSignal.timeout(12e3)}),i=await r.text();return e.json({success:!0,keyword:t,url:n,httpStatus:r.status,contentType:r.headers.get(`content-type`),bodyLength:i.length,bodyPreview:i.slice(0,800)})}catch(r){return e.json({success:!1,keyword:t,url:n,error:String(r?.message||r)})}}),B.get(`/debug-classify`,async e=>{if(!e.env.ANTHROPIC_API_KEY)return e.json({success:!1,error:`ANTHROPIC_API_KEY가 설정되지 않았습니다.`});let t=await Et(e.env,7),n=`당신은 국내 중소 패션 브랜드를 위한 패션 산업 뉴스 큐레이터입니다.
아래는 최근 수집된 패션 관련 뉴스 제목 목록입니다. 이 중에서 국내 중소 패션 브랜드 운영자가 알아두면 좋을 기사를 최대 8개 선별하고, 카카오톡 채널 메시지로 보낼 수 있게 정리해주세요.
...
원본 목록 순번(1-based)을 idx로 사용해서, 아래 JSON 형식으로만 응답하세요 (마크다운 코드펜스 없이, 내부/시스템 태그 없이):
{"overallSummary": string, "keywords": string[], "items": [{"idx": number, "category": string, "summary": string, "importance": number}]}`,r=async t=>{let n=Date.now(),r=await fetch(`https://api.anthropic.com/v1/messages`,{method:`POST`,headers:{"Content-Type":`application/json`,"x-api-key":e.env.ANTHROPIC_API_KEY,"anthropic-version":`2023-06-01`,"User-Agent":`EZlook-ContentDigest/1.0 (+https://www.aifashion.co.kr)`},body:JSON.stringify(t),signal:AbortSignal.timeout(45e3)}),i=await r.text(),a=null;try{a=JSON.parse(i)?.usage??null}catch{}let o=[`cf-ray`,`server`,`via`,`x-request-id`,`anthropic-ratelimit-requests-remaining`,`anthropic-ratelimit-requests-limit`,`retry-after`,`date`],s={};for(let e of o)s[e]=r.headers.get(e);return{httpStatus:r.status,tookMs:Date.now()-n,headers:s,usage:a,rawBodyPreview:i.slice(0,500)}};try{let[i,a]=await Promise.all([r({model:`claude-sonnet-5`,thinking:{type:`disabled`},max_tokens:3e3,messages:[{role:`user`,content:n}]}),r({model:`claude-sonnet-5`,thinking:{type:`disabled`},max_tokens:16,messages:[{role:`user`,content:`hi`}]})]);return e.json({success:!0,articleCount:t.length,real:i,control:a})}catch(n){return e.json({success:!1,articleCount:t.length,error:String(n?.message||n)})}}),B.get(`/debug-collect`,async e=>{let t=await Promise.all(bt.map(async t=>({keyword:t,count:(await Tt(e.env,t,12)).length}))),n=await Et(e.env,7);return e.json({success:!0,perKeyword:t,afterDedupeAndDateFilter:n.length,sample:n.slice(0,3)})}),B.get(`/debug-trends`,async e=>{if(!e.env.NAVER_CLIENT_ID||!e.env.NAVER_CLIENT_SECRET)return e.json({success:!1,error:`NAVER_CLIENT_ID/NAVER_CLIENT_SECRET이 설정되지 않았습니다.`});let{start:t,end:n}=Dt(),r={"X-NCP-APIGW-API-KEY-ID":e.env.NAVER_CLIENT_ID,"X-NCP-APIGW-API-KEY":e.env.NAVER_CLIENT_SECRET,"Content-Type":`application/json`};try{let[i,a]=await Promise.all([fetch(`https://naverapihub.apigw.ntruss.com/search-trend/v1/search`,{method:`POST`,headers:r,body:JSON.stringify({startDate:t,endDate:n,timeUnit:`week`,keywordGroups:xt().slice(0,5).map(e=>({groupName:e,keywords:[e]}))}),signal:AbortSignal.timeout(12e3)}),fetch(`https://naverapihub.apigw.ntruss.com/shopping/v1/category/keywords`,{method:`POST`,headers:r,body:JSON.stringify({startDate:t,endDate:n,timeUnit:`week`,category:At.code,keyword:xt().slice(0,5).map(e=>({name:e,param:[e]}))}),signal:AbortSignal.timeout(12e3)})]),[o,s]=await Promise.all([i.text(),a.text()]);return e.json({success:!0,searchTrend:{httpStatus:i.status,body:o.slice(0,1500)},shoppingInsight:{httpStatus:a.status,body:s.slice(0,1500)}})}catch(t){return e.json({success:!1,error:String(t?.message||t)})}});var zt={news:775e3,search_trend:5e4,shopping_insight:5e4};B.get(`/usage`,async e=>{let t=new Date().toISOString().slice(0,7),{results:n}=await e.env.LOOKBOOK_DB.prepare(`SELECT api_type, call_count FROM naver_api_usage WHERE year_month = ?`).bind(t).all(),r={news:0,search_trend:0,shopping_insight:0};for(let e of n)r[e.api_type]=e.call_count;let i=Object.keys(zt).map(e=>({apiType:e,count:r[e]||0,limit:zt[e],pct:Math.round((r[e]||0)/zt[e]*1e3)/10}));return e.json({success:!0,yearMonth:t,usage:i})}),B.get(`/list`,async e=>{let t=e.req.query(`type`),n=t?`WHERE d.type = ?`:``,r=t?[t]:[],{results:i}=await e.env.LOOKBOOK_DB.prepare(`SELECT d.*, (SELECT COUNT(*) FROM digest_articles a WHERE a.digest_id = d.id AND a.excluded = 0) AS article_count
     FROM content_digests d ${n} ORDER BY d.generated_at DESC LIMIT 50`).bind(...r).all();return e.json({success:!0,digests:i})}),B.get(`/:id`,async e=>{let t=e.req.param(`id`),n=await e.env.LOOKBOOK_DB.prepare(`SELECT * FROM content_digests WHERE id = ?`).bind(t).first();if(!n)return e.json({success:!1,message:`찾을 수 없습니다.`},404);let{results:r}=await e.env.LOOKBOOK_DB.prepare(`SELECT * FROM digest_articles WHERE digest_id = ? ORDER BY importance DESC, id ASC`).bind(t).all(),{results:i}=await e.env.LOOKBOOK_DB.prepare(`SELECT * FROM digest_trends WHERE digest_id = ? ORDER BY change_pct DESC`).bind(t).all();return e.json({success:!0,digest:{...n,keywords:JSON.parse(n.keywords||`[]`)},articles:r,trends:i})}),B.delete(`/:id`,async e=>{let t=e.req.param(`id`);return await e.env.LOOKBOOK_DB.prepare(`DELETE FROM digest_articles WHERE digest_id = ?`).bind(t).run(),await e.env.LOOKBOOK_DB.prepare(`DELETE FROM digest_trends WHERE digest_id = ?`).bind(t).run(),await e.env.LOOKBOOK_DB.prepare(`DELETE FROM content_digests WHERE id = ?`).bind(t).run(),e.json({success:!0})}),B.patch(`/:id`,async e=>{let t=e.req.param(`id`),n=await e.req.json(),r=[],i=[];return n.summary!==void 0&&(r.push(`summary = ?`),i.push(n.summary)),n.status!==void 0&&(r.push(`status = ?`),i.push(n.status),n.status===`reviewed`&&r.push(`reviewed_at = datetime('now')`),n.status===`sent`&&r.push(`sent_at = datetime('now')`)),r.length?(i.push(t),await e.env.LOOKBOOK_DB.prepare(`UPDATE content_digests SET ${r.join(`, `)} WHERE id = ?`).bind(...i).run(),e.json({success:!0})):e.json({success:!1,message:`변경할 필드가 없습니다.`},400)}),B.patch(`/:id/articles/:articleId`,async e=>{let{articleId:t}=e.req.param(),n=await e.req.json(),r=[],i=[];return n.excluded!==void 0&&(r.push(`excluded = ?`),i.push(+!!n.excluded)),n.summary!==void 0&&(r.push(`summary = ?`),i.push(n.summary)),r.length?(i.push(t),await e.env.LOOKBOOK_DB.prepare(`UPDATE digest_articles SET ${r.join(`, `)} WHERE id = ?`).bind(...i).run(),e.json({success:!0})):e.json({success:!1,message:`변경할 필드가 없습니다.`},400)}),B.get(`/:id/kakao-text`,async e=>{let t=e.req.param(`id`),n=await e.env.LOOKBOOK_DB.prepare(`SELECT * FROM content_digests WHERE id = ?`).bind(t).first();if(!n)return e.json({success:!1,message:`찾을 수 없습니다.`},404);let r=n.type===`search_trend`||n.type===`shopping_insight`?n.type:`news`,i=e=>`${e>0?`📈+`:e<0?`📉`:`➖`}${e}%`,a=[];if(r===`news`){let{results:r}=await e.env.LOOKBOOK_DB.prepare(`SELECT * FROM digest_articles WHERE digest_id = ? AND excluded = 0 ORDER BY importance DESC, id ASC LIMIT 5`).bind(t).all();a.push(`🧵 EZlook 패션 트렌드 위클리 — ${n.period}`,``,n.summary,``),r.forEach((e,t)=>{a.push(`${t+1}. [${e.category}] ${e.title}`),a.push(e.summary),e.url&&a.push(`🔗 ${e.url}`),a.push(``)})}else{let{results:o}=await e.env.LOOKBOOK_DB.prepare(`SELECT * FROM digest_trends WHERE digest_id = ? ORDER BY change_pct DESC`).bind(t).all(),s=r===`search_trend`?`🔎 EZlook 검색어트렌드 위클리`:`🛍️ EZlook 쇼핑인사이트 위클리`;a.push(`${s} — ${n.period}`,``,n.summary,``),o.forEach(e=>a.push(`- ${e.label} ${i(e.change_pct)}`)),a.push(``)}return a.push(`👉 AI 룩북 무료 체험: https://www.aifashion.co.kr/?utm_source=kakao&utm_medium=channel&utm_campaign=weekly_digest`),e.text(a.join(`
`))});var Bt=`muc0ukkf`;function Vt(e){return e.env.TECHPACK_MENU_VISIBLE===`true`}var Ht=e=>e?`
  ...
  <script>
    ...
    gtag('js', new Date());
    gtag('config', '${e}');
  <\/script>`:``,V=new ke;V.use(`/api/*`,Ve()),V.use(`/static/*`,Be({root:`./public`})),V.onError(async(e,t)=>{console.error(`[onError] 미처리 서버 에러:`,e);try{await yt(t.env,{source:`server`,message:e?.message||String(e),stack:e?.stack,route:new URL(t.req.url).pathname})}catch{}return t.json({success:!1,message:`서버 오류가 발생했습니다.`},500)}),V.use(`*`,async(e,t)=>{if((e.req.header(`host`)||``).includes(`studiob.aifashion.co.kr`)&&!e.req.path.startsWith(`/api/`)&&!e.req.path.startsWith(`/payment/`)){let t=new URL(e.req.url);return e.redirect(`https://www.aifashion.co.kr${t.pathname}${t.search}`,302)}await t()}),V.route(`/api/admin/leads`,I),V.route(`/api/admin/bizleads`,z),V.route(`/api/admin/content-digest`,B);var H=`https://api.atlascloud.ai`,U=`https://www.aifashion.co.kr`,Ut=`https://ezlook-techpack-web.onrender.com`,W=async(e,t)=>{let n=e.req.header(`X-Admin-Password`),r=e.env.ADMIN_PASSWORD;if(!r)return e.json({success:!1,message:`서버 설정 오류: ADMIN_PASSWORD 환경변수가 설정되지 않았습니다.`},500);if(n!==r)return e.json({success:!1,message:`인증 실패`},401);await t()},Wt=async(e,t)=>{let n=e.env.ADMIN_PASSWORD,r=e.env.MAINT_API_TOKEN,i=e.req.header(`X-Admin-Password`),a=e.req.header(`X-Maint-Token`);if(!n)return e.json({success:!1,message:`서버 설정 오류: ADMIN_PASSWORD 환경변수가 설정되지 않았습니다.`},500);if(i!==n&&!(r&&a===r))return e.json({success:!1,message:`인증 실패`},401);await t()};V.post(`/api/errors/report`,async e=>{try{let t=await e.req.json().catch(()=>({}));return!t||!t.message?e.json({success:!1},400):(await yt(e.env,{source:`client`,message:t.message,stack:t.stack,route:t.url,extra:{userAgent:e.req.header(`user-agent`)||``}}),e.json({success:!0}))}catch{return e.json({success:!1},500)}}),V.get(`/api/admin/errors`,Wt,async e=>{try{let t=e.env.LOOKBOOK_DB,n=parseInt(e.req.query(`page`)||`1`),r=Math.min(parseInt(e.req.query(`limit`)||`30`),100),i=e.req.query(`status`)||``,a=(n-1)*r,o=`WHERE 1=1`,s=[];i&&(o+=` AND status = ?`,s.push(i));let l=await t.prepare(`SELECT COUNT(*) as cnt FROM error_logs ${o}`).bind(...s).first(),u=await t.prepare(`SELECT status, COUNT(*) as cnt FROM error_logs GROUP BY status`).all(),d=await t.prepare(`SELECT * FROM error_logs ${o} ORDER BY created_at DESC LIMIT ? OFFSET ?`).bind(...s,r,a).all(),f={open:0,in_review:0,resolved:0};for(let e of u.results||[])f[e.status]=e.cnt;return e.json({success:!0,errors:d.results,total:l?.cnt||0,page:n,limit:r,counts:f})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.patch(`/api/admin/errors/:id`,Wt,async e=>{try{let t=e.env.LOOKBOOK_DB,n=e.req.param(`id`),r=await e.req.json().catch(()=>({}));if(r.status&&![`open`,`in_review`,`resolved`].includes(r.status))return e.json({success:!1,message:`잘못된 status 값입니다.`},400);let i=[`updated_at = datetime('now')`],a=[];return r.status!==void 0&&(i.push(`status = ?`),a.push(r.status)),r.fix_pr_url!==void 0&&(i.push(`fix_pr_url = ?`),a.push(r.fix_pr_url)),r.note!==void 0&&(i.push(`note = ?`),a.push(r.note)),a.push(n),await t.prepare(`UPDATE error_logs SET ${i.join(`, `)} WHERE id = ?`).bind(...a).run(),e.json({success:!0})}catch(t){return e.json({success:!1,message:t.message},500)}});var G={enabled:!0,prefix:``,suffix:``,styleGuide:[`SCENE-FIRST INTEGRATION PRINCIPLE: Every composited element — clothing, face, skin — must look as if it was photographed in the original background scene, not pasted in.`,`LIGHTING MATCH: Identify the background scene's primary light source direction, color temperature (warm/neutral/cool), and intensity. Apply identical lighting to the person's clothing, face, and all exposed skin. Cast-shadows, specular highlights, and subsurface skin scattering must all originate from the scene's light source.`,`COLOR GRADE MATCH: The background scene carries a specific color grade and tonal mood (e.g. warm golden, cool blue, high-contrast, soft pastel, moody dark). Render all composited elements under this exact color cast — do NOT render clothing or skin under a neutral white balance if the scene is warm or cool-toned.`,`MOOD AND ATMOSPHERE MATCH: Preserve the scene's overall visual mood and atmosphere. The final image must feel tonally coherent — bright and airy scenes stay bright, moody scenes stay moody, editorial high-contrast stays high-contrast.`,`FABRIC LIGHT INTERACTION: Simulate realistic light interaction with the new fabric — specular sheen on satin/silk, soft diffuse on cotton/knit, translucency on chiffon/voile — all under the scene's lighting conditions.`,`Fashion editorial quality: magazine cover level seamless compositing, physically grounded.`].join(` `),technicalSpec:[`초사실적 표현, 직물 질감과 피부 디테일 극사실 재현.`,`의류 드레이프와 핏 완벽 재현. 자연스러운 주름 외 구김 없음.`,`의류에 선명한 포커스. 배경과 동일한 심도 및 렌즈 특성 유지.`,`배경 씬의 색감·무드·조명 톤을 인물과 의류에 완전히 통합. 합성 아티팩트 없음.`,`참조 이미지에 없는 의류, 액세서리, 소품 절대 추가 금지.`,`ABSOLUTE PROHIBITION: DO NOT modify, redesign, or alter any detail of the uploaded clothing item.`].join(` `),updatedAt:new Date().toISOString()};function Gt(e){if(!G.enabled)return e;let t=[];return G.prefix.trim()&&t.push(G.prefix.trim()),t.push(e),G.styleGuide.trim()&&t.push(G.styleGuide.trim()),G.technicalSpec.trim()&&t.push(G.technicalSpec.trim()),G.suffix.trim()&&t.push(G.suffix.trim()),t.join(` `)}async function K(e){let t=await e.get(`model_index`);return t?JSON.parse(t):[]}async function Kt(e,t){await e.put(`model_index`,JSON.stringify(t))}async function q(e){let t=await e.get(`bg_index`);return t?JSON.parse(t):[]}async function qt(e,t){await e.put(`bg_index`,JSON.stringify(t))}async function Jt(e,t){let n=await q(e),r=n.find(e=>e.id===t);if(!r)return{ok:!1,isDefault:!1};let i=!r.isDefault;return n.forEach(e=>{e.isDefault=i&&e.id===t}),await qt(e,n),{ok:!0,isDefault:i}}async function Yt(e){let t=await e.get(`id_counter`),n=t?parseInt(t)+1:1001;return await e.put(`id_counter`,String(n)),String(n-1==0?1e3:n-1)}async function Xt(e){await e.prepare(`UPDATE id_counter SET value = value + 1 WHERE id = 1`).run();let t=await e.prepare(`SELECT value FROM id_counter WHERE id = 1`).first();return String(t?.value??1e3)}async function Zt(e){let{results:t}=await e.prepare(`SELECT id, name, desc_text, gender, age, mood, created_at FROM custom_models ORDER BY created_at ASC`).all();return t.map(e=>({id:e.id,name:e.name,desc:e.desc_text,gender:e.gender||`미분류`,age:e.age||`미분류`,mood:e.mood||`미분류`,createdAt:e.created_at}))}async function Qt(e,t){let n=[];for(let r of t){let{name:t,desc:i,gender:a,age:o,mood:s,imageBase64:l}=r;if(!t||!l)continue;let u=await Xt(e),d=new Date().toISOString();try{await e.prepare(`INSERT INTO custom_models (id, name, desc_text, gender, age, mood, image_b64, created_at) VALUES (?,?,?,?,?,?,?,?)`).bind(u,t,i||t,a||`미분류`,o||`미분류`,s||`미분류`,l,d).run()}catch{await e.prepare(`INSERT INTO custom_models (id, name, desc_text, image_b64, created_at) VALUES (?,?,?,?,?)`).bind(u,t,i||t,l,d).run()}n.push({id:u,name:t,desc:i||t,gender:a||`미분류`,age:o||`미분류`,mood:s||`미분류`,createdAt:d})}return n}async function $t(e,t){return((await e.prepare(`DELETE FROM custom_models WHERE id = ?`).bind(t).run()).meta?.changes??0)>0}async function en(e,t){return(await e.prepare(`SELECT image_b64 FROM custom_models WHERE id = ?`).bind(t).first())?.image_b64??null}async function tn(e){try{let{results:t}=await e.prepare(`SELECT id, name, category, bg_desc, created_at, is_default, CASE WHEN gen_image_b64 IS NOT NULL AND gen_image_b64 != '' THEN 1 ELSE 0 END AS has_gen_image FROM custom_bgs ORDER BY created_at ASC`).all();return t.map(e=>({id:e.id,name:e.name,bgDesc:e.bg_desc,category:e.category,createdAt:e.created_at,hasGenImage:!!e.has_gen_image,isDefault:!!e.is_default}))}catch{let{results:t}=await e.prepare(`SELECT id, name, category, bg_desc, created_at, CASE WHEN gen_image_b64 IS NOT NULL AND gen_image_b64 != '' THEN 1 ELSE 0 END AS has_gen_image FROM custom_bgs ORDER BY created_at ASC`).all();return t.map(e=>({id:e.id,name:e.name,bgDesc:e.bg_desc,category:e.category,createdAt:e.created_at,hasGenImage:!!e.has_gen_image,isDefault:!1}))}}async function nn(e,t){let n=await e.prepare(`SELECT is_default FROM custom_bgs WHERE id = ?`).bind(t).first();if(!n)return{ok:!1,isDefault:!1};let r=!n.is_default;return await e.prepare(`UPDATE custom_bgs SET is_default = 0`).run(),r&&await e.prepare(`UPDATE custom_bgs SET is_default = 1 WHERE id = ?`).bind(t).run(),{ok:!0,isDefault:r}}async function rn(e,t){let n=[];for(let r of t){let{name:t,bgDesc:i,category:a,imageBase64:o}=r;if(!t||!o)continue;let s=await Xt(e),l=new Date().toISOString();await e.prepare(`INSERT INTO custom_bgs (id, name, category, bg_desc, image_b64, created_at) VALUES (?,?,?,?,?,?)`).bind(s,t,a||`커스텀`,i||t,o,l).run(),n.push({id:s,name:t,bgDesc:i||t,category:a||`커스텀`,createdAt:l})}return n}async function an(e,t){return((await e.prepare(`DELETE FROM custom_bgs WHERE id = ?`).bind(t).run()).meta?.changes??0)>0}async function on(e,t){return(await e.prepare(`SELECT image_b64 FROM custom_bgs WHERE id = ?`).bind(t).first())?.image_b64??null}async function sn(e,t,n){return((await e.prepare(`UPDATE custom_bgs SET gen_image_b64 = ? WHERE id = ?`).bind(n,t).run()).meta?.changes??0)>0}var J=[],Y=[],cn=1e3,X=[{code:`TOP`,group:`상의`,label:`상의`},{code:`BOTTOM`,group:`하의`,label:`하의`},{code:`OUTER`,group:`아우터`,label:`아우터`},{code:`DRESS`,group:`원피스`,label:`원피스`},{code:`JUMPSUIT`,group:`점프수트`,label:`점프수트`},{code:`SET`,group:`세트`,label:`세트`}],ln=new Set(X.map(e=>e.code));async function un(e,t){return await e.get(`ghostcut_img:${t}`)}async function dn(e,t,n){await e.put(`ghostcut_img:${t}`,n)}async function fn(e,t){await e.delete(`ghostcut_img:${t}`)}async function pn(e){let t=await Promise.all(X.map(async t=>[t.code,!!await e.get(`ghostcut_img:${t.code}`)]));return Object.fromEntries(t)}async function mn(e,t){return(await e.prepare(`SELECT image_b64 FROM ghost_cut_samples WHERE category = ?`).bind(t).first())?.image_b64??null}async function hn(e,t,n,r,i){await e.prepare(`INSERT INTO ghost_cut_samples (category, group_name, label_ko, image_b64, updated_at) VALUES (?,?,?,?,datetime('now'))
     ON CONFLICT(category) DO UPDATE SET image_b64 = excluded.image_b64, updated_at = excluded.updated_at`).bind(t,n,r,i).run()}async function gn(e,t){return((await e.prepare(`DELETE FROM ghost_cut_samples WHERE category = ?`).bind(t).run()).meta?.changes??0)>0}async function _n(e){let{results:t}=await e.prepare(`SELECT category FROM ghost_cut_samples WHERE image_b64 IS NOT NULL AND image_b64 != ''`).all(),n=new Set(t.map(e=>e.category)),r={};return X.forEach(e=>{r[e.code]=n.has(e.code)}),r}var Z={};V.post(`/api/admin/models`,W,async e=>{try{let t=await e.req.json(),n=Array.isArray(t)?t:[t];if(n.length===0)return e.json({success:!1,message:`업로드할 항목이 없습니다.`},400);let r=e.env?.LOOKBOOK_KV,i=e.env?.LOOKBOOK_DB,a=[];if(r){let e=await K(r);for(let t of n){let{name:n,desc:i,imageBase64:o}=t;if(!n||!o)continue;let s=await Yt(r),l={id:s,name:n,desc:i||n,createdAt:new Date().toISOString()};e.push(l),await r.put(`model_img:${s}`,o),a.push(l)}await Kt(r,e)}else if(i)a=await Qt(i,n);else for(let e of n){let{name:t,desc:n,imageBase64:r}=e;if(!t||!r)continue;let i=String(cn++),o={id:i,name:t,desc:n||t,imageBase64:r,createdAt:new Date().toISOString()};J.push(o),a.push({id:i,name:t,desc:o.desc,createdAt:o.createdAt})}return e.json({success:!0,models:a,count:a.length})}catch(t){return e.json({success:!1,message:t.message},500)}});var vn={TOP:`You are an image classifier for a fashion shopping app upload slot labeled "상의" (TOP). Does this image contain a TOP garment (shirt, blouse, t-shirt, jacket, coat, sweater, hoodie, etc.) that is clearly identifiable somewhere in the image — whether laid flat, on a hanger, worn alone, or worn together with a bottom in a full-outfit/styled photo? A full-outfit shot showing both a top and a bottom together is fine as long as a top is present — the app will extract only the top from it.
...
Respond with ONLY one word: YES or NO.`};V.post(`/api/validate/clothing`,async e=>{try{let t=await e.req.json(),n=t?.imageBase64||``,r=t?.cat||``;if(!n)return e.json({success:!1,message:`imageBase64 필수`},400);let i=e.env?.OPENAI_API_KEY||``;if(!i)return e.json({success:!0,isClothing:!0});let a=await xn(i,n,vn[r]||vn.DRESS);return a===null?e.json({success:!0,isClothing:!0}):e.json({success:!0,isClothing:a.trim().toUpperCase().startsWith(`YES`)})}catch(t){return console.error(`validate/clothing error:`,t),e.json({success:!0,isClothing:!0})}});var yn=[`You are a clothing product classifier for a "ghost mannequin" e-commerce photography app.`,`Look at the uploaded image. First decide: is this clearly a photo of a SINGLE clothing/fashion product (worn, laid flat, on a hanger, or on a mannequin)? It does not need to be a professional product shot.`,`If it is NOT a clothing product at all (e.g. a person's portrait unrelated to showing a garment, an animal, food, scenery, a random object, or an image with no identifiable single garment), respond with exactly: NOT_CLOTHING`,`If it IS a clothing product, respond with EXACTLY ONE of these category codes (the single best match, nothing else):`,X.map(e=>`${e.code} (${e.group} - ${e.label})`).join(`, `),`Respond with ONLY the code (e.g. "TOP") or "NOT_CLOTHING" — no explanation, no punctuation, no extra words.`].join(`
`);V.post(`/api/ghostcut/classify`,async e=>{try{let t=(await e.req.json())?.imageBase64||``;if(!t)return e.json({success:!1,message:`imageBase64 필수`},400);let n=e.env?.OPENAI_API_KEY||``;if(!n)return e.json({success:!1,message:`서버 설정 오류: OPENAI_API_KEY 미설정`},500);let r=await xn(n,t,yn);if(r===null)return e.json({success:!1,message:`이미지 분석에 실패했습니다. 잠시 후 다시 시도해주세요.`});let i=r.trim().toUpperCase().replace(/[^A-Z_]/g,``);if(i===`NOT_CLOTHING`)return e.json({success:!0,isClothing:!1});let a=X.find(e=>e.code===i);if(!a)return console.warn(`ghostcut/classify: 알 수 없는 분류 응답:`,r),e.json({success:!1,message:`이미지 분류에 실패했습니다. 다른 사진으로 다시 시도해주세요.`});let o=e.env?.LOOKBOOK_KV,s=e.env?.LOOKBOOK_DB,l=!1;return l=o?!!await un(o,i):s?!!await mn(s,i):!!Z[i],e.json({success:!0,isClothing:!0,category:i,group:a.group,label:a.label,displayLabel:a.label,sampleReady:l})}catch(t){return console.error(`ghostcut/classify error:`,t),e.json({success:!1,message:`이미지 분석 중 오류가 발생했습니다.`})}}),V.post(`/api/admin/auto-label`,W,async e=>{try{let{type:t,imageBase64:n}=await e.req.json();if(!n)return e.json({success:!1,message:`imageBase64 필수`},400);let r=e.env?.OPENAI_API_KEY||``;if(!r)return e.json({success:!1,message:`OPENAI_API_KEY 미설정`},500);let i=e=>{let t=e.match(/\{[\s\S]*\}/);return JSON.parse(t?.[0]||e)};if(t===`model`){let t=await xn(r,n,`You are an AI fashion model classifier. Analyze this model photo and return ONLY a JSON object with these exact fields:
{
  "gender": "여성" or "남성",
...
Return ONLY the JSON, no explanation.`);if(t===null)return e.json({success:!1,message:`라벨링 요청 실패`},500);try{let n=i(t);return e.json({success:!0,labels:{category:n.category||`스튜디오`,mood:n.mood||`미니멀`,name_ko:n.name_ko||``}})}catch{return e.json({success:!1,message:`응답 파싱 실패`,raw:t})}}return e.json({success:!1,message:`type은 model 또는 background 이어야 합니다.`},400)}catch(t){return e.json({success:!1,message:t.message},500)}}),V.patch(`/api/admin/models/:id/labels`,W,async e=>{try{let t=e.req.param(`id`),{gender:n,age:r,mood:i}=await e.req.json(),a=e.env?.LOOKBOOK_KV,o=e.env?.LOOKBOOK_DB;if(a){let o=await K(a),s=o.findIndex(e=>e.id===t);return s===-1?e.json({success:!1,message:`모델을 찾을 수 없습니다.`},404):(o[s]={...o[s],gender:n||`미분류`,age:r||`미분류`,mood:i||`미분류`},await Kt(a,o),e.json({success:!0}))}return o?(await o.prepare(`UPDATE custom_models SET gender=?, age=?, mood=? WHERE id=?`).bind(n||`미분류`,r||`미분류`,i||`미분류`,t).run(),e.json({success:!0})):e.json({success:!1,message:`D1/KV 없음`},500)}catch(t){return e.json({success:!1,message:t.message},500)}}),V.get(`/api/admin/models`,W,async e=>{let t=e.env?.LOOKBOOK_KV,n=e.env?.LOOKBOOK_DB;if(t){let n=await K(t);return e.json({success:!0,models:n})}if(n){let t=await Zt(n);return e.json({success:!0,models:t})}let r=J.map(e=>({id:e.id,name:e.name,desc:e.desc,createdAt:e.createdAt}));return e.json({success:!0,models:r})}),V.delete(`/api/admin/models/:id`,W,async e=>{let t=e.req.param(`id`),n=e.env?.LOOKBOOK_KV,r=e.env?.LOOKBOOK_DB;if(n){let r=await K(n),i=r.filter(e=>e.id!==t);return await Kt(n,i),await n.delete(`model_img:${t}`),e.json({success:r.length>i.length})}if(r){let n=await $t(r,t);return e.json({success:n})}let i=J.length;return J=J.filter(e=>e.id!==t),e.json({success:J.length<i})}),V.get(`/api/proxy/custom-model/:id`,async e=>{let t=e.req.param(`id`),n=e.env?.LOOKBOOK_KV,r=e.env?.LOOKBOOK_DB,i=null;if(i=n?await n.get(`model_img:${t}`):r?await en(r,t):J.find(e=>e.id===t)?.imageBase64||null,!i)return e.notFound();let[a,o]=i.split(`,`),s=(a.match(/data:([^;]+)/)||[])[1]||`image/jpeg`,l=atob(o),u=new Uint8Array(l.length);for(let e=0;e<l.length;e++)u[e]=l.charCodeAt(e);return new Response(u.buffer,{headers:{"Content-Type":s,"Cache-Control":`public, max-age=3600`}})}),V.post(`/api/admin/backgrounds`,W,async e=>{try{let t=await e.req.json(),n=Array.isArray(t)?t:[t];if(n.length===0)return e.json({success:!1,message:`업로드할 항목이 없습니다.`},400);let r=e.env?.LOOKBOOK_KV,i=e.env?.LOOKBOOK_DB,a=[];if(r){let e=await q(r);for(let t of n){let{name:n,bgDesc:i,category:o,imageBase64:s}=t;if(!n||!s)continue;let l=await Yt(r),u={id:l,name:n,bgDesc:i||n,category:o||`커스텀`,createdAt:new Date().toISOString()};e.push(u),await r.put(`bg_img:${l}`,s),a.push(u)}await qt(r,e)}else if(i)a=await rn(i,n);else for(let e of n){let{name:t,bgDesc:n,category:r,imageBase64:i}=e;if(!t||!i)continue;let o=String(cn++),s={id:o,name:t,bgDesc:n||t,category:r||`커스텀`,imageBase64:i,createdAt:new Date().toISOString()};Y.push(s),a.push({id:o,name:t,bgDesc:s.bgDesc,category:s.category,createdAt:s.createdAt})}return e.json({success:!0,backgrounds:a,count:a.length})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.get(`/api/admin/backgrounds`,W,async e=>{let t=e.env?.LOOKBOOK_KV,n=e.env?.LOOKBOOK_DB;if(t){let n=await q(t);return e.json({success:!0,backgrounds:n})}if(n){let t=await tn(n);return e.json({success:!0,backgrounds:t})}let r=Y.map(e=>({id:e.id,name:e.name,bgDesc:e.bgDesc,category:e.category,createdAt:e.createdAt,hasGenImage:!!e.genImageBase64,isDefault:!!e.isDefault}));return e.json({success:!0,backgrounds:r})}),V.delete(`/api/admin/backgrounds/:id`,W,async e=>{let t=e.req.param(`id`),n=e.env?.LOOKBOOK_KV,r=e.env?.LOOKBOOK_DB;if(n){let r=await q(n),i=r.filter(e=>e.id!==t);return await qt(n,i),await n.delete(`bg_img:${t}`),await n.delete(`bg_gen_img:${t}`),e.json({success:r.length>i.length})}if(r){let n=await an(r,t);return e.json({success:n})}let i=Y.length;return Y=Y.filter(e=>e.id!==t),e.json({success:Y.length<i})}),V.put(`/api/admin/backgrounds/:id/gen-image`,W,async e=>{let t=e.req.param(`id`);try{let n=(await e.req.json())?.imageBase64||``;if(!n)return e.json({success:!1,message:`imageBase64 필수`},400);let r=e.env?.LOOKBOOK_KV,i=e.env?.LOOKBOOK_DB;if(r){let i=await q(r),a=i.find(e=>e.id===t);return a?(await r.put(`bg_gen_img:${t}`,n),a.hasGenImage=!0,await qt(r,i),e.json({success:!0})):e.json({success:!1,message:`배경을 찾을 수 없습니다.`},404)}if(i)return await sn(i,t,n)?e.json({success:!0}):e.json({success:!1,message:`배경을 찾을 수 없습니다.`},404);let a=Y.find(e=>e.id===t);return a?(a.genImageBase64=n,e.json({success:!0})):e.json({success:!1,message:`배경을 찾을 수 없습니다.`},404)}catch(t){return e.json({success:!1,message:t.message},500)}}),V.put(`/api/admin/backgrounds/:id/default`,W,async e=>{let t=e.req.param(`id`);try{let n=e.env?.LOOKBOOK_KV,r=e.env?.LOOKBOOK_DB;if(n){let r=await Jt(n,t);return r.ok?e.json({success:!0,isDefault:r.isDefault}):e.json({success:!1,message:`배경을 찾을 수 없습니다.`},404)}if(r){let n=await nn(r,t);return n.ok?e.json({success:!0,isDefault:n.isDefault}):e.json({success:!1,message:`배경을 찾을 수 없습니다.`},404)}let i=Y.find(e=>e.id===t);if(!i)return e.json({success:!1,message:`배경을 찾을 수 없습니다.`},404);let a=!i.isDefault;return Y.forEach(e=>{e.isDefault=!1}),i.isDefault=a,e.json({success:!0,isDefault:a})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.get(`/api/proxy/custom-bg/:id`,async e=>{let t=e.req.param(`id`),n=e.env?.LOOKBOOK_KV,r=e.env?.LOOKBOOK_DB,i=null;if(i=n?await n.get(`bg_img:${t}`):r?await on(r,t):Y.find(e=>e.id===t)?.imageBase64||null,!i)return e.notFound();let[a,o]=i.split(`,`),s=(a.match(/data:([^;]+)/)||[])[1]||`image/jpeg`,l=atob(o),u=new Uint8Array(l.length);for(let e=0;e<l.length;e++)u[e]=l.charCodeAt(e);return new Response(u.buffer,{headers:{"Content-Type":s,"Cache-Control":`public, max-age=3600`}})}),V.get(`/api/admin/ghostcut-samples`,W,async e=>{let t=e.env?.LOOKBOOK_KV,n=e.env?.LOOKBOOK_DB,r={};t?r=await pn(t):n?r=await _n(n):X.forEach(e=>{r[e.code]=!!Z[e.code]});let i=X.map(e=>({...e,hasSample:!!r[e.code]}));return e.json({success:!0,categories:i})}),V.get(`/api/admin/ghostcut-samples/:category/image`,W,async e=>{let t=e.req.param(`category`);if(!ln.has(t))return e.json({success:!1,message:`알 수 없는 카테고리`},400);let n=e.env?.LOOKBOOK_KV,r=e.env?.LOOKBOOK_DB,i=null;return i=n?await un(n,t):r?await mn(r,t):Z[t]||null,e.json({success:!0,imageBase64:i})}),V.post(`/api/admin/ghostcut-samples/:category`,W,async e=>{let t=e.req.param(`category`),n=X.find(e=>e.code===t);if(!n)return e.json({success:!1,message:`알 수 없는 카테고리`},400);try{let r=(await e.req.json())?.imageBase64||``;if(!r)return e.json({success:!1,message:`imageBase64 필수`},400);let i=e.env?.LOOKBOOK_KV,a=e.env?.LOOKBOOK_DB;return i?await dn(i,t,r):a?await hn(a,t,n.group,n.label,r):Z[t]=r,e.json({success:!0})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.delete(`/api/admin/ghostcut-samples/:category`,W,async e=>{let t=e.req.param(`category`);if(!ln.has(t))return e.json({success:!1,message:`알 수 없는 카테고리`},400);let n=e.env?.LOOKBOOK_KV,r=e.env?.LOOKBOOK_DB;if(n)return await fn(n,t),e.json({success:!0});if(r){let n=await gn(r,t);return e.json({success:n})}let i=!!Z[t];return delete Z[t],e.json({success:i})}),V.get(`/api/proxy/clothing/:jobId/:idx`,async e=>{let t=e.req.param(`jobId`),n=parseInt(e.req.param(`idx`)||`0`,10),r=e.env?.LOOKBOOK_KV;if(!r)return e.notFound();let i=await r.get(`clothing_img:${t}`);if(!i)return e.notFound();let a=[];try{a=JSON.parse(i)}catch{}let o=a[n];if(!o)return e.notFound();let[s,l]=o.split(`,`),u=(s.match(/data:([^;]+)/)||[])[1]||`image/jpeg`,d=atob(l),f=new Uint8Array(d.length);for(let e=0;e<d.length;e++)f[e]=d.charCodeAt(e);return new Response(f.buffer,{headers:{"Content-Type":u,"Cache-Control":`public, max-age=3600`}})});var Q=e=>({Authorization:`Bearer ${e}`,"Content-Type":`application/json`}),bn=`https://api.openai.com`;async function xn(e,t,n){let r=`${bn}/v1/chat/completions`;try{let i=await fetch(r,{method:`POST`,headers:{Authorization:`Bearer ${e}`,"Content-Type":`application/json`},body:JSON.stringify({model:`gpt-4o-mini`,messages:[{role:`user`,content:[{type:`image_url`,image_url:{url:t.startsWith(`data:`)?t:`data:image/jpeg;base64,${t}`}},{type:`text`,text:n}]}]})}),a=await i.text();if(console.log(`openaiChatVision: POST ${r} → HTTP ${i.status} | body(첫 500자): ${a.slice(0,500)}`),!i.ok)return console.error(`openaiChatVision: 요청 실패 (HTTP ${i.status})`),null;let o;try{o=JSON.parse(a)}catch{return console.error(`openaiChatVision: JSON 파싱 실패, 원문:`,a.slice(0,300)),null}let s=o?.choices?.[0]?.message?.content;return typeof s==`string`?s:(console.warn(`openaiChatVision: 예상치 못한 응답 형식:`,JSON.stringify(o).slice(0,300)),null)}catch(e){return console.error(`openaiChatVision error:`,e?.message||e),null}}V.get(`/api/presets/models`,async e=>{let t=e.env?.LOOKBOOK_KV,n=e.env?.LOOKBOOK_DB,r;r=t?await K(t):n?await Zt(n):J.map(e=>({id:e.id,name:e.name,desc:e.desc,createdAt:e.createdAt}));let i=r.map(e=>({id:Number(e.id),name:e.name,gender:e.gender||`미분류`,age:e.age||`미분류`,mood:e.mood||`미분류`,body:`-`,skin:`-`,desc:e.desc,unsplashId:null,isCustom:!0,customId:e.id}));return e.json({models:i})}),V.get(`/api/presets/backgrounds`,async e=>{let t=e.env?.LOOKBOOK_KV,n=e.env?.LOOKBOOK_DB,r;r=t?await q(t):n?await tn(n):Y.map(e=>({id:e.id,name:e.name,bgDesc:e.bgDesc,category:e.category,createdAt:e.createdAt,isDefault:!!e.isDefault}));let i=r.map(e=>({id:Number(e.id),name:e.name,category:e.category,mood:`-`,bgDesc:e.bgDesc,unsplashId:null,isCustom:!0,customId:e.id,isDefault:!!e.isDefault}));return e.json({backgrounds:i})});var Sn=`home_showcase_images`,Cn=[1,2,3,4,5,6];async function wn(e){let t=await e.get(Sn);if(!t)return[];try{return JSON.parse(t)}catch{return[]}}async function Tn(e,t){await e.put(Sn,JSON.stringify(t))}V.get(`/api/home/showcase`,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({images:[]});let n=await wn(t);return e.json({images:n.map(e=>({id:e.id,imageBase64:e.imageBase64}))})}),V.get(`/api/admin/home-showcase`,W,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({success:!1,message:`KV 미설정`},500);let n=await wn(t);return e.json({success:!0,images:n})}),V.post(`/api/admin/home-showcase`,W,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({success:!1,message:`KV 미설정`},500);try{let n=await e.req.json(),r=Array.isArray(n?.images)?n.images:[];if(!r.length)return e.json({success:!1,message:`이미지가 필요합니다.`},400);let i=await wn(t),a=r.map(e=>({id:crypto.randomUUID(),imageBase64:e,createdAt:new Date().toISOString()})),o=[...i,...a];return await Tn(t,o),e.json({success:!0,count:a.length,images:o})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.delete(`/api/admin/home-showcase/:id`,W,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({success:!1,message:`KV 미설정`},500);let n=e.req.param(`id`);return await Tn(t,(await wn(t)).filter(e=>e.id!==n)),e.json({success:!0})});async function En(e,t){return await e.get(`home_feature_bg_${t}`)}V.get(`/api/home/feature-bgs`,async e=>{let t=e.env?.LOOKBOOK_KV,n={};if(t)for(let e of Cn)n[e]=await En(t,e);else Cn.forEach(e=>{n[e]=null});return e.json({backgrounds:n})}),V.put(`/api/admin/home-feature-bg/:slot`,W,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({success:!1,message:`KV 미설정`},500);let n=Number(e.req.param(`slot`));if(!Cn.includes(n))return e.json({success:!1,message:`잘못된 슬롯`},400);try{let r=(await e.req.json())?.imageBase64||``;return r?(await t.put(`home_feature_bg_${n}`,r),e.json({success:!0})):e.json({success:!1,message:`imageBase64 필수`},400)}catch(t){return e.json({success:!1,message:t.message},500)}}),V.delete(`/api/admin/home-feature-bg/:slot`,W,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({success:!1,message:`KV 미설정`},500);let n=Number(e.req.param(`slot`));return Cn.includes(n)?(await t.delete(`home_feature_bg_${n}`),e.json({success:!0})):e.json({success:!1,message:`잘못된 슬롯`},400)});var Dn=[1],On=22*1024*1024;V.get(`/api/home/howto-videos`,async e=>{let t=e.env?.LOOKBOOK_KV,n={};if(t)for(let e of Dn)n[e]=(await t.getWithMetadata(`home_howto_video_${e}`)).value==null?null:`/api/home/howto-video/${e}`;else Dn.forEach(e=>{n[e]=null});return e.json({videos:n})}),V.get(`/api/home/howto-video/:slot`,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.notFound();let n=Number(e.req.param(`slot`));if(!Dn.includes(n))return e.notFound();let{value:r,metadata:i}=await t.getWithMetadata(`home_howto_video_${n}`,`arrayBuffer`);if(!r)return e.notFound();let a=r,o=i?.contentType||`video/mp4`,s=a.byteLength,l=e.req.header(`range`);if(l){let e=l.match(/bytes=(\d*)-(\d*)/);if(e){let t=e[1]?parseInt(e[1],10):0,n=e[2]?parseInt(e[2],10):s-1,r=Math.max(0,Math.min(t,s-1)),i=Math.max(r,Math.min(n,s-1)),l=a.slice(r,i+1);return new Response(l,{status:206,headers:{"Content-Type":o,"Content-Range":`bytes ${r}-${i}/${s}`,"Accept-Ranges":`bytes`,"Content-Length":String(l.byteLength),"Cache-Control":`public, max-age=3600`}})}}return new Response(a,{headers:{"Content-Type":o,"Accept-Ranges":`bytes`,"Cache-Control":`public, max-age=3600`,"Content-Length":String(s)}})}),V.put(`/api/admin/home-howto-video/:slot`,W,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({success:!1,message:`KV 미설정`},500);let n=Number(e.req.param(`slot`));if(!Dn.includes(n))return e.json({success:!1,message:`잘못된 슬롯`},400);try{let r=await e.req.arrayBuffer();if(!r||r.byteLength===0)return e.json({success:!1,message:`영상 파일이 필요합니다.`},400);if(r.byteLength>On)return e.json({success:!1,message:`영상 용량은 ${Math.floor(On/1024/1024)}MB 이하만 가능합니다.`},400);let i=e.req.header(`content-type`)||`video/mp4`;return await t.put(`home_howto_video_${n}`,r,{metadata:{contentType:i}}),e.json({success:!0})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.delete(`/api/admin/home-howto-video/:slot`,W,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({success:!1,message:`KV 미설정`},500);let n=Number(e.req.param(`slot`));return Dn.includes(n)?(await t.delete(`home_howto_video_${n}`),e.json({success:!0})):e.json({success:!1,message:`잘못된 슬롯`},400)});var kn=[1,2,3,4,5],An=22*1024*1024;V.get(`/api/gen-loading-videos`,async e=>{let t=e.env?.LOOKBOOK_KV,n={};if(t)for(let e of kn)n[e]=(await t.getWithMetadata(`gen_loading_video_${e}`)).value==null?null:`/api/gen-loading-video/${e}`;else kn.forEach(e=>{n[e]=null});return e.json({videos:n})}),V.get(`/api/gen-loading-video/:slot`,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.notFound();let n=Number(e.req.param(`slot`));if(!kn.includes(n))return e.notFound();let{value:r,metadata:i}=await t.getWithMetadata(`gen_loading_video_${n}`,`arrayBuffer`);if(!r)return e.notFound();let a=r,o=i?.contentType||`video/mp4`,s=a.byteLength,l=e.req.header(`range`);if(l){let e=l.match(/bytes=(\d*)-(\d*)/);if(e){let t=e[1]?parseInt(e[1],10):0,n=e[2]?parseInt(e[2],10):s-1,r=Math.max(0,Math.min(t,s-1)),i=Math.max(r,Math.min(n,s-1)),l=a.slice(r,i+1);return new Response(l,{status:206,headers:{"Content-Type":o,"Content-Range":`bytes ${r}-${i}/${s}`,"Accept-Ranges":`bytes`,"Content-Length":String(l.byteLength),"Cache-Control":`public, max-age=3600`}})}}return new Response(a,{headers:{"Content-Type":o,"Accept-Ranges":`bytes`,"Cache-Control":`public, max-age=3600`,"Content-Length":String(s)}})}),V.put(`/api/admin/gen-loading-video/:slot`,W,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({success:!1,message:`KV 미설정`},500);let n=Number(e.req.param(`slot`));if(!kn.includes(n))return e.json({success:!1,message:`잘못된 슬롯`},400);try{let r=await e.req.arrayBuffer();if(!r||r.byteLength===0)return e.json({success:!1,message:`영상 파일이 필요합니다.`},400);if(r.byteLength>An)return e.json({success:!1,message:`영상 용량은 ${Math.floor(An/1024/1024)}MB 이하만 가능합니다.`},400);let i=e.req.header(`content-type`)||`video/mp4`;return await t.put(`gen_loading_video_${n}`,r,{metadata:{contentType:i}}),e.json({success:!0})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.delete(`/api/admin/gen-loading-video/:slot`,W,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({success:!1,message:`KV 미설정`},500);let n=Number(e.req.param(`slot`));return kn.includes(n)?(await t.delete(`gen_loading_video_${n}`),e.json({success:!0})):e.json({success:!1,message:`잘못된 슬롯`},400)});var jn=[1,2,3,4,5],Mn=8*1024*1024;V.get(`/api/gc-loading-images`,async e=>{let t=e.env?.LOOKBOOK_KV,n={};if(t)for(let e of jn)n[e]=(await t.getWithMetadata(`gc_loading_image_${e}`)).value==null?null:`/api/gc-loading-image/${e}`;else jn.forEach(e=>{n[e]=null});return e.json({images:n})}),V.get(`/api/gc-loading-image/:slot`,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.notFound();let n=Number(e.req.param(`slot`));if(!jn.includes(n))return e.notFound();let{value:r,metadata:i}=await t.getWithMetadata(`gc_loading_image_${n}`,`arrayBuffer`);if(!r)return e.notFound();let a=r,o=i?.contentType||`image/jpeg`;return new Response(a,{headers:{"Content-Type":o,"Cache-Control":`public, max-age=3600`,"Content-Length":String(a.byteLength)}})}),V.put(`/api/admin/gc-loading-image/:slot`,W,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({success:!1,message:`KV 미설정`},500);let n=Number(e.req.param(`slot`));if(!jn.includes(n))return e.json({success:!1,message:`잘못된 슬롯`},400);try{let r=await e.req.arrayBuffer();if(!r||r.byteLength===0)return e.json({success:!1,message:`이미지 파일이 필요합니다.`},400);if(r.byteLength>Mn)return e.json({success:!1,message:`이미지 용량은 ${Math.floor(Mn/1024/1024)}MB 이하만 가능합니다.`},400);let i=e.req.header(`content-type`)||`image/jpeg`;return await t.put(`gc_loading_image_${n}`,r,{metadata:{contentType:i}}),e.json({success:!0})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.delete(`/api/admin/gc-loading-image/:slot`,W,async e=>{let t=e.env?.LOOKBOOK_KV;if(!t)return e.json({success:!1,message:`KV 미설정`},500);let n=Number(e.req.param(`slot`));return jn.includes(n)?(await t.delete(`gc_loading_image_${n}`),e.json({success:!0})):e.json({success:!1,message:`잘못된 슬롯`},400)});var Nn=`fashion_news_cache_v2`,Pn=1200*1e3;function Fn(e){let t=e.match(/<!\[CDATA\[([\s\S]*?)\]\]>/);return(t?t[1]:e).replace(/&amp;/g,`&`).replace(/&lt;/g,`<`).replace(/&gt;/g,`>`).replace(/&#39;/g,`'`).replace(/&quot;/g,`"`).trim()}function In(e){let t=[],n=e.match(/<item>[\s\S]*?<\/item>/g)||[];for(let e of n){let n=e.match(/<title>([\s\S]*?)<\/title>/),r=e.match(/<link>([\s\S]*?)<\/link>/),i=e.match(/<source[^>]*>([\s\S]*?)<\/source>/),a=e.match(/<pubDate>([\s\S]*?)<\/pubDate>/);if(!n||!r)continue;let o=Fn(n[1]),s=i?Fn(i[1]):``;if(s){let e=` - `+s;o.endsWith(e)&&(o=o.slice(0,-e.length).trim())}t.push({title:o,link:Fn(r[1]),source:s,pubDate:a?Fn(a[1]):``})}return t.slice(0,12)}V.get(`/api/fashion-news`,async e=>{let t=e.env?.LOOKBOOK_KV;async function n(){if(!t)return null;let e=await t.get(Nn);if(!e)return null;try{return JSON.parse(e).news||null}catch{return null}}try{if(t){let n=await t.get(Nn);if(n){let t=JSON.parse(n);if(Date.now()-t.fetchedAt<Pn)return e.json({news:t.news})}}let r=await fetch(`https://news.google.com/rss/search?q=%ED%8C%A8%EC%85%98%20when%3A7d&hl=ko&gl=KR&ceid=KR:ko`,{headers:{"User-Agent":`Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36`,Accept:`application/rss+xml, application/xml, text/xml, */*`,"Accept-Language":`ko-KR,ko;q=0.9`}});if(!r.ok)throw Error(`RSS fetch failed: ${r.status}`);let i=await r.text(),a=In(i),o=Date.now(),s=a.filter(e=>{let t=e.pubDate?Date.parse(e.pubDate):NaN;return!Number.isNaN(t)&&o-t<=6048e5});if(s.length===0){let t=await n();return t&&t.length>0?e.json({news:t}):(console.warn(`fashion-news: RSS 응답에서 기사 파싱 실패 (item 0개)`),e.json({news:[],_debug:{httpStatus:r.status,xmlLength:i.length,hasItemTag:i.includes(`<item>`),hasRssTag:i.includes(`<rss`),preview:i.slice(0,500)}}))}return t&&await t.put(Nn,JSON.stringify({fetchedAt:Date.now(),news:s})),e.json({news:s})}catch(t){console.warn(`fashion-news fetch error:`,t.message);let r=await n();return r&&r.length>0?e.json({news:r}):e.json({news:[],_debug:{error:t.message}})}}),V.get(`/api/proxy/model-image/:id`,e=>e.notFound()),V.get(`/api/proxy/bg-image/:id`,e=>e.notFound()),V.get(`/api/proxy/gen-image`,async e=>{let t=e.req.query(`url`),n=e.req.query(`download`)===`1`;if(!t)return e.json({error:`Missing url param`},400);try{if(!new URL(t).protocol.startsWith(`https`))return e.json({error:`Only HTTPS URLs allowed`},400);let r=e.req.header(`Range`),i={"User-Agent":`Mozilla/5.0 (compatible; LookbookAI/1.0)`};r&&(i.Range=r);let a=await fetch(t,{headers:i});if(!a.ok&&a.status!==206)return e.json({error:`Upstream error: ${a.status}`},a.status);let o=a.headers.get(`Content-Type`)||`image/jpeg`,s=o.includes(`video`)?`mp4`:`jpg`,l=`lookbook_ai_${Date.now()}.${s}`,u={"Content-Type":o,"Cache-Control":`public, max-age=3600`,"Access-Control-Allow-Origin":`*`,"Accept-Ranges":`bytes`},d=a.headers.get(`Content-Range`);d&&(u[`Content-Range`]=d);let f=a.headers.get(`Content-Length`);return f&&(u[`Content-Length`]=f),n&&(u[`Content-Disposition`]=`attachment; filename="${l}"`),new Response(a.body,{status:a.status,headers:u})}catch(t){return console.error(`Gen image proxy error:`,t),e.json({error:t.message},500)}});var Ln=[{id:`p1`,name:`2024 S/S 룩북`,status:`done`,images:8,created:`2024-03-15`,thumb_color:`#FF6B9D`},{id:`p2`,name:`캐주얼 티셔츠 컷`,status:`done`,images:4,created:`2024-03-12`,thumb_color:`#3182F6`},{id:`p3`,name:`데님 라인 촬영`,status:`processing`,images:0,created:`2024-03-10`,thumb_color:`#3B82F6`},{id:`p4`,name:`원피스 봄 컬렉션`,status:`draft`,images:0,created:`2024-03-08`,thumb_color:`#00D4AA`}];V.get(`/api/projects`,e=>e.json({projects:Ln}));function Rn(e){let t=(e||``).toUpperCase();return t===`KR`||!t?{locale:`ko`,currency:`KRW`,pg:`toss`,messenger:`kakao`}:t===`JP`?{locale:`ja`,currency:`JPY`,pg:`stripe`,messenger:`line`}:{locale:`en`,currency:`USD`,pg:`stripe`,messenger:`web-share`}}V.get(`/api/locale`,async e=>{let t=(e.req.header(`CF-IPCountry`)??e.req.header(`cf-ipcountry`)??e.req.raw.cf?.country??``).toUpperCase(),n=e.req.header(`X-Session-Token`)||``;if(n){let r=await e.env.LOOKBOOK_DB.prepare(`SELECT u.locale, u.country, u.currency FROM user_sessions s
       JOIN users u ON u.id = s.user_id
       WHERE s.token = ? AND s.expires_at > datetime('now')`).bind(n).first();if(r?.locale)return e.json({country:r.country||t,locale:r.locale,currency:r.currency||`USD`,pg:r.locale===`ko`?`toss`:`stripe`,messenger:r.locale===`ko`?`kakao`:r.locale===`ja`?`line`:`web-share`})}let r=Rn(t);return e.json({country:t,...r})}),V.put(`/api/user/locale`,async e=>{let t=e.env.LOOKBOOK_DB,n=e.req.header(`X-Session-Token`)||``;if(!n)return e.json({success:!1,message:`로그인이 필요합니다.`},401);let r=await t.prepare(`SELECT user_id FROM user_sessions WHERE token = ? AND expires_at > datetime('now')`).bind(n).first();if(!r)return e.json({success:!1,message:`세션이 만료되었습니다.`},401);let i=await e.req.json(),a=[`ko`,`en`,`ja`].includes(i?.locale)?i.locale:null;if(!a)return e.json({success:!1,message:`지원하지 않는 언어입니다.`},400);let o=typeof i?.country==`string`?i.country.slice(0,8):null,s=typeof i?.currency==`string`?i.currency.slice(0,8):null;return await t.prepare(`UPDATE users SET locale = ?, country = ?, currency = ?, updated_at = datetime('now') WHERE id = ?`).bind(a,o,s,r.user_id).run(),e.json({success:!0})}),V.get(`/api/config/kakao-js-key`,e=>e.json({key:e.env.KAKAO_JS_KEY||``}));function zn(){let e=new Uint8Array(32);return crypto.getRandomValues(e),Array.from(e).map(e=>e.toString(16).padStart(2,`0`)).join(``)}function Bn(){return`u_`+zn().substring(0,16)}async function Vn(e){let t=zn().substring(0,16),n=new TextEncoder().encode(t+e),r=await crypto.subtle.digest(`SHA-256`,n);return`${t}:${Array.from(new Uint8Array(r)).map(e=>e.toString(16).padStart(2,`0`)).join(``)}`}async function Hn(e,t){let[n,r]=t.split(`:`),i=new TextEncoder().encode(n+e),a=await crypto.subtle.digest(`SHA-256`,i);return Array.from(new Uint8Array(a)).map(e=>e.toString(16).padStart(2,`0`)).join(``)===r}async function Un(e,t){if(!t)return null;let n=new Date().toISOString();return await e.prepare(`
    SELECT u.id, u.name, u.email, u.role, u.status, u.credits, u.avatar_url, u.provider, u.referrer
    FROM user_sessions s JOIN users u ON s.user_id = u.id
    WHERE s.token = ? AND s.expires_at > ? AND u.status = 'active'
  `).bind(t,n).first()||null}async function Wn(e,t){let n=zn(),r=new Date(Date.now()+720*60*60*1e3).toISOString();return await e.prepare(`DELETE FROM user_sessions WHERE user_id = ? AND token NOT IN (
    SELECT token FROM user_sessions WHERE user_id = ? ORDER BY created_at DESC LIMIT 4
  )`).bind(t,t).run(),await e.prepare(`INSERT INTO user_sessions (token, user_id, expires_at) VALUES (?, ?, ?)`).bind(n,t,r).run(),n}function Gn(e){return{id:e.id,name:e.name,email:e.email,role:e.role,credits:e.credits,avatar_url:e.avatar_url,provider:e.provider,referrer:e.referrer??null}}function Kn(e){let t=e instanceof Uint8Array?e:new Uint8Array(e),n=``;for(let e of t)n+=String.fromCharCode(e);return btoa(n).replace(/\+/g,`-`).replace(/\//g,`_`).replace(/=+$/,``)}async function qn(e,t){let n={alg:`HS256`,typ:`JWT`},r=Math.floor(Date.now()/1e3),i={...t,iat:r,exp:r+60},a=new TextEncoder,o=`${Kn(a.encode(JSON.stringify(n)))}.${Kn(a.encode(JSON.stringify(i)))}`,s=await crypto.subtle.importKey(`raw`,a.encode(e),{name:`HMAC`,hash:`SHA-256`},!1,[`sign`]);return`${o}.${Kn(await crypto.subtle.sign(`HMAC`,s,a.encode(o)))}`}function Jn(e){let t=e.replace(/-/g,`+`).replace(/_/g,`/`)+`=`.repeat((4-e.length%4)%4),n=atob(t),r=new Uint8Array(n.length);for(let e=0;e<n.length;e++)r[e]=n.charCodeAt(e);return r}async function Yn(e,t){let n=t.split(`.`);if(n.length!==3)return null;let[r,i,a]=n,o=new TextEncoder;try{let t=await crypto.subtle.importKey(`raw`,o.encode(e),{name:`HMAC`,hash:`SHA-256`},!1,[`verify`]);if(!await crypto.subtle.verify(`HMAC`,t,Jn(a),o.encode(`${r}.${i}`)))return null;let n=JSON.parse(new TextDecoder().decode(Jn(i)));return!n?.sub||typeof n.exp!=`number`||n.exp<Math.floor(Date.now()/1e3)?null:{sub:String(n.sub)}}catch{return null}}var Xn=[`BFM회원`,`코오롱 FnC`,`한섬`],Zn=500,Qn={BFM회원:.2};V.post(`/api/auth/signup`,async e=>{try{let t=e.env.LOOKBOOK_DB,n=await e.req.json(),{name:r,email:i,password:a,agreeMarketing:o}=n,s=Xn.includes(n?.referrer)?n.referrer:null;if(!r||!i||!a)return e.json({success:!1,message:`모든 항목을 입력해주세요.`},400);if(a.length<8)return e.json({success:!1,message:`비밀번호는 8자 이상이어야 합니다.`},400);if(await t.prepare(`SELECT id FROM users WHERE email = ?`).bind(i).first())return e.json({success:!1,message:`이미 가입된 이메일입니다.`},409);let l=Bn(),u=await Vn(a),d=+!!o,f=s===`BFM회원`?Zn:200;await t.prepare(`
    ...
    `).bind(l,i.toLowerCase(),r,u,f,d,s).run(),await t.prepare(`INSERT INTO credit_logs (user_id, type, amount, balance, reason, ref_id)
       VALUES (?, 'grant', ?, ?, 'signup_bonus', ?)`).bind(l,f,f,`signup_${l}`).run();let p=await Wn(t,l),m={id:l,name:r,email:i.toLowerCase(),role:`user`,credits:f,avatar_url:null,provider:`email`,referrer:s};return e.json({success:!0,user:m,token:p})}catch(t){return console.error(`signup error:`,t),e.json({success:!1,message:`서버 오류가 발생했습니다.`},500)}}),V.post(`/api/auth/login`,async e=>{try{let t=e.env.LOOKBOOK_DB,{email:n,password:r}=await e.req.json();if(!n||!r)return e.json({success:!1,message:`이메일과 비밀번호를 입력해주세요.`},400);let i=await t.prepare(`SELECT * FROM users WHERE email = ? AND provider = 'email'`).bind(n.toLowerCase()).first();if(!i)return e.json({success:!1,message:`이메일 또는 비밀번호가 올바르지 않습니다.`},401);if(i.status!==`active`)return e.json({success:!1,message:`정지된 계정입니다. 관리자에게 문의하세요.`},403);if(!i.password_hash)return e.json({success:!1,message:`소셜 계정으로 가입된 이메일입니다.`},400);if(!await Hn(r,i.password_hash))return e.json({success:!1,message:`이메일 또는 비밀번호가 올바르지 않습니다.`},401);await t.prepare(`UPDATE users SET last_login_at = datetime('now') WHERE id = ?`).bind(i.id).run();let a=await Wn(t,i.id);return e.json({success:!0,user:Gn(i),token:a})}catch(t){return console.error(`login error:`,t),e.json({success:!1,message:`서버 오류가 발생했습니다.`},500)}}),V.get(`/api/auth/me`,async e=>{try{let t=e.env.LOOKBOOK_DB,n=await Un(t,e.req.header(`X-Session-Token`)||e.req.query(`token`)||null);return n?e.json({success:!0,user:Gn(n)}):e.json({success:!1,message:`로그인이 필요합니다.`},401)}catch{return e.json({success:!1,message:`서버 오류`},500)}}),V.get(`/api/techpack/handoff-token`,async e=>{try{let t=e.env.LOOKBOOK_DB,n=await Un(t,e.req.header(`X-Session-Token`)||e.req.query(`token`)||null);if(!n)return e.json({success:!1,message:`로그인이 필요합니다.`},401);let r=await qn(e.env.TECHPACK_HANDOFF_SECRET,{sub:n.id,email:n.email,name:n.name});return e.json({success:!0,token:r})}catch{return e.json({success:!1,message:`서버 오류`},500)}}),V.get(`/api/techpack/upload-history`,async e=>{try{let t=e.req.header(`authorization`)||``,n=t.startsWith(`Bearer `)?t.slice(7):null,r=n?await Yn(e.env.TECHPACK_HANDOFF_SECRET,n):null;if(!r)return e.json({success:!1,message:`인증에 실패했습니다.`},401);let i=e.env.LOOKBOOK_DB,a=e.env?.LOOKBOOK_KV,o=$(e),s=await i.prepare(`SELECT job_id, model_name, created_at FROM generation_logs
       WHERE user_id = ? AND (kind IS NULL OR kind != 'video') ORDER BY created_at DESC LIMIT 30`).bind(r.sub).all(),l=[];if(a)for(let e of s.results||[])try{let t=await a.get(`clothing_img:${e.job_id}`);if(!t)continue;let n=JSON.parse(t);if(!Array.isArray(n)||n.length===0)continue;let r=!!(e.model_name&&String(e.model_name).startsWith(`고스트컷`));l.push({job_id:e.job_id,created_at:e.created_at,label:r?`누끼컷`:`모델컷`,images:n.map((t,n)=>`${o}/api/proxy/clothing/${encodeURIComponent(e.job_id)}/${n}`)})}catch{}return e.json({success:!0,uploads:l})}catch(t){return console.error(`techpack/upload-history error:`,t),e.json({success:!1,message:`서버 오류`},500)}}),V.get(`/api/techpack/account-summary`,async e=>{try{let t=e.req.header(`authorization`)||``,n=t.startsWith(`Bearer `)?t.slice(7):null,r=n?await Yn(e.env.TECHPACK_HANDOFF_SECRET,n):null;if(!r)return e.json({success:!1,message:`인증에 실패했습니다.`},401);let i=await e.env.LOOKBOOK_DB.prepare(`SELECT name, email, credits FROM users WHERE id = ?`).bind(r.sub).first();return i?e.json({success:!0,name:i.name,email:i.email,credits:i.credits}):e.json({success:!1,message:`사용자를 찾을 수 없습니다.`},404)}catch(t){return console.error(`techpack/account-summary error:`,t),e.json({success:!1,message:`서버 오류`},500)}}),V.post(`/api/auth/logout`,async e=>{try{let t=e.env.LOOKBOOK_DB,n=e.req.header(`X-Session-Token`);return n&&await t.prepare(`DELETE FROM user_sessions WHERE token = ?`).bind(n).run(),e.json({success:!0})}catch{return e.json({success:!0})}});function $(e){let t=e.req.header(`host`)||e.req.header(`x-forwarded-host`)||``;return`${t.startsWith(`localhost`)?`http`:`https`}://${t}`}V.get(`/api/auth/kakao`,e=>{let t=$(e),n=e.req.query(`mode`)||`popup`,r=`${t}/api/auth/kakao/callback`,i=e.env.KAKAO_CLIENT_ID||``;if(!i)return n===`redirect`?e.redirect(`/?oauth_error=kakao_no_key`):e.html(`<script>window.opener?.postMessage({type:'oauth_error',provider:'kakao',error:'카카오 앱 키가 설정되지 않았습니다.'},'*');window.close();<\/script>`);let a=`https://kauth.kakao.com/oauth/authorize?client_id=${i}&redirect_uri=${encodeURIComponent(r)}&response_type=code&state=${n}&scope=profile_nickname,account_email,plusfriends`;return e.redirect(a)}),V.get(`/api/auth/kakao/callback`,async e=>{let t=e.env.LOOKBOOK_DB,n=$(e),r=e.req.query(`code`),i=e.req.query(`error`),a=e.req.query(`state`)||`popup`;function o(t){return a===`redirect`?e.redirect(`/?oauth_error=${encodeURIComponent(t)}`):e.html(`<script>window.opener?.postMessage({type:'oauth_error',provider:'kakao',error:'${t}'},'*');window.close();<\/script>`)}if(i||!r)return o(i||`cancelled`);try{let i=`${n}/api/auth/kakao/callback`,o=await(await fetch(`https://kauth.kakao.com/oauth/token`,{method:`POST`,headers:{"Content-Type":`application/x-www-form-urlencoded`},body:new URLSearchParams({grant_type:`authorization_code`,code:r,client_id:e.env.KAKAO_CLIENT_ID||``,client_secret:e.env.KAKAO_CLIENT_SECRET||``,redirect_uri:i})})).json();if(!o.access_token)throw Error(`카카오 토큰 발급 실패`);let s=await(await fetch(`https://kapi.kakao.com/v2/user/me`,{headers:{Authorization:`Bearer ${o.access_token}`}})).json(),l=String(s.id),u=s.kakao_account?.email||`kakao_${l}@kakao.local`,d=s.kakao_account?.profile?.nickname||`카카오 사용자`,f=s.kakao_account?.profile?.profile_image_url||null,p=s.kakao_account?.phone_number||null,m=!1,h=await t.prepare(`SELECT * FROM users WHERE provider = 'kakao' AND provider_id = ?`).bind(l).first();if(h){let e=[],n=[];p&&h.phone_number!==p&&(e.push(`phone_number = ?`),n.push(p),h.phone_number=p),h.name===`카카오 사용자`&&d!==`카카오 사용자`&&(e.push(`name = ?`),n.push(d),h.name=d),!h.avatar_url&&f&&(e.push(`avatar_url = ?`),n.push(f),h.avatar_url=f),e.length&&(n.push(h.id),await t.prepare(`UPDATE users SET ${e.join(`, `)} WHERE id = ?`).bind(...n).run())}else if(h=await t.prepare(`SELECT * FROM users WHERE email = ?`).bind(u).first(),h)await t.prepare(`UPDATE users SET provider_id = ?, avatar_url = ?, phone_number = COALESCE(?, phone_number) WHERE id = ?`).bind(l,f,p,h.id).run();else{let e=Bn();await t.prepare(`INSERT INTO users (id, email, name, provider, provider_id, avatar_url, phone_number, status, credits, role) VALUES (?, ?, ?, 'kakao', ?, ?, ?, 'active', 200, 'user')`).bind(e,u,d,l,f,p).run(),await t.prepare(`INSERT INTO credit_logs (user_id, type, amount, balance, reason, ref_id)
           VALUES (?, 'grant', 200, 200, 'signup_bonus', ?)`).bind(e,`signup_${e}`).run(),h=await t.prepare(`SELECT * FROM users WHERE id = ?`).bind(e).first(),m=!0}if(!h||h.status!==`active`)throw Error(`계정이 정지 상태입니다.`);await t.prepare(`UPDATE users SET last_login_at = datetime('now') WHERE id = ?`).bind(h.id).run();let g=await Wn(t,h.id),_=JSON.stringify(Gn(h));return a===`redirect`?e.html(`<!DOCTYPE html>
<html lang="ko">
...
<\/script>
       ...
       FROM payment_logs WHERE user_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?`).bind(n,i,a).all();return e.json({success:!0,payments:s.results,total:o?.cnt||0,page:r,limit:i})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.get(`/api/admin/users/:id/generations`,W,async e=>{try{let t=e.env.LOOKBOOK_DB,n=e.env?.LOOKBOOK_KV,r=e.req.param(`id`),i=Math.max(1,parseInt(e.req.query(`page`)||`1`)),a=Math.max(1,parseInt(e.req.query(`limit`)||`10`)),o=(i-1)*a,s=await t.prepare(`SELECT COUNT(*) as cnt FROM generation_logs WHERE user_id = ?`).bind(r).first(),l=await t.prepare(`SELECT id, job_id, image_count, model_name, bg_name, kind, video_url, image_urls, expires_at, created_at, downloaded_indices
       FROM generation_logs WHERE user_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?`).bind(r,a,o).all(),u=Date.now(),d=await Promise.all((l.results||[]).map(async e=>{let t=!e.expires_at||new Date(String(e.expires_at).replace(` `,`T`)+`Z`).getTime()<u,r=0;if(n&&e.job_id)try{let t=await n.get(`clothing_img:${e.job_id}`);if(t){let e=JSON.parse(t);r=Array.isArray(e)?e.length:0}}catch{}return{id:e.id,job_id:e.job_id,image_count:e.image_count,model_name:e.model_name,bg_name:e.bg_name,kind:e.kind,created_at:e.created_at,expires_at:e.expires_at,expired:t,video_url:t?null:e.video_url,image_urls:t?null:e.image_urls,downloaded_indices:e.downloaded_indices,clothing_count:r}}));return e.json({success:!0,generations:d,total:s?.cnt||0,page:i,limit:a})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.get(`/api/admin/users/:id/credit-events`,W,async e=>{try{let t=e.env.LOOKBOOK_DB,n=e.req.param(`id`),r=Math.max(1,parseInt(e.req.query(`page`)||`1`)),i=Math.max(1,parseInt(e.req.query(`limit`)||`10`)),a=(r-1)*i,o=await t.prepare(`SELECT COUNT(*) as cnt FROM credit_logs WHERE user_id = ?`).bind(n).first(),s=await t.prepare(`SELECT id, type, amount, balance, reason, ref_id, created_at
       FROM credit_logs WHERE user_id = ? ORDER BY created_at DESC, id DESC LIMIT ? OFFSET ?`).bind(n,i,a).all();return e.json({success:!0,events:s.results,total:o?.cnt||0,page:r,limit:i})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.patch(`/api/admin/users/:id`,W,async e=>{try{let t=e.env.LOOKBOOK_DB,n=await e.req.json(),r=e.req.param(`id`),i=[],a=[];if(n.status!==void 0&&(i.push(`status = ?`),a.push(n.status)),n.role!==void 0&&(i.push(`role = ?`),a.push(n.role)),n.set_password!==void 0){if(typeof n.set_password!=`string`||n.set_password.length<6)return e.json({success:!1,message:`비밀번호는 6자 이상이어야 합니다.`},400);i.push(`password_hash = ?`),a.push(await Vn(n.set_password))}if(n.add_credits!==void 0){let o=(await t.prepare(`SELECT credits FROM users WHERE id = ?`).bind(r).first())?.credits??0,s=parseInt(n.add_credits),l=Math.max(0,o+s);return i.push(`credits = ?`),a.push(l),i.push(`updated_at = datetime('now')`),await t.prepare(`UPDATE users SET ${i.join(`, `)} WHERE id = ?`).bind(...a,r).run(),await t.prepare(`INSERT INTO credit_logs (user_id, type, amount, balance, reason, ref_id)
         VALUES (?, 'grant', ?, ?, 'admin_grant', ?)`).bind(r,s,l,`admin_${Date.now()}`).run(),n.status===`suspended`&&await t.prepare(`DELETE FROM user_sessions WHERE user_id = ?`).bind(r).run(),e.json({success:!0,newCredits:l})}if(n.credits!==void 0){let o=(await t.prepare(`SELECT credits FROM users WHERE id = ?`).bind(r).first())?.credits??0,s=parseInt(n.credits),l=s-o;return i.push(`credits = ?`),a.push(s),i.push(`updated_at = datetime('now')`),await t.prepare(`UPDATE users SET ${i.join(`, `)} WHERE id = ?`).bind(...a,r).run(),l!==0&&await t.prepare(`INSERT INTO credit_logs (user_id, type, amount, balance, reason, ref_id)
           VALUES (?, ?, ?, ?, 'admin_set', ?)`).bind(r,l>0?`grant`:`deduct`,l,s,`admin_${Date.now()}`).run(),n.status===`suspended`&&await t.prepare(`DELETE FROM user_sessions WHERE user_id = ?`).bind(r).run(),e.json({success:!0,newCredits:s})}return i.length===0?e.json({success:!1,message:`변경할 항목이 없습니다.`},400):(i.push(`updated_at = datetime('now')`),await t.prepare(`UPDATE users SET ${i.join(`, `)} WHERE id = ?`).bind(...a,r).run(),n.status===`suspended`&&await t.prepare(`DELETE FROM user_sessions WHERE user_id = ?`).bind(r).run(),e.json({success:!0}))}catch(t){return e.json({success:!1,message:t.message},500)}}),V.delete(`/api/admin/users/:id`,W,async e=>{try{let t=e.env.LOOKBOOK_DB,n=e.req.param(`id`);return await t.prepare(`UPDATE users SET status = 'deleted', updated_at = datetime('now') WHERE id = ?`).bind(n).run(),await t.prepare(`DELETE FROM user_sessions WHERE user_id = ?`).bind(n).run(),e.json({success:!0})}catch(t){return e.json({success:!1,message:t.message},500)}});var $n=.2,er=async(e,t)=>{let n=e.env.LOOKBOOK_DB,r=await Un(n,e.req.header(`X-Session-Token`)||null);if(!r)return e.json({success:!1,message:`로그인이 필요합니다.`},401);if(r.role!==`bfm_admin`)return e.json({success:!1,message:`BFM 관리자 권한이 없습니다.`},403);await t()};V.post(`/api/bfm/login`,async e=>{try{let t=e.env.LOOKBOOK_DB,n=await e.req.json(),r=(n.email||``).trim().toLowerCase(),i=n.password||``;if(!r||!i)return e.json({success:!1,message:`이메일과 비밀번호를 입력해주세요.`},400);let a=await t.prepare(`SELECT * FROM users WHERE email = ?`).bind(r).first();if(!a||a.status!==`active`||!a.password_hash||!await Hn(i,a.password_hash))return e.json({success:!1,message:`이메일 또는 비밀번호가 올바르지 않습니다.`},401);if(a.role!==`bfm_admin`)return e.json({success:!1,message:`BFM 관리자 권한이 없습니다.`},403);let o=await Wn(t,a.id);return e.json({success:!0,token:o,user:Gn(a)})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.get(`/api/bfm/members`,er,async e=>{try{let t=e.env.LOOKBOOK_DB,n=Math.max(1,parseInt(e.req.query(`page`)||`1`)),r=Math.max(1,Math.min(50,parseInt(e.req.query(`limit`)||`20`))),i=(n-1)*r,a=await t.prepare(`SELECT COUNT(*) as cnt FROM users WHERE referrer = 'BFM회원'`).first(),o=((await t.prepare(`SELECT u.id, u.name, u.email, u.credits, u.status, u.created_at,
              COALESCE((SELECT SUM(amount) FROM payment_logs WHERE user_id = u.id AND status = 'paid'), 0) AS total_paid
       FROM users u WHERE u.referrer = 'BFM회원' ORDER BY u.created_at DESC LIMIT ? OFFSET ?`).bind(r,i).all()).results||[]).map(e=>({...e,bfm_revenue:Math.round((e.total_paid||0)*$n)}));return e.json({success:!0,members:o,total:a?.cnt||0,page:n,limit:r})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.get(`/api/bfm/members/:id/payments`,er,async e=>{try{let t=e.env.LOOKBOOK_DB,n=e.req.param(`id`);if(!await t.prepare(`SELECT id FROM users WHERE id = ? AND referrer = 'BFM회원'`).bind(n).first())return e.json({success:!1,message:`조회 권한이 없는 회원입니다.`},403);let r=Math.max(1,parseInt(e.req.query(`page`)||`1`)),i=Math.max(1,parseInt(e.req.query(`limit`)||`10`)),a=(r-1)*i,o=await t.prepare(`SELECT COUNT(*) as cnt FROM payment_logs WHERE user_id = ?`).bind(n).first(),s=await t.prepare(`SELECT order_id, amount, credits, status, pg_provider, currency, pg_method, created_at, paid_at
       FROM payment_logs WHERE user_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?`).bind(n,i,a).all();return e.json({success:!0,payments:s.results,total:o?.cnt||0,page:r,limit:i})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.get(`/api/admin/stats`,W,async e=>{try{let t=e.env.LOOKBOOK_DB,n=await t.prepare(`SELECT COUNT(*) as cnt FROM users WHERE status != 'deleted'`).first(),r=await t.prepare(`SELECT COUNT(*) as cnt FROM users WHERE status = 'active'`).first(),i=await t.prepare(`SELECT COUNT(*) as cnt FROM users WHERE status = 'suspended'`).first(),a=await t.prepare(`SELECT COUNT(*) as cnt FROM users WHERE date(created_at) = date('now') AND status != 'deleted'`).first(),o=await t.prepare(`SELECT COUNT(*) as cnt FROM users WHERE provider = 'kakao' AND status = 'active'`).first(),s=await t.prepare(`SELECT COUNT(*) as cnt FROM users WHERE provider = 'google' AND status = 'active'`).first(),l=await t.prepare(`SELECT COUNT(*) as cnt FROM users WHERE provider = 'email' AND status = 'active'`).first();return e.json({success:!0,stats:{total:n?.cnt||0,active:r?.cnt||0,suspended:i?.cnt||0,today:a?.cnt||0,by_provider:{kakao:o?.cnt||0,google:s?.cnt||0,email:l?.cnt||0}}})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.get(`/api/credits/history`,async e=>{try{let t=e.env.LOOKBOOK_DB,n=e.req.header(`X-Session-Token`)||``;if(!n)return e.json({error:`로그인이 필요합니다.`},401);let r=await t.prepare(`SELECT s.user_id, u.credits FROM user_sessions s
       JOIN users u ON u.id = s.user_id
       ...
       JOIN users u ON u.id = s.user_id
       WHERE s.token = ? AND s.expires_at > datetime('now')`).bind(n).first();if(!r)return e.json({error:`세션이 만료되었습니다.`,code:`UNAUTHORIZED`},401);let i=await e.req.json().catch(()=>({})),a=i?.job_id,o=Number.isInteger(i?.idx)?i.idx:void 0,s=null,l=[];if(a){if(s=await t.prepare(`SELECT id, downloaded_indices, model_name FROM generation_logs WHERE job_id = ? AND user_id = ? ORDER BY id DESC LIMIT 1`).bind(a,r.user_id).first(),s?.downloaded_indices)try{l=JSON.parse(s.downloaded_indices)}catch{}if(s&&o!==void 0&&l.includes(o))return e.json({success:!0,creditsUsed:0,creditsRemaining:r.credits,alreadyDownloaded:!0})}let u=!!(s?.model_name&&String(s.model_name).startsWith(`고스트컷디테일·`)),d=!!(s?.model_name&&String(s.model_name).startsWith(`고스트컷·`)),f=u?dr:d?lr:cr;if(r.credits<f)return e.json({error:`크레딧이 부족합니다. (보유: ${r.credits}크레딧, 필요: ${f}크레딧)`,code:`INSUFFICIENT_CREDITS`,available:r.credits,required:f},402);let p=r.credits-f;return f>0&&(await t.prepare(`UPDATE users SET credits = ?, updated_at = datetime('now') WHERE id = ?`).bind(p,r.user_id).run(),await t.prepare(`INSERT INTO credit_logs (user_id, type, amount, balance, reason, ref_id)
         VALUES (?, 'deduct', ?, ?, 'image_download', ?)`).bind(r.user_id,-f,p,`dl_${Date.now()}`).run()),s&&o!==void 0&&(l.push(o),await t.prepare(`UPDATE generation_logs SET downloaded_indices = ? WHERE id = ?`).bind(JSON.stringify(l),s.id).run()),console.log(`[Credits] Download deduct: ${r.name} ${r.credits} → ${p} (-${f})`),e.json({success:!0,creditsUsed:f,creditsRemaining:p,alreadyDownloaded:!1})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.delete(`/api/generation/history/:id`,async e=>{try{let t=e.env.LOOKBOOK_DB,n=e.req.header(`X-Session-Token`)||``;if(!n)return e.json({error:`로그인이 필요합니다.`},401);let r=await t.prepare(`SELECT user_id FROM user_sessions WHERE token = ? AND expires_at > datetime('now')`).bind(n).first();if(!r)return e.json({error:`세션이 만료되었습니다.`},401);let i=e.req.param(`id`);return await t.prepare(`DELETE FROM generation_logs WHERE id = ? AND user_id = ?`).bind(i,r.user_id).run(),e.json({success:!0})}catch(t){return e.json({success:!1,message:t.message},500)}});var tr={pkg_20000:{amount:2e4,credits:1e3,label:`20,000원 → 1,000크레딧`,usdCents:1999,jpyAmount:2980},pkg_40000:{amount:4e4,credits:2300,label:`40,000원 → 2,300크레딧`,usdCents:3499,jpyAmount:4980},pkg_60000:{amount:6e4,credits:4e3,label:`60,000원 → 4,000크레딧`,usdCents:4999,jpyAmount:7980}};V.get(`/api/payments/packages`,e=>e.json({success:!0,packages:tr})),V.post(`/api/payments/prepare`,async e=>{try{let t=e.env.LOOKBOOK_DB,n=e.req.header(`X-Session-Token`)||``;if(!n)return e.json({error:`로그인이 필요합니다.`},401);let r=await t.prepare(`SELECT s.user_id, u.name, u.email, u.referrer FROM user_sessions s
       JOIN users u ON u.id = s.user_id
       WHERE s.token = ? AND s.expires_at > datetime('now')`).bind(n).first();if(!r)return e.json({error:`세션이 만료되었습니다.`},401);let{packageId:i}=await e.req.json(),a=tr[i];if(!a)return e.json({error:`잘못된 패키지입니다.`},400);let o=Qn[r.referrer]||0,s=o>0?Math.round(a.amount*(1-o)):a.amount,l=await t.prepare(`SELECT order_id FROM payment_logs
       WHERE user_id = ? AND amount = ? AND status = 'pending'
         AND created_at > datetime('now', '-5 minutes')
       ORDER BY created_at DESC LIMIT 1`).bind(r.user_id,s).first();if(l?.order_id)return e.json({success:!0,orderId:l.order_id,amount:s,credits:a.credits,orderName:a.label,customerName:r.name,customerEmail:r.email,userId:r.user_id,clientKey:e.env.TOSS_CLIENT_KEY||``});let u=`lookbook-${r.user_id.slice(0,6)}-${Date.now()}-${Math.random().toString(36).slice(2,6).toUpperCase()}`;return await t.prepare(`INSERT INTO payment_logs (user_id, order_id, amount, credits, status, pg_provider, currency)
       VALUES (?, ?, ?, ?, 'pending', 'toss', 'KRW')`).bind(r.user_id,u,s,a.credits).run(),e.json({success:!0,orderId:u,amount:s,credits:a.credits,orderName:a.label,customerName:r.name,customerEmail:r.email,userId:r.user_id,clientKey:e.env.TOSS_CLIENT_KEY||``})}catch(t){return e.json({success:!1,message:t.message},500)}});async function nr(e,t){let n=await crypto.subtle.importKey(`raw`,new TextEncoder().encode(e),{name:`HMAC`,hash:`SHA-256`},!1,[`sign`]),r=await crypto.subtle.sign(`HMAC`,n,new TextEncoder().encode(t));return Array.from(new Uint8Array(r)).map(e=>e.toString(16).padStart(2,`0`)).join(``)}V.post(`/api/stripe/checkout`,async e=>{let t=e.env.STRIPE_SECRET_KEY;if(!t)return e.json({success:!1,message:`서버 설정 오류: STRIPE_SECRET_KEY 미설정`},500);let n=e.env.LOOKBOOK_DB,r=e.req.header(`X-Session-Token`)||``;if(!r)return e.json({error:`로그인이 필요합니다.`},401);let i=await n.prepare(`SELECT s.user_id, u.email, u.referrer FROM user_sessions s
     JOIN users u ON u.id = s.user_id
     WHERE s.token = ? AND s.expires_at > datetime('now')`).bind(r).first();if(!i)return e.json({error:`세션이 만료되었습니다.`},401);try{let{packageId:r,currency:a}=await e.req.json(),o=tr[r];if(!o)return e.json({error:`잘못된 패키지입니다.`},400);let s=String(a||`USD`).toUpperCase();if(s!==`USD`&&s!==`JPY`)return e.json({error:`지원하지 않는 통화입니다.`},400);let l=s===`USD`?o.usdCents:o.jpyAmount,u=Qn[i.referrer]||0,d=u>0?Math.round(l*(1-u)):l,f=`lookbook-${i.user_id.slice(0,6)}-${Date.now()}-${Math.random().toString(36).slice(2,6).toUpperCase()}`;await n.prepare(`INSERT INTO payment_logs (user_id, order_id, amount, credits, status, pg_provider, currency)
       VALUES (?, ?, ?, ?, 'pending', 'stripe', ?)`).bind(i.user_id,f,d,o.credits,s).run();let p=new URL(e.req.url).origin,m=new URLSearchParams;m.set(`mode`,`payment`),m.set(`success_url`,`${p}/payment/stripe/return?order_id=${f}`),m.set(`cancel_url`,`${p}/payment/fail`),m.set(`client_reference_id`,f),i.email&&!String(i.email).endsWith(`@kakao.local`)&&m.set(`customer_email`,i.email),m.set(`line_items[0][price_data][currency]`,s.toLowerCase()),m.set(`line_items[0][price_data][product_data][name]`,`${o.label.split(` → `)[1]||o.label} (${o.credits.toLocaleString()} credits)`),m.set(`line_items[0][price_data][product_data][tax_code]`,`txcd_10000000`),m.set(`line_items[0][price_data][unit_amount]`,String(d)),m.set(`line_items[0][quantity]`,`1`),m.set(`metadata[order_id]`,f),m.set(`managed_payments[enabled]`,`false`);let h=await fetch(`https://api.stripe.com/v1/checkout/sessions`,{method:`POST`,headers:{Authorization:`Bearer ${t}`,"Content-Type":`application/x-www-form-urlencoded`},body:m.toString()}),g=await h.json();return h.ok?e.json({success:!0,url:g.url}):(console.error(`Stripe checkout session error:`,g),e.json({success:!1,message:g?.error?.message||`Stripe 결제 세션 생성 실패`},500))}catch(t){return e.json({success:!1,message:t.message},500)}}),V.get(`/payment/stripe/return`,async e=>{let t=e.req.query(`order_id`)||``;return e.redirect(`/payment/success?orderId=${encodeURIComponent(t)}`,302)}),V.post(`/payment/stripe/webhook`,async e=>{let t=e.env.STRIPE_WEBHOOK_SECRET,n=e.env.LOOKBOOK_DB;try{if(!t)throw Error(`STRIPE_WEBHOOK_SECRET 미설정`);let r=e.req.header(`stripe-signature`)||``,i=await e.req.text(),a=Object.fromEntries(r.split(`,`).map(e=>e.split(`=`))),o=a.t,s=a.v1;if(!o||!s)throw Error(`서명 헤더 형식 오류`);if(await nr(t,`${o}.${i}`)!==s)throw Error(`서명 불일치`);let l=JSON.parse(i);if(l.type===`checkout.session.completed`){let e=l.data.object,t=e.client_reference_id||e.metadata?.order_id;if(t){let r=await n.prepare(`SELECT id, user_id, credits, status FROM payment_logs WHERE order_id = ? AND pg_provider = 'stripe'`).bind(t).first();if(r&&r.status===`pending`){await n.prepare(`UPDATE payment_logs SET status='paid', payment_key=?, pg_raw=?, paid_at=datetime('now') WHERE id=?`).bind(e.payment_intent||e.id,JSON.stringify(e),r.id).run();let i=((await n.prepare(`SELECT credits FROM users WHERE id=?`).bind(r.user_id).first())?.credits??0)+r.credits;await n.prepare(`UPDATE users SET credits=?, updated_at=datetime('now') WHERE id=?`).bind(i,r.user_id).run(),await n.prepare(`INSERT INTO credit_logs (user_id, type, amount, balance, reason, ref_id) VALUES (?, 'grant', ?, ?, 'payment', ?)`).bind(r.user_id,r.credits,i,t).run(),console.log(`[Stripe] Payment completed: ${t} → +${r.credits} credits (user ${r.user_id})`)}}}else if(l.type===`charge.refunded`||l.type===`payment_intent.canceled`){let e=l.data.object,t=e.payment_intent||e.id,r=await n.prepare(`SELECT id, user_id, credits, status FROM payment_logs WHERE payment_key = ? AND pg_provider = 'stripe'`).bind(t).first();if(r&&r.status===`paid`){let e=(await n.prepare(`SELECT credits FROM users WHERE id=?`).bind(r.user_id).first())?.credits??0,i=Math.min(r.credits,e),a=e-i;await n.prepare(`UPDATE users SET credits=?, updated_at=datetime('now') WHERE id=?`).bind(a,r.user_id).run(),await n.prepare(`INSERT INTO credit_logs (user_id, type, amount, balance, reason, ref_id) VALUES (?, 'deduct', ?, ?, 'payment_refund', ?)`).bind(r.user_id,-i,a,t).run(),await n.prepare(`UPDATE payment_logs SET status='canceled' WHERE id=?`).bind(r.id).run(),console.log(`[Stripe] Refund clawback: ${t} → -${i} credits (user ${r.user_id})`)}}return e.text(`OK`,200)}catch(t){return console.error(`Stripe webhook error:`,t.message),e.text(`Bad Request`,400)}}),V.get(`/payment/toss/success`,async e=>{let t=e.env.LOOKBOOK_DB;try{let n=e.req.query(`paymentKey`)||``,r=e.req.query(`orderId`)||``,i=e.req.query(`amount`)||``,a=await t.prepare(`SELECT id, user_id, amount, credits, status FROM payment_logs
       WHERE order_id = ? AND status = 'pending'`).bind(r).first();if(!a)return e.redirect(`/payment/fail?message=%EA%B2%B0%EC%A0%9C%20%EC%A0%95%EB%B3%B4%EB%A5%BC%20%EC%B0%BE%EC%9D%84%20%EC%88%98%20%EC%97%86%EA%B1%B0%EB%82%98%20%EC%9D%B4%EB%AF%B8%20%EC%B2%98%EB%A6%AC%EB%90%98%EC%97%88%EC%8A%B5%EB%8B%88%EB%8B%A4.`,302);if(Number(i)!==Number(a.amount))return e.redirect(`/payment/fail?message=%EA%B2%B0%EC%A0%9C%20%EA%B8%88%EC%95%A1%EC%9D%B4%20%EC%9D%BC%EC%B9%98%ED%95%98%EC%A7%80%20%EC%95%8A%EC%8A%B5%EB%8B%88%EB%8B%A4.`,302);let o=e.env.TOSS_API_BASE||`https://api.tosspayments.com`,s=e.env.TOSS_SECRET_KEY||``,l=`Basic `+btoa(`${s}:`),u=await fetch(`${o}/v1/payments/confirm`,{method:`POST`,headers:{Authorization:l,"Content-Type":`application/json`},body:JSON.stringify({paymentKey:n,orderId:r,amount:Number(i)})}),d=await u.json();if(!u.ok)return await t.prepare(`UPDATE payment_logs
         SET status='failed', pg_raw=?, paid_at=datetime('now')
         WHERE order_id=?`).bind(JSON.stringify(d),r).run(),e.redirect(`/payment/fail?message=${encodeURIComponent(d.message||`결제 승인 실패`)}&code=${encodeURIComponent(d.code||``)}`,302);await t.prepare(`UPDATE payment_logs
       SET status='paid', payment_key=?, pg_method=?, pg_raw=?, paid_at=datetime('now')
       WHERE order_id=?`).bind(d.paymentKey||n,d.method||``,JSON.stringify(d),r).run();let f=((await t.prepare(`SELECT credits FROM users WHERE id=?`).bind(a.user_id).first())?.credits??0)+Number(a.credits);return await t.prepare(`UPDATE users SET credits=?, updated_at=datetime('now') WHERE id=?`).bind(f,a.user_id).run(),await t.prepare(`INSERT INTO credit_logs (user_id, type, amount, balance, reason, ref_id)
       VALUES (?, 'grant', ?, ?, 'payment', ?)`).bind(a.user_id,a.credits,f,r).run(),e.redirect(`/payment/success?orderId=${encodeURIComponent(r)}`,302)}catch(t){return console.error(`payment/toss/success error:`,t),e.redirect(`/payment/fail?message=%EA%B2%B0%EC%A0%9C%20%EC%B2%98%EB%A6%AC%20%EC%A4%91%20%EC%98%A4%EB%A5%98%EA%B0%80%20%EB%B0%9C%EC%83%9D%ED%96%88%EC%8A%B5%EB%8B%88%EB%8B%A4.`,302)}}),V.get(`/payment/toss/fail`,e=>{let t=e.req.query(`message`)||`결제가 취소되었습니다.`,n=e.req.query(`code`)||``;return e.redirect(`/payment/fail?message=${encodeURIComponent(t)}&code=${encodeURIComponent(n)}`,302)}),V.post(`/payment/toss/webhook`,async e=>{let t=e.env.LOOKBOOK_DB,n=()=>e.text(`OK`,200);try{let r=await e.req.text(),i={};try{i=JSON.parse(r)}catch{return n()}if(String(i.eventType||``)!==`PAYMENT_STATUS_CHANGED`)return n();let a=i.data||{},o=String(a.orderId||``),s=String(a.paymentKey||``),l=String(a.status||``);if(!(l===`CANCELED`||l===`PARTIAL_CANCELED`)||!o||!s)return n();let u=await t.prepare(`SELECT id, user_id, credits, status FROM payment_logs WHERE order_id = ? OR payment_key = ?`).bind(o,s).first();if(!u)return console.error(`토스 웹훅: 결제 내역을 찾을 수 없음:`,o,s),n();if(u.status===`canceled`)return n();let d=e.env.TOSS_API_BASE||`https://api.tosspayments.com`,f=e.env.TOSS_SECRET_KEY||``,p=await fetch(`${d}/v1/payments/${encodeURIComponent(s)}`,{headers:{Authorization:`Basic `+btoa(`${f}:`)}});if(!p.ok)return console.error(`토스 웹훅: 결제 조회 API 실패 — 크레딧 처리 보류:`,s,p.status),n();let m=await p.json();if(m.orderId!==o||m.status!==`CANCELED`&&m.status!==`PARTIAL_CANCELED`)return console.error(`토스 웹훅: 조회 결과가 웹훅 내용과 불일치 — 위조 의심, 크레딧 처리 건너뜀:`,o,m.orderId,m.status),n();let h=(await t.prepare(`SELECT credits FROM users WHERE id=?`).bind(u.user_id).first())?.credits??0,g=Number(u.credits)||0,_=Math.min(g,h),v=Math.max(0,h-_);return _>0&&(await t.prepare(`UPDATE users SET credits=?, updated_at=datetime('now') WHERE id=?`).bind(v,u.user_id).run(),await t.prepare(`INSERT INTO credit_logs (user_id, type, amount, balance, reason, ref_id)
         VALUES (?, 'revoke', ?, ?, 'payment_cancel', ?)`).bind(u.user_id,-_,v,o).run()),await t.prepare(`UPDATE payment_logs SET status='canceled' WHERE id=?`).bind(u.id).run(),console.log(`토스 웹훅: 결제취소 처리 완료 — orderId=${o}, 회수 크레딧=${_}`),n()}catch(e){return console.error(`payment/toss/webhook error:`,e),n()}}),V.get(`/api/admin/debug/recent-payments`,W,async e=>{let t=e.env.LOOKBOOK_DB;try{let n=await t.prepare(`SELECT p.order_id, p.user_id, u.email, u.name, p.amount, p.credits, p.status,
       ...
       LIMIT 50`).all();return e.json({success:!0,jobs:n.results||[]})}catch(t){return e.json({success:!1,message:t.message},500)}}),V.get(`/api/admin/debug/atlas-job/:jobId`,W,async e=>{let t=e.req.param(`jobId`),n=e.env.LOOKBOOK_DB;try{let r=await n.prepare(`SELECT id, job_id, kind, status, video_url, created_at FROM generation_logs WHERE job_id = ?`).bind(t).first(),i=await fetch(`${H}/api/v1/model/prediction/${t}`,{headers:{Authorization:`Bearer ${e.env.ATLAS_API_KEY}`}}),a=await i.text(),o=null;try{o=JSON.parse(a)}catch{}return e.json({dbRow:r,atlasHttpStatus:i.status,atlasRaw:o??a})}catch(t){return e.json({success:!1,message:t.message},500)}});async function hr(e){try{let t=await e.prepare(`SELECT value FROM app_settings WHERE key = 'admin_prompt_config'`).first();if(t?.value){let e=JSON.parse(t.value);return{enabled:typeof e.enabled==`boolean`?e.enabled:G.enabled,prefix:typeof e.prefix==`string`?e.prefix:G.prefix,suffix:typeof e.suffix==`string`?e.suffix:G.suffix,styleGuide:typeof e.styleGuide==`string`?e.styleGuide:G.styleGuide,technicalSpec:typeof e.technicalSpec==`string`?e.technicalSpec:G.technicalSpec,updatedAt:e.updatedAt||G.updatedAt}}}catch(e){console.warn(`d1GetPromptConfig fallback to memory:`,e)}return G}async function gr(e,t){await e.prepare(`INSERT INTO app_settings (key, value, updated_at) VALUES ('admin_prompt_config', ?, datetime('now'))
     ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = excluded.updated_at`).bind(JSON.stringify(t)).run()}V.get(`/api/admin/prompt`,W,async e=>{try{let t=e.env.LOOKBOOK_DB,n=await hr(t);return e.json({success:!0,config:n})}catch{return e.json({success:!0,config:G})}}),V.put(`/api/admin/prompt`,W,async e=>{try{let t=e.env.LOOKBOOK_DB,n=await e.req.json(),r=await hr(t),i={enabled:typeof n.enabled==`boolean`?n.enabled:r.enabled,prefix:typeof n.prefix==`string`?n.prefix:r.prefix,suffix:typeof n.suffix==`string`?n.suffix:r.suffix,styleGuide:typeof n.styleGuide==`string`?n.styleGuide:r.styleGuide,technicalSpec:typeof n.technicalSpec==`string`?n.technicalSpec:r.technicalSpec,updatedAt:new Date().toISOString()};return await gr(t,i),G=i,console.log(`Admin prompt config saved to D1:`,i.updatedAt),e.json({success:!0,config:i})}catch(t){return e.json({success:!1,message:t.message},400)}}),V.post(`/api/admin/auth`,async e=>{let t=await e.req.json(),n=e.env.ADMIN_PASSWORD;return n?t.password===n?e.json({success:!0}):e.json({success:!1,message:`비밀번호가 올바르지 않습니다.`},401):e.json({success:!1,message:`서버 설정 오류: ADMIN_PASSWORD 환경변수가 설정되지 않았습니다.`},500)});var _r=`의류 이미지 하나로 AI 온모델 피팅컷과 룩북 세트를 자동 생성하세요.`,vr=(e,t,n=``,r=_r,i)=>`<!DOCTYPE html>
...
<head>
  ...
  <link rel="icon" type="image/svg+xml" href="/static/favicon.svg?v=${Bt}" />
  <link rel="manifest" href="/static/manifest.webmanifest?v=${Bt}" />
  <link rel="apple-touch-icon" href="/static/apple-touch-icon.png?v=${Bt}" />
  <meta name="theme-color" content="#EF4444" />
  <link rel="preconnect" href="https://fonts.googleapis.com" />
  <link href="https://fonts.googleapis.com/css2?family=Pretendard:wght@300;400;500;600;700;800&display=swap" rel="stylesheet" />
  <link href="https://cdn.jsdelivr.net/npm/@fortawesome/fontawesome-free@6.4.0/css/all.min.css" rel="stylesheet" />
  <link href="/static/style.css?v=${Bt}" rel="stylesheet" />
  ${Ht(i)}
  <!-- app.js는 head에 defer — body 인라인 script보다 항상 먼저 파싱·실행됨 -->
  <script src="/static/app.js?v=${Bt}" defer><\/script>
  ${n}
</head>
<body>
${t}
...
`;return e.text(t,200,{"Content-Type":`text/markdown; charset=utf-8`})});var yr=[{q:`EZlook은 어떤 서비스인가요?`,a:`의류 이미지 한 장을 업로드하면 AI가 온모델 피팅컷과 룩북 세트를 자동으로 생성해주는 AI 패션 이미지 생성 플랫폼입니다. 촬영 스튜디오나 모델 섭외 없이 몇 번의 클릭만으로 전문적인 착용샷을 만들 수 있습니다.`},{q:`이미지 생성에 비용이 드나요?`,a:`이미지 생성 자체는 무료입니다. 마음에 드는 결과물을 실제 파일로 다운로드할 때만 장당 90크레딧이 차감됩니다.`},{q:`무료로 체험할 수 있나요?`,a:`네, 신용카드 등록 없이 회원가입만 하면 무료 크레딧이 바로 지급되어 AI 룩북 제작을 체험해볼 수 있습니다.`},{q:`영상도 만들 수 있나요?`,a:`네, 생성된 피팅컷 이미지를 기반으로 모델이 자연스럽게 포즈를 취하는 7초 분량의 세로형(9:16) 영상을 만들 수 있습니다.`},{q:`결과물은 얼마나 빨리 나오나요?`,a:`평균 30초, 최대 90초 이내에 고품질 온모델 피팅컷 이미지가 생성됩니다.`},{q:`크레딧은 어떻게 충전하나요?`,a:`월 정액 없이 필요한 만큼만 충전하는 방식입니다. 스타터(₩20,000 · 1,000크레딧)부터 베스트 밸류(₩60,000 · 4,000크레딧)까지 선택할 수 있습니다.`}],br=()=>{let e={"@type":`Organization`,name:`EZlook`,alternateName:`벌거벗은호랑이`,url:U,logo:`${U}/static/favicon.svg`,telephone:`070-4581-8166`,address:{"@type":`PostalAddress`,streetAddress:`무심서로 377-3`,addressLocality:`청주시 서원구`,addressRegion:`충청북도`,addressCountry:`KR`}};return[{"@context":`https://schema.org`,"@type":`WebSite`,name:`EZlook`,url:U,publisher:e},{"@context":`https://schema.org`,"@type":`Service`,name:`EZlook AI 패션 룩북 생성 서비스`,serviceType:`AI 패션 이미지/영상 생성`,description:`옷 사진 한 장을 업로드하면 AI 모델이 착용한 온모델 피팅컷과 룩북 세트, 홍보 영상을 자동으로 생성하는 서비스`,provider:e,areaServed:`KR`,offers:[{"@type":`Offer`,name:`스타터`,price:`20000`,priceCurrency:`KRW`,description:`1,000 크레딧 · 이미지 최대 11장`},{"@type":`Offer`,name:`인기 충전권`,price:`40000`,priceCurrency:`KRW`,description:`2,300 크레딧 · 이미지 최대 25장`},{"@type":`Offer`,name:`베스트 밸류`,price:`60000`,priceCurrency:`KRW`,description:`4,000 크레딧 · 이미지 최대 44장`}]},{"@context":`https://schema.org`,"@type":`FAQPage`,mainEntity:yr.map(e=>({"@type":`Question`,name:e.q,acceptedAnswer:{"@type":`Answer`,text:e.a}}))}].map(e=>`<script type="application/ld+json">${JSON.stringify(e)}<\/script>`).join(`
      ...
      <div class="navbar-actions" style="position:relative;">
        ...
        <div id="navUserArea" style="display:none;align-items:center;gap:0;position:relative;">
          ...
          <div id="userDropdownMenu" style="display:none;position:absolute;top:44px;right:0;background:#FFFFFF;border:1px solid #E5E8EB;border-radius:16px;padding:6px;min-width:220px;box-shadow:0 12px 32px rgba(15,23,42,0.16);z-index:2000;">
            ...
            ${Vt(e)?`<a href="/techpack" onclick="document.getElementById('userDropdownMenu').style.display='none';" style="display:block;padding:10px 14px;font-size:14px;color:#333D4B;text-decoration:none;border-radius:10px;" onmouseover="this.style.background='#F2F4F6'" onmouseout="this.style.background=''">도식화 만들기</a>`:``}
  ...
  `,t,`옷 사진 한 장으로 AI 온모델 피팅컷과 룩북 세트를 무료로 자동 생성하세요. 촬영 스튜디오나 모델 섭외 없이 평균 30초 만에 완성됩니다.`,e.env.GA4_MEASUREMENT_ID))}),V.get(`/dashboard`,e=>e.html(vr(`내 프로필`,`
  <div class="toast-container" id="toastContainer"></div>
        ...
        <div style="display:flex;align-items:center;gap:8px;position:relative;flex-shrink:0;">
          ...
          <div id="navUserArea" style="display:none;align-items:center;gap:0;position:relative;">
            ...
            <div id="userDropdownMenu" style="display:none;position:absolute;top:40px;right:0;background:#FFFFFF;border:1px solid #E5E8EB;border-radius:16px;padding:6px;min-width:210px;box-shadow:0 12px 32px rgba(15,23,42,0.16);z-index:600;">
              ...
              ${Vt(e)?`<a href="/techpack" onclick="document.getElementById('userDropdownMenu').style.display='none';" style="display:block;padding:9px 12px;font-size:13px;color:#333D4B;text-decoration:none;border-radius:10px;" onmouseover="this.style.background='#F2F4F6'" onmouseout="this.style.background=''">도식화 만들기</a>`:``}
  ...
  <\/script>
  `,``,_r,e.env.GA4_MEASUREMENT_ID))),V.get(`/credits`,e=>e.html(vr(`크레딧 상세`,`
  <div class="toast-container" id="toastContainer"></div>
      ...
      <div style="display:flex;align-items:center;gap:8px;position:relative;">
        ...
        <div id="navUserArea" style="display:none;align-items:center;gap:0;position:relative;">
          ...
          <div id="userDropdownMenu" style="display:none;position:absolute;top:38px;right:0;background:var(--gapp-surface,#FFFFFF);border:1px solid var(--gapp-border,#E5E8EB);border-radius:16px;padding:6px;min-width:210px;box-shadow:0 12px 32px rgba(15,23,42,0.16);z-index:10001;">
            ...
            ${Vt(e)?`<a href="/techpack" onclick="document.getElementById('userDropdownMenu').style.display='none';" style="display:block;padding:9px 12px;font-size:13px;color:var(--gapp-text-secondary,#333D4B);text-decoration:none;border-radius:10px;" onmouseover="this.style.background='var(--gapp-surface-2,#F2F4F6)'" onmouseout="this.style.background=''">도식화 만들기</a>`:``}
            <a href="/dashboard#history" onclick="document.getElementById('userDropdownMenu').style.display='none';" style="display:block;padding:9px 12px;font-size:13px;color:var(--gapp-text-secondary,#333D4B);text-decoration:none;border-radius:10px;" onmouseover="this.style.background='var(--gapp-surface-2,#F2F4F6)'" onmouseout="this.style.background=''" data-i18n="nav-history">생성 내역</a>
  ...
  `,a,n,e.env.GA4_MEASUREMENT_ID))};V.get(`/`,e=>xr(e,`model`)),V.get(`/generator`,e=>e.redirect(`/`,301)),V.get(`/ghostcut`,e=>xr(e,`ghostcut`)),V.get(`/techpack`,e=>{let t=`<link rel="canonical" href="${U}/techpack" />`;return e.html(vr(`AI 도식화 만들기`,`
  <div class="toast-container" id="toastContainer"></div>
  ...
  <script>
    ...
    if (window.innerWidth > 1024) {
      window.location.replace(${JSON.stringify(Ut)})
    }
  <\/script>
...
</html>`)),V.get(`/admin/leads`,e=>e.redirect(`/admin02`,302)),V.get(`/admin`,e=>e.redirect(`/admin02`,302)),V.get(`/bfm-admin`,e=>e.html(`<!DOCTYPE html>
<html lang="ko">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
...
</html>`)),V.get(`/studio-b`,e=>e.redirect(`/`)),V.get(`/studio-b/*`,e=>{let t=e.req.path.replace(`/studio-b`,``)||`/`;return e.redirect(t)}),V.get(`/payment/success`,e=>e.html(`<!DOCTYPE html>
<html lang="ko">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
...
</html>`));var Sr=new ke,Cr=Object.assign({"/src/index.tsx":V}),wr=!1;for(let[,e]of Object.entries(Cr))e&&(Sr.all(`*`,t=>{let n;try{n=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,n)}),Sr.notFound(t=>{let n;try{n=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,n)}),wr=!0);if(!wr)throw Error(`Can't import modules from ['/src/index.ts','/src/index.tsx','/app/server.ts']`);export{Sr as default};