...
`),200,{"Content-Type":`text/csv; charset=utf-8`,"Content-Disposition":`attachment; filename="brand_leads_export.csv"`})});var Ue=`msyl919a`,We=e=>e?`
...
<\/script>
       ...
       VALUES (?, 'deduct', ?, ?, 'image_download', ?)`).bind(r.user_id,-u,d,`dl_${Date.now()}`).run(),s&&o!==void 0&&(l.push(o),await t.prepare(`UPDATE generation_logs SET downloaded_indices = ? WHERE id = ?`).bind(JSON.stringify(l),s.id).run()),console.log(`[Credits] Download deduct: ${r.name} ${r.credits} → ${d} (-${u})`),e.json({success:!0,creditsUsed:u,creditsRemaining:d,alreadyDownloaded:!1})}catch(t){return e.json({success:!1,message:t.message},500)}}),W.delete(`/api/generation/history/:id`,async e=>{try{let t=e.env.LOOKBOOK_DB,n=e.req.header(`X-Session-Token`)||``;if(!n)return e.json({error:`로그인이 필요합니다.`},401);let r=await t.prepare(`SELECT user_id FROM user_sessions WHERE token = ? AND expires_at > datetime('now')`).bind(n).first();if(!r)return e.json({error:`세션이 만료되었습니다.`},401);let i=e.req.param(`id`);return await t.prepare(`DELETE FROM generation_logs WHERE id = ? AND user_id = ?`).bind(i,r.user_id).run(),e.json({success:!0})}catch(t){return e.json({success:!1,message:t.message},500)}});var Ft={pkg_20000:{amount:2e4,credits:1e3,label:`20,000원 → 1,000크레딧`,usdCents:1999,jpyAmount:2980},pkg_40000:{amount:4e4,credits:2300,label:`40,000원 → 2,300크레딧`,usdCents:3499,jpyAmount:4980},pkg_60000:{amount:6e4,credits:4e3,label:`60,000원 → 4,000크레딧`,usdCents:4999,jpyAmount:7980}};W.get(`/api/payments/packages`,e=>e.json({success:!0,packages:Ft})),W.post(`/api/payments/prepare`,async e=>{try{let t=e.env.LOOKBOOK_DB,n=e.req.header(`X-Session-Token`)||``;if(!n)return e.json({error:`로그인이 필요합니다.`},401);let r=await t.prepare(`SELECT s.user_id, u.name, u.email, u.referrer FROM user_sessions s
       JOIN users u ON u.id = s.user_id
       WHERE s.token = ? AND s.expires_at > datetime('now')`).bind(n).first();if(!r)return e.json({error:`세션이 만료되었습니다.`},401);let{packageId:i}=await e.req.json(),a=Ft[i];if(!a)return e.json({error:`잘못된 패키지입니다.`},400);let o=Pt[r.referrer]||0,s=o>0?Math.round(a.amount*(1-o)):a.amount,l=await t.prepare(`SELECT order_id FROM payment_logs
...
`;return e.text(t,200,{"Content-Type":`text/markdown; charset=utf-8`})});var Xt=[{q:`EZlook은 어떤 서비스인가요?`,a:`의류 이미지 한 장을 업로드하면 AI가 온모델 피팅컷과 룩북 세트를 자동으로 생성해주는 AI 패션 이미지 생성 플랫폼입니다. 촬영 스튜디오나 모델 섭외 없이 몇 번의 클릭만으로 전문적인 착용샷을 만들 수 있습니다.`},{q:`이미지 생성에 비용이 드나요?`,a:`이미지 생성 자체는 무료입니다. 마음에 드는 결과물을 실제 파일로 다운로드할 때만 장당 90크레딧이 차감됩니다.`},{q:`무료로 체험할 수 있나요?`,a:`네, 신용카드 등록 없이 회원가입만 하면 무료 크레딧이 바로 지급되어 AI 룩북 제작을 체험해볼 수 있습니다.`},{q:`영상도 만들 수 있나요?`,a:`네, 생성된 피팅컷 이미지를 기반으로 모델이 자연스럽게 포즈를 취하는 7초 분량의 세로형(9:16) 영상을 만들 수 있습니다.`},{q:`결과물은 얼마나 빨리 나오나요?`,a:`평균 30초, 최대 90초 이내에 고품질 온모델 피팅컷 이미지가 생성됩니다.`},{q:`크레딧은 어떻게 충전하나요?`,a:`월 정액 없이 필요한 만큼만 충전하는 방식입니다. 스타터(₩20,000 · 1,000크레딧)부터 베스트 밸류(₩60,000 · 4,000크레딧)까지 선택할 수 있습니다.`}],Zt=()=>{let e={"@type":`Organization`,name:`EZlook`,alternateName:`벌거벗은호랑이`,url:K,logo:`${K}/static/favicon.svg`,telephone:`070-4581-8166`,address:{"@type":`PostalAddress`,streetAddress:`무심서로 377-3`,addressLocality:`청주시 서원구`,addressRegion:`충청북도`,addressCountry:`KR`}};return[{"@context":`https://schema.org`,"@type":`WebSite`,name:`EZlook`,url:K,publisher:e},{"@context":`https://schema.org`,"@type":`Service`,name:`EZlook AI 패션 룩북 생성 서비스`,serviceType:`AI 패션 이미지/영상 생성`,description:`옷 사진 한 장을 업로드하면 AI 모델이 착용한 온모델 피팅컷과 룩북 세트, 홍보 영상을 자동으로 생성하는 서비스`,provider:e,areaServed:`KR`,offers:[{"@type":`Offer`,name:`스타터`,price:`20000`,priceCurrency:`KRW`,description:`1,000 크레딧 · 이미지 최대 11장`},{"@type":`Offer`,name:`인기 충전권`,price:`40000`,priceCurrency:`KRW`,description:`2,300 크레딧 · 이미지 최대 25장`},{"@type":`Offer`,name:`베스트 밸류`,price:`60000`,priceCurrency:`KRW`,description:`4,000 크레딧 · 이미지 최대 44장`}]},{"@context":`https://schema.org`,"@type":`FAQPage`,mainEntity:Xt.map(e=>({"@type":`Question`,name:e.q,acceptedAnswer:{"@type":`Answer`,text:e.a}}))}].map(e=>`<script type="application/ld+json">${JSON.stringify(e)}<\/script>`).join(`
  ...
  `,t,`옷 사진 한 장으로 AI 온모델 피팅컷과 룩북 세트를 무료로 자동 생성하세요. 촬영 스튜디오나 모델 섭외 없이 평균 30초 만에 완성됩니다.`,e.env.GA4_MEASUREMENT_ID))}),W.get(`/dashboard`,e=>e.html(Yt(`내 프로필`,`
  <div class="toast-container" id="toastContainer"></div>
  ...
  <\/script>
  `,``,Jt,e.env.GA4_MEASUREMENT_ID))),W.get(`/generator`,e=>e.html(Yt(`무료 AI 룩북 생성기`,`
  <div class="toast-container" id="toastContainer"></div>
...
</html>`));var Qt=new Se,$t=Object.assign({"/src/index.tsx":W}),en=!1;for(let[,e]of Object.entries($t))e&&(Qt.all(`*`,t=>{let n;try{n=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,n)}),Qt.notFound(t=>{let n;try{n=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,n)}),en=!0);if(!en)throw Error(`Can't import modules from ['/src/index.ts','/src/index.tsx','/app/server.ts']`);export{Qt as default};