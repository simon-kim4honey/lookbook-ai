...
`,pt=`CASE WHEN email LIKE '%@%' AND email NOT LIKE '%**%' THEN email ELSE crawled_email END`,mt=`COALESCE(NULLIF(bzmnNm, ''), NULLIF(domain_clean, ''), '거래처')`;V.get(`/mail-batch/status`,async e=>{let t=e.env.LOOKBOOK_DB,n=(await t.prepare(`SELECT COUNT(*) AS n FROM biz_leads WHERE ${ft}`).first())?.n||0,r=await t.prepare(`
  ...
  `).bind(t).all();if(!n.length)return e.json({success:!1,message:`해당 배치를 찾을 수 없습니다.`},404);let r=ct(n.map(e=>({name:e.name,email:e.email})));return new Response(r,{status:200,headers:{"Content-Type":`application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`,"Content-Disposition":`attachment; filename="bizleads_mail_batch_${t}.xlsx"`,"X-Batch-Id":String(t),"X-Batch-Count":String(n.length)}})});var ht=`mt5ms7os`,gt=e=>e?`
