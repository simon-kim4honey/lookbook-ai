...
{"overallSummary": string, "keywords": string[], "items": [{"idx": number, "category": string, "summary": string, "importance": number}]}`,r=await fetch(`https://api.anthropic.com/v1/messages`,{method:`POST`,headers:{"Content-Type":`application/json`,"x-api-key":e.ANTHROPIC_API_KEY,"anthropic-version":`2023-06-01`},body:JSON.stringify({model:`claude-sonnet-5`,max_tokens:3e3,messages:[{role:`user`,content:n}]}),signal:AbortSignal.timeout(45e3)});if(!r.ok)throw Error(`Claude API 오류: HTTP ${r.status}`);let i=((await r.json())?.content?.[0]?.text||`{}`).replace(/^```json\s*|```$/g,``).trim(),a=JSON.parse(i),o=(a.items||[]).filter(e=>e.idx>=1&&e.idx<=t.length).map(e=>{let n=t[e.idx-1];return{category:St.includes(e.category)?e.category:`트렌드`,title:n.title,source:n.source||``,url:n.link,published_at:n.pubDate,summary:e.summary||``,importance:Math.max(1,Math.min(5,Number(e.importance)||3))}});return{summary:a.overallSummary||``,keywords:a.keywords||[],items:o}}async function wt(e){let{summary:t,keywords:n,items:r}=await Ct(e,await xt(7)),i=new Date,a=`${i.getFullYear()}년 ${i.getMonth()+1}월 ${Math.ceil(i.getDate()/7)}주차`,o=e.LOOKBOOK_DB,s=(await o.prepare(`INSERT INTO content_digests (period, status, summary, keywords) VALUES (?, 'draft', ?, ?)`).bind(a,t,JSON.stringify(n)).run()).meta.last_row_id;for(let e of r)await o.prepare(`INSERT INTO digest_articles (digest_id, category, title, source, url, published_at, summary, importance)
       VALUES (?,?,?,?,?,?,?,?)`).bind(s,e.category,e.title,e.source,e.url,e.published_at,e.summary,e.importance).run();return{digestId:s,articleCount:r.length}}V.post(`/generate`,async e=>{try{let t=await wt(e.env);return e.json({success:!0,...t})}catch(t){return e.json({success:!1,message:String(t?.message||t)},500)}}),V.get(`/debug-rss`,async e=>{let t=e.req.query(`url`),n=e.req.query(`kw`)||gt[0],r=t||`https://news.google.com/rss/search?q=${encodeURIComponent(n)}&hl=ko&gl=KR&ceid=KR:ko`;try{let t=await fetch(r,{headers:{"User-Agent":`Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36`,Accept:`application/rss+xml, application/xml, text/xml, */*`,"Accept-Language":`ko-KR,ko;q=0.9,en-US;q=0.8,en;q=0.7`},signal:AbortSignal.timeout(12e3)}),i=await t.text(),a=yt(i);return e.json({success:!0,keyword:n,url:r,httpStatus:t.status,contentType:t.headers.get(`content-type`),bodyLength:i.length,bodyPreview:i.slice(0,800),parsedItemCount:a.length,firstItems:a.slice(0,3)})}catch(t){return e.json({success:!1,keyword:n,url:r,error:String(t?.message||t)})}}),V.get(`/list`,async e=>{let{results:t}=await e.env.LOOKBOOK_DB.prepare(`SELECT d.*, (SELECT COUNT(*) FROM digest_articles a WHERE a.digest_id = d.id AND a.excluded = 0) AS article_count
...
`))});var Tt=`mu2nd2jp`;function Et(e){return e.env.TECHPACK_MENU_VISIBLE===`true`}var Dt=e=>e?`
