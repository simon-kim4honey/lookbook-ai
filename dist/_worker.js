...
</html>`;K.get(`/share/:jobId/:idx`,async e=>{let t=e.env.LOOKBOOK_DB,n=e.req.param(`jobId`),r=parseInt(e.req.param(`idx`)||`0`,10),i=t=>{let i=`${$(e)}/share/${n}/${r}`,a=``;if(t.state===`ok`){let e=t.tabs||[];a=`
        <div class="share-card">
          ${e.length>1?`
          <div class="share-tabs">
            ${e.map((e,t)=>`<button class="share-tab${t===0?` active`:``}" data-url="${e.url}" onclick="_switchShareTab(this)">${e.label}</button>`).join(``)}
          </div>`:``}
          <img id="shareMainImg" src="${t.imageUrl}" alt="AI 패션 룩북 스튜디오 생성 이미지" class="share-img" />
...
</html>`};if(!n||isNaN(r))return e.html(i({state:`notfound`}),404);try{let a=await t.prepare(`SELECT image_urls, expires_at, model_id, bg_id FROM generation_logs WHERE job_id = ? ORDER BY id DESC LIMIT 1`).bind(n).first();if(!a||!a.image_urls)return e.html(i({state:`notfound`}),404);if(a.expires_at&&new Date(String(a.expires_at).replace(` `,`T`)+`Z`).getTime()<=Date.now())return e.html(i({state:`expired`}));let o=[];try{o=JSON.parse(a.image_urls)}catch{}let s=o[r];if(!s)return e.html(i({state:`notfound`}),404);let l=$(e),u=`${l}/api/proxy/gen-image?url=${encodeURIComponent(s)}`,d=[{label:`생성결과`,url:u}],f=e.env?.LOOKBOOK_KV;if(f){let e=await f.get(`clothing_img:${n}`);if(e)try{let t=JSON.parse(e);t.forEach((e,r)=>{d.push({label:t.length>1?`의상${r+1}`:`의상`,url:`${l}/api/proxy/clothing/${n}/${r}`})})}catch{}}return a.model_id&&d.push({label:`모델`,url:`${l}/api/proxy/custom-model/${a.model_id}`}),a.bg_id&&d.push({label:`배경`,url:`${l}/api/proxy/custom-bg/${a.bg_id}`}),e.html(i({state:`ok`,imageUrl:u,tabs:d}))}catch(t){return console.error(`Share page error:`,t),e.html(i({state:`notfound`}),500)}}),K.get(`/terms`,e=>e.html(`<!DOCTYPE html>
<html lang="ko">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
