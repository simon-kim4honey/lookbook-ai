...
app.post('/api/generation/start', async (c) => {
  try {
    ...
    const HARD_CONSTRAINTS = [
      ...
      `2. Identity is locked to the reference face. This is the model's exact facial identity transformed to the new pose and illumination — not a lookalike, not a recast, and not a fresh generation. Preserve the reference face's identity-defining geometry (bone structure, eye shape, eye spacing, nose shape, lip shape, jawline, cheekbones, facial proportions, and skin tone). Only scene-driven changes are allowed: head angle, tilt, gaze direction, expression, perspective, brightness, shadows, and color temperature. If the result looks like a different person, it is wrong; if it keeps the reference photo's original angle or lighting instead of adapting to the target scene, it is also wrong.`,
    ...
    if (hasClothing && hasModel && hasBg) {
      ...
      prompt = [
        ...
        `  · CRITICAL — ROTATE & RELIGHT THIS EXACT FACE (not a new face): Use Image ${modelImgIdx}'s exact facial identity as the source face and transform THAT SAME face into the new pose and scene lighting. Keep the person's identity-defining structure unchanged — bone structure, eye shape/spacing, nose shape, lip shape, jawline, cheekbones, face width, and skin tone must remain unmistakably the same as Image ${modelImgIdx}. Change only what naturally changes when the same face is seen from a different angle and under different light: head angle, tilt, gaze, expression, perspective, shadows, highlights, exposure, and color temperature. Do NOT introduce a substitute face, an averaged face, or a newly invented variant, and do NOT keep Image ${modelImgIdx}'s original angle or lighting if they conflict with Image ${bgImgIdx}'s scene.`,
        `  · FACE-TO-BODY SCALE: The head/face size relative to the body must match natural human proportions and stay the SAME SCALE as Image ${modelImgIdx} — do NOT enlarge, shrink, or otherwise change the proportion of the face/head relative to the body and shoulders. Anatomically correct head-to-body ratio only.`,
        ...
        `  · COLOR TEMPERATURE & SATURATION: Tint face, skin, and clothing under the scene's color temperature AND saturation level (warm golden-hour tint, cool blue shade, neutral studio white, etc.) — face and body must share identical hue/saturation/brightness at every point, with zero visible tone or saturation boundary anywhere on exposed skin. Do NOT render any element under a different white balance or saturation from the scene.`,
        ...
        `  · GROUNDING: The model must cast a soft contact shadow onto the ground/floor matching Image ${bgImgIdx}'s light direction and shadow softness. Depth-of-field, grain, and sharpness falloff on the model must match the background photo's camera characteristics — same focal plane behavior, same lens characteristics.`,
        ...
        `PHOTOREALISM (critical): This must look like an actual photograph taken with a real camera — NOT an AI-generated, CGI, 3D-rendered, or illustrated image. Natural skin texture with visible pores, fine hairs, and subtle asymmetry/imperfections. Natural film grain and lens characteristics. Avoid overly smooth/plastic/airbrushed skin, avoid uncanny-valley symmetry, avoid the glossy "AI look." Shot on a professional camera, candid editorial photography style, not digital art.`,
