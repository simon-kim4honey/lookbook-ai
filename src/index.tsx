...
app.post('/api/generation/start', async (c) => {
  try {
    ...
    const HARD_CONSTRAINTS = [
      ...
      `2. This is a RE-PHOTOGRAPH, not a copy-paste: preserve the model's unique facial identity (bone structure, eye shape, nose shape, lip shape, face proportions) so the SAME person is recognizable, exactly like the same person shot in two different real photos. Head angle, tilt, gaze direction, brightness, and color-temperature MUST differ from the reference photo to match the new scene — copying the reference photo's exact angle/lighting is WRONG, just as generating a different-looking person is WRONG.`,
    ...
    if (hasClothing && hasModel && hasBg) {
      ...
      prompt = [
        ...
        `  · CRITICAL — SAME PERSON, DIFFERENT PHOTOSHOOT: Think of this like the SAME real person photographed on a DIFFERENT day, in a DIFFERENT location, by a DIFFERENT photographer — same unique face (bone structure, eye shape, nose, lips, jawline, cheekbones, face width, skin tone are all identical), but naturally NEW head angle, NEW expression, NEW lighting because it's a different photo session. Do NOT paste Image ${modelImgIdx}'s face as a flat sticker — RE-PHOTOGRAPH this exact person fresh, as they would actually look under Image ${bgImgIdx}'s scene lighting and from the new pose's angle. A viewer who knows this person would say "that's them, in a different photo" — not "that's a copy-pasted photo" and not "that's someone else."`,
